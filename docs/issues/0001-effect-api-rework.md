...
- [Shared contract foundations](0010-shared-contract-foundations.md) (#10) — **frontier now**; the expand step (branded ids + `Paged`/`Pagination` + 3 `TaggedError`s → `@mamen/shared/contract`). Blocks every port.
- [Port accounts](0011-port-accounts.md) (#11, ⟵ #10) — first DB-backed port; **settles the `:memory:` test-layer wiring + coverage threshold** every later port copies.
