...
Upload mechanics (uploads research): `HttpApiSchema.Multipart` + `SingleFileSchema`; handler gets a scoped-temp `PersistedFile`; MIME allow-list (`jpeg/png/webp/gif`) → `InvalidFileType (415)`; size via `maxFileSize` (2 MiB). `imageUrl` stays a **root-relative `/uploads/issuers/…` path**. Static `/uploads/*` = wildcard route → `HttpServerResponse.file` in the same Bun router, **with the path-traversal guard**. No `Conflict` — merchant name has no unique constraint (faithful port).

Dropped: `PUT /issuers/bulk-put` (client-only).

## Acceptance criteria

- [ ] `MerchantsGroup` matches spec §2.4; all reads/writes through the SDK, tested.
- [ ] `uploadImage` accepts a multipart file, enforces the MIME allow-list (`InvalidFileType` on miss), writes to `uploads/issuers/`, returns the updated `Merchant` with root-relative `imageUrl`.
