...
| Context | Path | Role |
|---|---|---|
| shared | `packages/shared/CONTEXT.md` | The HTTP API contract + shared schemas/types, plus the deployment constants several packages must agree on — the app's path prefix and the **port registry** rows. Source of truth for domain entities. |
