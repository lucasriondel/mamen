...
export default async function transactionRoutes(fastify: FastifyInstance) {
	fastify.get<{
		...
		if (accountId && importMonth) {
			return fastify.db.transactions.getByAccountIdAndMonth(
				Number(accountId),
				importMonth,
			);
		}
		...
		if (linkedRefundId) {
			return fastify.db.transactions.getByLinkedRefundId(
				Number(linkedRefundId),
			);
		}
		if (startDate && endDate) {
			return fastify.db.transactions.getByDateRange(
				new Date(startDate),
				new Date(endDate),
			);
		}
		if (orderBy === "date") {
			return fastify.db.transactions.getAllOrderedByDate(
				direction ?? undefined,
			);
		}
	...
	fastify.get<{
		...
		if (accountId && importMonth) {
			return {
				count: await fastify.db.transactions.countByAccountIdAndMonth(
					Number(accountId),
					importMonth,
				),
			};
		...
		if (importBatchId) {
			return {
				count:
					await fastify.db.transactions.countByImportBatchId(importBatchId),
			};
		}
	...
	fastify.post<{ Body: Omit<Transaction, "id"> }>(
		"/transactions",
		async (request, reply) => {
	...
	fastify.post<{ Body: { records: Omit<Transaction, "id">[] } }>(
		"/transactions/bulk",
		async (request, reply) => {
			const ids = await fastify.db.transactions.bulkAddReturningIds(
	...
	fastify.put<{ Params: { id: string }; Body: Partial<Transaction> }>(
		"/transactions/:id",
		async (request, reply) => {
			const id = Number(request.params.id);
			if (Number.isNaN(id))
	...
	fastify.put<{ Body: { records: Transaction[] } }>(
		"/transactions/bulk-put",
		async (request) => {
	...
	fastify.delete<{
		...
		if (!accountId || !importMonth)
			return reply
				.status(400)
				.send({ error: "accountId and importMonth required" });

		await fastify.db.transactions.deleteByAccountIdAndMonth(
			Number(accountId),
			importMonth,
		);
		return { ok: true };
	...
	fastify.delete<{ Params: { batchId: string } }>(
		"/transactions/by-import-batch/:batchId",
		async (request) => {
			await fastify.db.transactions.deleteByImportBatchId(
