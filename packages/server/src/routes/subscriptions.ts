...
export default async function subscriptionRoutes(fastify: FastifyInstance) {
	fastify.get<{ Querystring: { merchantId?: string; status?: string } }>(
		"/subscriptions",
		async (request) => {
	...
	fastify.post<{ Body: Omit<Subscription, "id"> }>(
		"/subscriptions",
		async (request, reply) => {
	...
	fastify.put<{ Params: { id: string }; Body: Partial<Subscription> }>(
		"/subscriptions/:id",
		async (request, reply) => {
			const id = Number(request.params.id);
			if (Number.isNaN(id))
	...
	fastify.get<{ Params: { merchantId: string } }>(
		"/subscriptions/first-by-merchant/:merchantId",
		async (request, reply) => {
			...
			const sub =
				await fastify.db.subscriptions.getFirstByMerchantId(merchantId);
			if (!sub) return reply.status(404).send({ error: "Not found" });
			return sub;
		},
	);

	fastify.get<{ Params: { merchantId: string; frequency: string } }>(
		"/subscriptions/by-merchant-frequency/:merchantId/:frequency",
		async (request, reply) => {
			const merchantId = Number(request.params.merchantId);
			if (Number.isNaN(merchantId))
				return reply.status(400).send({ error: "Invalid merchantId" });

			const sub = await fastify.db.subscriptions.getByMerchantIdAndFrequency(
				merchantId,
				request.params.frequency as SubscriptionFrequency,
			);
			if (!sub) return reply.status(404).send({ error: "Not found" });
			return sub;
		},
	...
	fastify.put<{ Body: { records: Subscription[] } }>(
		"/subscriptions/bulk-put",
		async (request) => {
