...
export default async function ruleRoutes(fastify: FastifyInstance) {
	fastify.get<{ Querystring: { merchantId?: string } }>(
		"/rules",
		async (request) => {
			if (request.query.merchantId) {
				return fastify.db.rules.getByMerchantId(
					Number(request.query.merchantId),
				);
			}
	...
	fastify.get<{ Querystring: { merchantId?: string } }>(
		...
		async (request) => {
			if (request.query.merchantId) {
				return {
					count: await fastify.db.rules.countByMerchantId(
						Number(request.query.merchantId),
					),
				};
			}
	...
	fastify.put<{ Params: { id: string }; Body: Partial<Rule> }>(
		"/rules/:id",
		async (request, reply) => {
			const id = Number(request.params.id);
			if (Number.isNaN(id))
	...
	fastify.post<{ Body: { records: Omit<Rule, "id">[] } }>(
		"/rules/bulk-add",
		async (request, reply) => {
	...
	fastify.get<{ Params: { merchantId: string; pattern: string } }>(
		"/rules/by-merchant-pattern/:merchantId/:pattern",
		async (request, reply) => {
			...
			const rule = await fastify.db.rules.getByMerchantIdAndPattern(
				merchantId,
				decodeURIComponent(request.params.pattern),
			);
			if (!rule) return reply.status(404).send({ error: "Not found" });
			return rule;
		},
