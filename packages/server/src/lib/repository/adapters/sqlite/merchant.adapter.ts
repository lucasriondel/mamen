...
export const createMerchantAdapter = (db: Database): MerchantRepository => ({
  get: async (id) => {
    const row = db.query<MerchantRow, [number]>('SELECT * FROM merchants WHERE id = ?').get(id)
    return row ? toEntity(row) : undefined
  },

  getAll: async () => {
    return db.query<MerchantRow, []>('SELECT * FROM merchants').all().map(toEntity)
  },

  add: async (record) => {
    const now = new Date().toISOString()
    const result = db.query<{ id: number }, [string, number | null, string, string]>(
  ...
  bulkAdd: async (records) => {
    const stmt = db.query<{ id: number }, [string, number | null, string, string]>(
      'INSERT INTO merchants (name, defaultCategoryId, createdAt, firstSeen) VALUES (?, ?, ?, ?) RETURNING id',
    )
    const now = new Date().toISOString()
  ...
  update: async (id, changes) => {
    ...
    if (changes.defaultCategoryId !== undefined) { fields.push('defaultCategoryId = ?'); values.push(changes.defaultCategoryId ?? null) }
    if (changes.createdAt !== undefined) { fields.push('createdAt = ?'); values.push(changes.createdAt.toISOString()) }
    if (changes.firstSeen !== undefined) { fields.push('firstSeen = ?'); values.push(changes.firstSeen.toISOString()) }
  ...
  bulkPut: async (records) => {
    ...
    const tx = db.transaction(() => {
      for (const record of records) {
        stmt.run(record.id!, record.name, record.defaultCategoryId ?? null, record.createdAt.toISOString(), record.firstSeen.toISOString())
      }
    })
    tx()
  },
  ...
  bulkGet: async (ids) => {
    if (ids.length === 0) return []
    const placeholders = ids.map(() => '?').join(',')
    const rows = db.query<MerchantRow, number[]>(`SELECT * FROM merchants WHERE id IN (${placeholders})`).all(...ids)
    const map = new Map(rows.map((r) => [r.id, toEntity(r)]))
    return ids.map((id) => map.get(id))
  },

  count: async () => {
    return db.query<{ cnt: number }, []>('SELECT COUNT(*) as cnt FROM merchants').get()!.cnt
  },
  ...
  getByName: async (name) => {
    const row = db.query<MerchantRow, [string]>('SELECT * FROM merchants WHERE name = ?').get(name)
    return row ? toEntity(row) : undefined
  },

  getByNameCaseInsensitive: async (name) => {
    const row = db.query<MerchantRow, [string]>('SELECT * FROM merchants WHERE name COLLATE NOCASE = ?').get(name)
    return row ? toEntity(row) : undefined
  },

  getAllOrderedByName: async () => {
    return db.query<MerchantRow, []>('SELECT * FROM merchants ORDER BY name COLLATE NOCASE ASC').all().map(toEntity)
  },
})
