...
export const createRuleAdapter = (db: Database): RuleRepository => ({
  get: async (id) => {
    const row = db.query<RuleRow, [number]>('SELECT * FROM rules WHERE id = ?').get(id)
  ...
  add: async (record) => {
    ...
    const result = db.query<{ id: number }, [number, string, number | null, number, string]>(
  ...
  bulkAdd: async (records) => {
    const stmt = db.query<{ id: number }, [number, string, number | null, number, string]>(
      'INSERT INTO rules (merchantId, pattern, categoryOverride, matchCount, createdAt) VALUES (?, ?, ?, ?, ?) RETURNING id',
    )
    const now = new Date().toISOString()
  ...
  update: async (id, changes) => {
    ...
    if (changes.pattern !== undefined) { fields.push('pattern = ?'); values.push(changes.pattern) }
    if (changes.categoryOverride !== undefined) { fields.push('categoryOverride = ?'); values.push(changes.categoryOverride ?? null) }
    if (changes.matchCount !== undefined) { fields.push('matchCount = ?'); values.push(changes.matchCount) }
  ...
  bulkPut: async (records) => {
    ...
    const tx = db.transaction(() => {
      for (const record of records) {
        stmt.run(record.id!, record.merchantId, record.pattern, record.categoryOverride ?? null, record.matchCount, record.createdAt.toISOString())
      }
    })
    tx()
  },
  ...
  bulkGet: async (ids) => {
    if (ids.length === 0) return []
    const placeholders = ids.map(() => '?').join(',')
    const rows = db.query<RuleRow, number[]>(`SELECT * FROM rules WHERE id IN (${placeholders})`).all(...ids)
    const map = new Map(rows.map((r) => [r.id, toEntity(r)]))
    return ids.map((id) => map.get(id))
  },

  count: async () => {
    return db.query<{ cnt: number }, []>('SELECT COUNT(*) as cnt FROM rules').get()!.cnt
  },
  ...
  getByMerchantId: async (merchantId) => {
    return db.query<RuleRow, [number]>('SELECT * FROM rules WHERE merchantId = ?').all(merchantId).map(toEntity)
  },

  countByMerchantId: async (merchantId) => {
    return db.query<{ cnt: number }, [number]>(
      'SELECT COUNT(*) as cnt FROM rules WHERE merchantId = ?',
    ).get(merchantId)!.cnt
  },

  getByMerchantIdAndPattern: async (merchantId, pattern) => {
    const row = db.query<RuleRow, [number, string]>(
