...
export const createSubscriptionAdapter = (
	db: Database,
	...
	getAll: async () => {
		return db
			.query<SubscriptionRow, []>("SELECT * FROM subscriptions")
			.all()
			.map(toEntity);
	},
	...
	update: async (id, changes) => {
		...
		if (changes.merchantName !== undefined) {
			fields.push("merchantName = ?");
			values.push(changes.merchantName);
		}
		if (changes.typicalAmount !== undefined) {
			fields.push("typicalAmount = ?");
			values.push(changes.typicalAmount);
		}
		if (changes.frequency !== undefined) {
			fields.push("frequency = ?");
			values.push(changes.frequency);
		}
		if (changes.intervalDays !== undefined) {
			fields.push("intervalDays = ?");
			values.push(changes.intervalDays);
		}
		if (changes.lastChargeDate !== undefined) {
			fields.push("lastChargeDate = ?");
			values.push(changes.lastChargeDate);
		}
		if (changes.firstChargeDate !== undefined) {
			fields.push("firstChargeDate = ?");
			values.push(changes.firstChargeDate);
		}
		if (changes.chargeCount !== undefined) {
			fields.push("chargeCount = ?");
			values.push(changes.chargeCount);
		}
		if (changes.status !== undefined) {
			fields.push("status = ?");
			values.push(changes.status);
		}
		if (changes.transactionIds !== undefined) {
			fields.push("transactionIds = ?");
			values.push(JSON.stringify(changes.transactionIds));
		}
		if (changes.detectedAt !== undefined) {
			fields.push("detectedAt = ?");
			values.push(changes.detectedAt);
		}
	...
	getByMerchantId: async (merchantId) => {
		return db
			.query<SubscriptionRow, [number]>(
				"SELECT * FROM subscriptions WHERE merchantId = ?",
			)
			.all(merchantId)
			.map(toEntity);
	},
	...
	getByMerchantIdAndFrequency: async (merchantId, frequency) => {
		const row = db
			.query<SubscriptionRow, [number, string]>(
