...
export const createSettingAdapter = (db: Database): SettingRepository => ({
  get: async (id) => {
    const row = db.query<SettingRow, [number]>('SELECT * FROM settings WHERE id = ?').get(id)
    return row ? toEntity(row) : undefined
  },

  getAll: async () => {
    return db.query<SettingRow, []>('SELECT * FROM settings').all().map(toEntity)
  },

  add: async (record) => {
    const result = db.query<{ id: number }, [string, string]>(
  ...
  update: async (id, changes) => {
    ...
    if (changes.key !== undefined) { fields.push('key = ?'); values.push(changes.key) }
    if (changes.value !== undefined) { fields.push('value = ?'); values.push(changes.value) }
  ...
  bulkGet: async (ids) => {
    if (ids.length === 0) return []
    const placeholders = ids.map(() => '?').join(',')
    const rows = db.query<SettingRow, number[]>(`SELECT * FROM settings WHERE id IN (${placeholders})`).all(...ids)
    const map = new Map(rows.map((r) => [r.id, toEntity(r)]))
    return ids.map((id) => map.get(id))
  },

  count: async () => {
    return db.query<{ cnt: number }, []>('SELECT COUNT(*) as cnt FROM settings').get()!.cnt
  },
