...
export const createAccountAdapter = (db: Database): AccountRepository => ({
	get: async (id) => {
		const row = db
			.query<AccountRow, [number]>("SELECT * FROM accounts WHERE id = ?")
			.get(id);
	...
	getAll: async () => {
		return db
			.query<AccountRow, []>("SELECT * FROM accounts")
			.all()
			.map(toEntity);
	},
	...
	update: async (id, changes) => {
		...
		if (changes.updatedAt !== undefined) {
			...
			values.push(changes.updatedAt.toISOString());
		}
	...
	getByName: async (name) => {
		const row = db
			.query<AccountRow, [string]>("SELECT * FROM accounts WHERE name = ?")
			.get(name);
		return row ? toEntity(row) : undefined;
	},
	...
	getByType: async (type) => {
		return db
			.query<AccountRow, [string]>("SELECT * FROM accounts WHERE type = ?")
			.all(type)
			.map(toEntity);
	},
});
