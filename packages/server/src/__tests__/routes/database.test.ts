...
describe('database routes', () => {
  ...
  describe('POST /api/database/reset', () => {
    it('clears all tables', async () => {
      await app.inject({ method: 'POST', url: '/api/accounts', payload: makeAccount() })
      await app.inject({ method: 'POST', url: '/api/categories', payload: makeCategory() })

      const res = await app.inject({ method: 'POST', url: '/api/database/reset' })
  ...
  describe('POST /api/database/export', () => {
    it('exports all data', async () => {
      ...
      const res = await app.inject({ method: 'POST', url: '/api/database/export' })
      expect(res.statusCode).toBe(200)
  ...
  describe('POST /api/database/import', () => {
    it('import round-trips with export', async () => {
      await app.inject({ method: 'POST', url: '/api/accounts', payload: makeAccount({ name: 'A' }) })
      await app.inject({ method: 'POST', url: '/api/categories', payload: makeCategory({ name: 'C', slug: 'c' }) })

      const exported = await app.inject({ method: 'POST', url: '/api/database/export' })
