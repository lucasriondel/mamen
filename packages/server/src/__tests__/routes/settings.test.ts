...
describe('settings routes', () => {
  ...
  describe('GET /api/settings/by-key/:key', () => {
    it('returns a setting by key', async () => {
      ...
      const res = await app.inject({ method: 'GET', url: '/api/settings/by-key/currency_symbol' })
      expect(res.statusCode).toBe(200)
  ...
  describe('DELETE /api/settings/:id', () => {
    it('deletes a setting', async () => {
      ...
      const getRes = await app.inject({ method: 'GET', url: '/api/settings/by-key/currency_symbol' })
      const { id } = getRes.json()

      const res = await app.inject({ method: 'DELETE', url: `/api/settings/${id}` })
      expect(res.statusCode).toBe(200)
      expect(res.json()).toEqual({ ok: true })
    })

    it('returns 400 for invalid id', async () => {
      const res = await app.inject({ method: 'DELETE', url: '/api/settings/abc' })
  ...
  describe('POST /api/settings/clear', () => {
    it('clears all settings', async () => {
      ...
      const res = await app.inject({ method: 'POST', url: '/api/settings/clear' })
      expect(res.statusCode).toBe(200)
      expect(res.json()).toEqual({ ok: true })
