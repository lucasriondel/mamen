...
const makeRule = (merchantId: number, overrides: Record<string, unknown> = {}) => ({
  merchantId,
  pattern: 'TEST*',
  matchCount: 0,
  createdAt: new Date().toISOString(),
...
describe('rules routes', () => {
  ...
  describe('GET /api/rules', () => {
    ...
    it('filters by merchantId', async () => {
      await app.inject({ method: 'POST', url: '/api/rules', payload: makeRule(merchantId) })

      const res = await app.inject({ method: 'GET', url: `/api/rules?merchantId=${merchantId}` })
      expect(res.statusCode).toBe(200)
  ...
  describe('GET /api/rules/count', () => {
    ...
    it('returns count by merchantId', async () => {
      ...
      const res = await app.inject({ method: 'GET', url: `/api/rules/count?merchantId=${merchantId}` })
  ...
  describe('GET /api/rules/:id', () => {
    it('returns a rule by id', async () => {
      const create = await app.inject({ method: 'POST', url: '/api/rules', payload: makeRule(merchantId, { pattern: 'PAT1' }) })
  ...
  describe('PUT /api/rules/:id', () => {
    it('updates a rule', async () => {
      const create = await app.inject({ method: 'POST', url: '/api/rules', payload: makeRule(merchantId) })
      const { id } = create.json()

      const res = await app.inject({ method: 'PUT', url: `/api/rules/${id}`, payload: { pattern: 'UPDATED*' } })
      expect(res.statusCode).toBe(200)
    ...
    it('returns 400 for invalid id', async () => {
      const res = await app.inject({ method: 'PUT', url: '/api/rules/abc', payload: { pattern: 'X' } })
      expect(res.statusCode).toBe(400)
    })
  })
  ...
  describe('POST /api/rules/bulk-delete', () => {
    it('deletes multiple rules', async () => {
      const r1 = await app.inject({ method: 'POST', url: '/api/rules', payload: makeRule(merchantId, { pattern: 'X' }) })
      const r2 = await app.inject({ method: 'POST', url: '/api/rules', payload: makeRule(merchantId, { pattern: 'Y' }) })
  ...
  describe('GET /api/rules/by-merchant-pattern/:merchantId/:pattern', () => {
    it('returns rule by merchant and pattern', async () => {
      await app.inject({ method: 'POST', url: '/api/rules', payload: makeRule(merchantId, { pattern: 'UNIQUE' }) })

      const res = await app.inject({ method: 'GET', url: `/api/rules/by-merchant-pattern/${merchantId}/UNIQUE` })
      expect(res.statusCode).toBe(200)
      expect(res.json().pattern).toBe('UNIQUE')
    })

    it('returns 400 for invalid merchantId', async () => {
      const res = await app.inject({ method: 'GET', url: '/api/rules/by-merchant-pattern/abc/PAT' })
      expect(res.statusCode).toBe(400)
    })

    it('returns 404 when not found', async () => {
      const res = await app.inject({ method: 'GET', url: `/api/rules/by-merchant-pattern/${merchantId}/MISSING` })
      expect(res.statusCode).toBe(404)
    })
  })
})
