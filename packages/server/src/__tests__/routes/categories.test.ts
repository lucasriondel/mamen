	...
	describe("GET /api/categories", () => {
		...
		it("filters by parentId", async () => {
			const parent = await app.inject({
				method: "POST",
				url: "/api/categories",
				payload: makeCategory({ name: "Parent", slug: "parent" }),
			});
			const parentId = parent.json().id;
			await app.inject({
				method: "POST",
				url: "/api/categories",
				payload: makeCategory({ name: "Child", slug: "child", parentId }),
			});

			const res = await app.inject({
				method: "GET",
				url: `/api/categories?parentId=${parentId}`,
			});
			expect(res.statusCode).toBe(200);
	...
	describe("GET /api/categories/root", () => {
		it("returns root categories", async () => {
			await app.inject({
				method: "POST",
				url: "/api/categories",
				payload: makeCategory({ name: "Root", slug: "root", parentId: null }),
			});
	...
	describe("GET /api/categories/:id", () => {
		it("returns a category by id", async () => {
			const create = await app.inject({
				method: "POST",
				url: "/api/categories",
				payload: makeCategory({ name: "Transport", slug: "transport" }),
			});
	...
	describe("GET /api/categories/by-slug/:slug", () => {
		it("returns category by slug", async () => {
			await app.inject({
				method: "POST",
				url: "/api/categories",
				payload: makeCategory({ name: "Entertainment", slug: "entertainment" }),
			});

			const res = await app.inject({
				method: "GET",
				url: "/api/categories/by-slug/entertainment",
			});
			expect(res.statusCode).toBe(200);
		...
		it("returns 404 when not found", async () => {
			const res = await app.inject({
				method: "GET",
				url: "/api/categories/by-slug/missing",
			});
			expect(res.statusCode).toBe(404);
		});
	});
