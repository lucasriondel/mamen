...
describe("transactions routes", () => {
	...
	describe("GET /api/transactions", () => {
		...
		it("filters by accountId and importMonth", async () => {
			await app.inject({
				method: "POST",
				url: "/api/transactions",
				payload: makeTransaction(accountId, { importMonth: "2025-01" }),
			});
			await app.inject({
				method: "POST",
				url: "/api/transactions",
				payload: makeTransaction(accountId, { importMonth: "2025-02" }),
			});

			const res = await app.inject({
				method: "GET",
				url: `/api/transactions?accountId=${accountId}&importMonth=2025-01`,
			});
			expect(res.statusCode).toBe(200);
			expect(res.json()).toHaveLength(1);
		});
		...
		it("filters by importBatchId", async () => {
			await app.inject({
				method: "POST",
				url: "/api/transactions",
				payload: makeTransaction(accountId, { importBatchId: "b1" }),
			});

			const res = await app.inject({
				method: "GET",
				url: "/api/transactions?importBatchId=b1",
			});
			expect(res.statusCode).toBe(200);
			expect(res.json()).toHaveLength(1);
		});
	...
	describe("GET /api/transactions/count", () => {
		...
		it("returns count by accountId and importMonth", async () => {
			...
			const res = await app.inject({
				method: "GET",
				url: `/api/transactions/count?accountId=${accountId}&importMonth=2025-01`,
			});
		...
		it("returns count by importBatchId", async () => {
			...
			const res = await app.inject({
				method: "GET",
				url: "/api/transactions/count?importBatchId=b1",
			});
			expect(res.json()).toEqual({ count: 1 });
		});
	});
	...
	describe("PUT /api/transactions/bulk-put", () => {
		it("bulk upserts transactions", async () => {
			...
			const res = await app.inject({
				...
				payload: {
					records: [{ id, ...makeTransaction(accountId, { amount: -200 }) }],
				},
			});
			expect(res.statusCode).toBe(200);
	...
	describe("POST /api/transactions/bulk-get", () => {
		it("returns transactions by ids", async () => {
			const r1 = await app.inject({
				method: "POST",
				url: "/api/transactions",
				payload: makeTransaction(accountId, { rawMerchantString: "X" }),
			});
			const r2 = await app.inject({
				method: "POST",
				url: "/api/transactions",
				payload: makeTransaction(accountId, { rawMerchantString: "Y" }),
			});
	...
	describe("DELETE /api/transactions/by-account-month", () => {
		...
		it("returns 400 when accountId or importMonth missing", async () => {
			const res = await app.inject({
				method: "DELETE",
				url: "/api/transactions/by-account-month",
			});
			expect(res.statusCode).toBe(400);
		});
	});
	...
	describe("DELETE /api/transactions/by-import-batch/:batchId", () => {
		it("deletes transactions by import batch", async () => {
			await app.inject({
				method: "POST",
				url: "/api/transactions",
				payload: makeTransaction(accountId, { importBatchId: "del-batch" }),
			});

			const res = await app.inject({
				method: "DELETE",
				url: "/api/transactions/by-import-batch/del-batch",
			});
			expect(res.statusCode).toBe(200);

			const list = await app.inject({
				method: "GET",
				url: "/api/transactions?importBatchId=del-batch",
			});
			expect(list.json()).toEqual([]);
		});
	});
