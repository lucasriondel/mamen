...
export const ruleKeys = {
  ...
  preview: (input: RulePreviewInput) => [...ruleKeys.previews(), input] as const,
};
...
export const ruleQueries = {
  ...
  count: (params: RuleCountParams = {}) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.rules.count({ urlParams: params })),
          signal,
        ),
    }),
  ...
  getById: (id: RuleId) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.rules.getById({ path: { id } })),
          signal,
        ),
    }),
  ...
  deletePreview: (id: RuleId) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.rules.previewDelete({ path: { id } })),
          signal,
        ),
    }),
};
...
export const ruleMutations = {
  ...
  preview: (payload: RulePreviewInput) =>
    runQuery(Effect.flatMap(Client, (client) => client.rules.preview({ payload }))),

  create: (payload: RuleCreate) =>
    runQuery(Effect.flatMap(Client, (client) => client.rules.create({ payload }))),

  update: (id: RuleId, payload: RuleUpdate) =>
    runQuery(Effect.flatMap(Client, (client) => client.rules.update({ path: { id }, payload }))),

  remove: (id: RuleId) =>
    runQuery(Effect.flatMap(Client, (client) => client.rules.remove({ path: { id } }))),
};
