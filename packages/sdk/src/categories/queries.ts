			...
			queryFn: ({ signal }) =>
				runQuery(
					Effect.flatMap(Client, (client) =>
						client.categories.list({ urlParams }),
					),
					signal,
				),
		});
	...
	getById: (id: CategoryId) =>
		queryOptions({
			...
			queryFn: ({ signal }) =>
				runQuery(
					Effect.flatMap(Client, (client) =>
						client.categories.getById({ path: { id } }),
					),
					signal,
				),
		}),
	...
	getBySlug: (slug: string) =>
		queryOptions({
			...
			queryFn: ({ signal }) =>
				runQuery(
					Effect.flatMap(Client, (client) =>
						client.categories.getBySlug({ path: { slug } }),
					),
					signal,
				),
		}),
...
export const categoryMutations = {
	create: (payload: CategoryCreate) =>
		runQuery(
			Effect.flatMap(Client, (client) => client.categories.create({ payload })),
		),

	bulkCreate: (records: CategoryBulkCreate["records"]) =>
		runQuery(
			Effect.flatMap(Client, (client) =>
				client.categories.bulkCreate({ payload: { records } }),
			),
		),
	...
	remove: (id: CategoryId) =>
		runQuery(
			Effect.flatMap(Client, (client) =>
				client.categories.remove({ path: { id } }),
			),
		),
};
