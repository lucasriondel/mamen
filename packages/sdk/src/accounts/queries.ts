			...
			queryFn: ({ signal }) =>
				runQuery(
					Effect.flatMap(Client, (client) =>
						client.accounts.list({ urlParams }),
					),
					signal,
				),
		});
	...
	getById: (id: AccountId) =>
		queryOptions({
			...
			queryFn: ({ signal }) =>
				runQuery(
					Effect.flatMap(Client, (client) =>
						client.accounts.getById({ path: { id } }),
					),
					signal,
				),
		}),
	...
	getByName: (name: string) =>
		queryOptions({
			...
			queryFn: ({ signal }) =>
				runQuery(
					Effect.flatMap(Client, (client) =>
						client.accounts.getByName({ path: { name } }),
					),
					signal,
				),
		}),
...
export const accountMutations = {
	create: (payload: AccountCreate) =>
		runQuery(
			Effect.flatMap(Client, (client) => client.accounts.create({ payload })),
		),

	update: (id: AccountId, payload: AccountUpdate) =>
		runQuery(
			Effect.flatMap(Client, (client) =>
				client.accounts.update({ path: { id }, payload }),
			),
		),
	...
	remove: (id: AccountId) =>
		runQuery(
			Effect.flatMap(Client, (client) =>
				client.accounts.remove({ path: { id } }),
			),
		),
};
