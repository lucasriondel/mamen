...
import { PaginationDefaults, type UNASSIGNED_FILTER } from "@mamen/shared/contract";
...
export type TransactionListParams = {
   ...
   */
  categoryId?: CategoryId | ReadonlyArray<CategoryId> | typeof UNASSIGNED_FILTER;
...
export const transactionKeys = {
  ...
  count: (params: TransactionCountParams) => [...transactionKeys.counts(), params] as const,
...
export const transactionQueries = {
  ...
  list: (params: TransactionListParams = {}) => {
    ...
    return queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.transactions.list({ urlParams })),
          signal,
        ),
    });
  },
  ...
  count: (params: TransactionCountParams = {}) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.transactions.count({ urlParams: params })),
          signal,
        ),
    }),
  ...
  getById: (id: TransactionId) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.transactions.getById({ path: { id } })),
          signal,
        ),
    }),
  ...
  transferCandidates: () =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.transactions.transferCandidates()),
          signal,
        ),
    }),
  ...
  recap: (params: RecapParams = {}) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.transactions.recap({ urlParams: params })),
          signal,
        ),
    }),
  ...
  bulkGet: (ids: ReadonlyArray<TransactionId>) =>
    queryOptions({
      ...
      queryFn: ({ signal }) =>
        runQuery(
          Effect.flatMap(Client, (client) => client.transactions.bulkGet({ payload: { ids } })),
          signal,
        ),
    }),
};
...
export const transactionMutations = {
  create: (payload: TransactionCreate) =>
    runQuery(Effect.flatMap(Client, (client) => client.transactions.create({ payload }))),

  /** Create many at once → the created rows with generated ids (201). */
  bulkCreate: (records: TransactionBulkCreate["records"]) =>
    runQuery(
      Effect.flatMap(Client, (client) => client.transactions.bulkCreate({ payload: { records } })),
    ),

  update: (id: TransactionId, payload: TransactionUpdate) =>
    runQuery(
      Effect.flatMap(Client, (client) => client.transactions.update({ path: { id }, payload })),
    ),

  /** Upsert many full rows by id → `{ count }` written (the client holds the rows). */
  bulkPut: (records: TransactionBulkPut["records"]) =>
    runQuery(
      Effect.flatMap(Client, (client) => client.transactions.bulkPut({ payload: { records } })),
    ),

  remove: (id: TransactionId) =>
    runQuery(Effect.flatMap(Client, (client) => client.transactions.remove({ path: { id } }))),
  ...
  removeManualIssuer: (id: TransactionId) =>
    runQuery(
      Effect.flatMap(Client, (client) => client.transactions.removeManualIssuer({ path: { id } })),
    ),

  /** Delete many by id → `{ count }` actually deleted (unknown ids ignored). */
  bulkDelete: (ids: ReadonlyArray<TransactionId>) =>
    runQuery(
      Effect.flatMap(Client, (client) => client.transactions.bulkDelete({ payload: { ids } })),
    ),
  ...
  linkTransfer: (ids: ReadonlyArray<TransactionId>) =>
    runQuery(
      Effect.flatMap(Client, (client) => client.transactions.linkTransfer({ payload: { ids } })),
    ),
