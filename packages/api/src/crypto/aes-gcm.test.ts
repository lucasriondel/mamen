...
describe("keyFromHex", () => {
	...
	it("refuses a non-hex key of the right length", () => {
		...
		assert.isTrue(
			Option.isNone(keyFromHex(`${"z".repeat(2)}${"a".repeat(62)}`)),
		);
	});
});
...
describe("encrypt / decrypt", () => {
	...
	it("round-trips non-ASCII", () => {
		...
		assert.deepStrictEqual(
			decrypt(blob, key(KEY_A)),
			Option.some("clé-privée-€-🔑"),
		);
	});
	...
	it("cannot read a blob that is too short to be one", () => {
		...
		assert.isTrue(
			Option.isNone(decrypt(Buffer.alloc(20).toString("base64"), key(KEY_A))),
		);
	});
