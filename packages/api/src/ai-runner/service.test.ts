...
const runnerWith = (generate: HostedGenerate) =>
  Layer.mergeAll(AiRunner.Default, SecretsRepo.Default, TaskProvider.Default, ClaudeCodeStub).pipe(
 ...
 */
const chooseHosted = (provider: "anthropic" | "google", model: string, key: string) =>
  Effect.gen(function* () {
...
describe("a hosted failure is not recoverable by trying elsewhere", () => {
  it.effect("a payload the codec refuses is a schema failure, not a vendor one", () => {
    ...
    const { generate } = recorder(() => Promise.resolve({ transactions: [{ date: "nope" }] }));
  ...
  it.effect("a vendor refusal fails the run and calls no one else", () => {
    const { calls, generate } = recorder(() => Promise.reject(new Error("429 rate limited")));
