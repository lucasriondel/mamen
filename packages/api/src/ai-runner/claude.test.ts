...
const tokenOf = (config: ClaudeConfigService) => {
  ...
  return config.token as Effect.Effect<Redacted.Redacted<string>, ClaudeTokenMissingError>;
};
...
describe("the CLI's config reads its token from the credential store", () => {
  ...
  it.effect("fails the call with ClaudeTokenMissingError when none is stored", () =>
    Effect.gen(function* () {
      const config = yield* ClaudeConfig;
  ...
  it.effect("resolves per call, so a token pasted now runs the next call", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(Redacted.value(yield* token), "sk-ant-oat01-3fQ2xLmPqR7v-KjnW8sd");
