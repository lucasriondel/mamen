...
const asObject = (schema: unknown): Record<string, unknown> => schema as Record<string, unknown>;
...
describe("the reason this adapter exists", () => {
  ...
  it("means no mamen package depends on zod", () => {
    ...
    const withZod = packages.filter((name) => {
      ...
      return "zod" in (manifest.dependencies ?? {}) || "zod" in (manifest.devDependencies ?? {});
    });
...
describe("the JSON schema handed to the model", () => {
  ...
  it("leaves a root that already carries a type untouched", () => {
    const root = asObject(effectSchemaCodec(Schema.Struct({ n: Schema.Number })).jsonSchema);
