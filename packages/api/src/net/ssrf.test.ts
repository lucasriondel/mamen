...
describe("assertPublicUrl", () => {
  ...
  it.effect("refuses a non-https URL without resolving anything", () =>
    Effect.gen(function* () {
      const resolver = stubResolver({ "logos.example": ["93.184.216.34"] });
      assert.strictEqual(yield* refusalFor("http://logos.example/acme.png", resolver), "not-https");
  ...
  it.effect("checks a literal IP host directly, without resolving", () =>
    Effect.gen(function* () {
      const resolver = stubResolver({});
      assert.strictEqual(yield* refusalFor("https://127.0.0.1/x.png", resolver), "private-address");
      assert.strictEqual(
        yield* refusalFor("https://169.254.169.254/latest/meta-data/", resolver),
        "private-address",
      );
  ...
  it.effect("checks a bracketed IPv6 literal host", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(yield* refusalFor("https://[::1]/x.png", resolver), "private-address");
      assert.deepStrictEqual(resolver.lookups, []);
    }),
  );
  ...
  it.effect("allows a literal public IP host", () =>
    Effect.gen(function* () {
      const resolver = stubResolver({});
      assert.strictEqual(yield* refusalFor("https://93.184.216.34/x.png", resolver), null);
    }),
  );
  ...
  it.effect("refuses a host that does not resolve", () =>
    Effect.gen(function* () {
      const resolver = stubResolver({});
      assert.strictEqual(yield* refusalFor("https://nope.example/x.png", resolver), "unresolvable");
    }),
  );
