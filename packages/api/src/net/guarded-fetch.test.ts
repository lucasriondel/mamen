...
describe("fetchGuarded", () => {
	...
	it.effect("never lets the platform follow redirects itself", () =>
		Effect.gen(function* () {
			...
			yield* fetchGuarded("https://logos.example/acme.png").pipe(
				Effect.provide(net.layer),
			);
	...
	it.effect("refuses a non-https URL before opening any connection", () =>
		Effect.gen(function* () {
			...
			assert.strictEqual(
				yield* reasonFor("http://logos.example/acme.png", net),
				"not-https",
			);
	...
	it.effect("follows a redirect to another public host", () =>
		Effect.gen(function* () {
			const net = stubNet({
				...
				routes: {
					"https://logos.example/acme.png": () =>
						redirect("https://cdn.example/real.png"),
	...
	it.effect("re-checks the scheme at every redirect hop", () =>
		Effect.gen(function* () {
			const net = stubNet({
				...
				routes: {
					"https://logos.example/acme.png": () =>
						redirect("http://logos.example/downgraded.png"),
					"http://logos.example/downgraded.png": () =>
						new Response(new Uint8Array([1])),
				},
			});
			assert.strictEqual(
		...
		Effect.gen(function* () {
			const net = stubNet({
				...
				routes: {
					...
					"https://logos.example/b/real.png": () =>
						new Response(new Uint8Array([4])),
				},
			});
			const bytes = yield* fetchGuarded(
				"https://logos.example/a/acme.png",
	...
	it.effect("caps the redirect chain", () =>
		Effect.gen(function* () {
			...
			for (let hop = 0; hop <= MAX_REDIRECTS + 1; hop++) {
				routes[`https://logos.example/${hop}`] = () =>
					redirect(`https://logos.example/${hop + 1}`);
			}
			const net = stubNet({ addresses: PUBLIC, routes });
			assert.strictEqual(
				yield* reasonFor("https://logos.example/0", net),
				"too-many-redirects",
			);
	...
	it.effect("follows a chain exactly at the cap", () =>
		Effect.gen(function* () {
			...
			routes[`https://logos.example/${MAX_REDIRECTS}`] = () =>
				new Response(new Uint8Array([42]));
			const net = stubNet({ addresses: PUBLIC, routes });
			const bytes = yield* fetchGuarded("https://logos.example/0").pipe(
				Effect.provide(net.layer),
			);
	...
	it.effect("accepts a body just under the cap", () =>
		Effect.gen(function* () {
			const net = stubNet({
				...
				routes: {
					"https://logos.example/acme.png": () =>
						new Response(new Uint8Array(MAX_FETCH_BYTES)),
				},
			});
			const bytes = yield* fetchGuarded("https://logos.example/acme.png").pipe(
				Effect.provide(net.layer),
	...
	it.effect("aborts an oversized body during streaming, not after", () =>
		Effect.gen(function* () {
			...
			assert.strictEqual(
				yield* reasonFor("https://logos.example/endless.png", net),
				"too-large",
			);
		...
		Effect.gen(function* () {
			const net = stubNet({
				...
				routes: {
					...
					"https://logos.example/blackhole.png": () =>
						new Promise<Response>(() => {}),
				},
			});
			const fiber = yield* Effect.fork(
				reasonFor("https://logos.example/blackhole.png", net),
			);
	...
	it.effect("abandons a body that stalls mid-stream", () =>
		Effect.gen(function* () {
			...
			const fiber = yield* Effect.fork(
				reasonFor("https://logos.example/trickle.png", net),
			);
			yield* TestClock.adjust(FETCH_TIMEOUT);
			assert.strictEqual(yield* Fiber.join(fiber), "timeout");
