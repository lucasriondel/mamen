...
/** One `::`-delimited half as its 16-bit groups, or `null` if any is malformed. */
 ...
 */
export const assertPublicUrl = (raw: string): Effect.Effect<URL, ImageFetchRefused, Outbound> =>
  Effect.gen(function* () {
    ...
    if (addresses.some(isBlockedAddress)) return yield* refuse("private-address");
