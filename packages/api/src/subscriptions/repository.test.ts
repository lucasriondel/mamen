...
describe("SubscriptionRepo", () => {
  it.effect("create assigns an id, then a lookup returns it", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(created.transactionIds, [asTransaction(10), asTransaction(20)]);
  ...
  it.effect("list composes issuerId AND status (both must match)", () =>
    Effect.gen(function* () {
      ...
      yield* repo.create(make({ issuerId: asIssuer(1), status: "possibly-cancelled" }));
  ...
  it.effect("getFirstByIssuer returns the lowest-id match", () =>
    Effect.gen(function* () {
      const repo = yield* SubscriptionRepo;
      const first = yield* repo.create(make({ issuerId: asIssuer(9), issuerName: "first" }));
  ...
  it.effect("update merges the partial and keeps other fields", () =>
    Effect.gen(function* () {
      const repo = yield* SubscriptionRepo;
      const created = yield* repo.create(make({ chargeCount: 3, status: "active" }));
  ...
  it.effect("getFirstByIssuer fails NotFound when the issuer has none", () =>
    Effect.gen(function* () {
      const repo = yield* SubscriptionRepo;
      const error = yield* repo.getFirstByIssuer(asIssuer(404)).pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "subscription", id: asIssuer(404) }));
    }).pipe(Effect.provide(RepoTest)),
  );
  ...
  it.effect("getByIssuerFrequency fails NotFound on no match", () =>
    Effect.gen(function* () {
      ...
      const error = yield* repo.getByIssuerFrequency(asIssuer(1), "weekly").pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "subscription", id: "weekly" }));
    }).pipe(Effect.provide(RepoTest)),
  );
  ...
  it.effect("update fails NotFound on a missing id", () =>
    Effect.gen(function* () {
      const repo = yield* SubscriptionRepo;
      const error = yield* repo.update(asSubscription(404), { chargeCount: 9 }).pipe(Effect.flip);
