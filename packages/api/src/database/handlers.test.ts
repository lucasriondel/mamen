...
describe("database endpoints", () => {
  it.effect("export on a fresh DB is empty but for the seeded category tree", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      ...
      assert.strictEqual(dump.categories.filter((c) => c.parentId === null).length, 6);
      assert.ok(dump.categories.every((c) => c.parentId === null || c.id > 0));
  ...
  it.effect("export captures every seeded table", () =>
    Effect.gen(function* () {
      ...
      assert.ok(dump.categories.some((c) => c.id === seeded.category.id && c.slug === "groceries"));
      ...
      assert.strictEqual(dumpedRule?.createdAt.getTime(), seeded.rule.createdAt.getTime());
