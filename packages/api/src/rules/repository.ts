...
export const RuleFromRow = Schema.transform(RuleRow, Rule, {
  ...
  decode: (row) => ({
    ...
    ...(row.matchAccountId !== null ? { matchAccountId: row.matchAccountId } : {}),
...
export class RuleRepo extends Effect.Service<RuleRepo>()("api/RuleRepo", {
  effect: Effect.gen(function* () {
    ...
    const countQuery = SqlSchema.single({
      ...
      Result: CountResult,
      execute: ({ issuerId }) => sql`SELECT COUNT(*) AS count FROM rules ${whereClause(issuerId)}`,
    });
    ...
    const updateQuery = SqlSchema.single({
      Request: Schema.Any as Schema.Schema<WriteRow & { id: typeof RuleId.Type }>,
      Result: RuleFromRow,
      execute: (row) =>
    ...
    const list = (filter: ListFilter) =>
      Effect.all({
        ...
        total: countQuery({ issuerId: filter.issuerId }).pipe(Effect.map((r) => r.count)),
      }).pipe(
    ...
    const getByIssuerPattern = (issuerId: typeof IssuerId.Type, pattern: string) =>
    ...
    const create = (payload: RuleCreate) =>
      nowIso.pipe(
        Effect.flatMap((now) => insertQuery({ ...toWriteFields(payload), createdAt: now })),
        orDieSql,
      );
