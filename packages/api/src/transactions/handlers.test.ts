...
/** The derived client every endpoint test in this file talks through. */
...
describe("transactions endpoints", () => {
  ...
  it.effect("transferSuggestions returns the counterpart leg over the wire", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("transferSuggestions 404s an unknown id", () =>
    Effect.gen(function* () {
      ...
      const error = yield* client.transactions
        ...
        .pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "transaction", id: 9999 }));
    }).pipe(Effect.provide(HttpLive)),
  );
  ...
  it.effect("excludedFromRecap boolean filter decodes from the query string", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      yield* client.transactions.create({
        payload: make({
  ...
  it.effect("getById 404s on a missing id", () =>
    Effect.gen(function* () {
      ...
      const error = yield* client.transactions
        ...
        .pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "transaction", id: asTx(999) }));
    }).pipe(Effect.provide(HttpLive)),
  );
...
describe("transactions bulk endpoints", () => {
  it.effect("bulkCreate returns 201 with the created rows and generated ids", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("bulkDelete returns the count actually deleted (partial existence)", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("bulkGet returns only the ids that exist (partial existence)", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(got.map((t) => t.rawIssuerString).sort(), ["A", "B"]);
    }).pipe(Effect.provide(HttpLive)),
  );
  ...
  it.effect("no route deletes an account's month", () =>
    Effect.gen(function* () {
      ...
      const res = yield* http.execute(
        HttpClientRequest.del("/api/transactions/by-account-month?accountId=1&importMonth=2026-03"),
      );
...
describe("an overlapping import adds rows rather than replacing them", () => {
  it.effect("keeps the earlier rows and the curation on them", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: june2.id } })).bundleId,
        parent.id,
      );
...
describe("import matching via bulkCreate", () => {
  it.effect("assigns the matching rule's issuer and counts the row it won", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("the more-specific rule (longer literal) wins when several match one row", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("an invalid regex pattern is skipped, not a 500 — the import still succeeds", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
...
describe("derived category through issuer", () => {
  ...
  it.effect("a non-manual row reads its category through the issuer's defaultCategoryId", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("recategorising the issuer reclassifies its whole history (no per-row writes)", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* client.issuers.create({
        payload: {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: a.id } })).categoryId,
        asCategory(7),
      );
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: b.id } })).categoryId,
        asCategory(9),
      );
    }).pipe(Effect.provide(HttpLive)),
  );

  it.effect("a manual category wins over the issuer default and survives recategorising", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* client.issuers.create({
        payload: {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: manual.id } })).categoryId,
        asCategory(3),
      );
  ...
  it.effect("clearing the issuer default unassigns its non-manual transactions", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* client.issuers.create({
        payload: {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: created.id } })).categoryId,
        asCategory(7),
      );
  ...
  it.effect("removing an override (manualCategory → false) reverts to the issuer default", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* client.issuers.create({
        payload: {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: tx.id } })).categoryId,
        asCategory(3),
      );
...
describe("derived recap exclusion through issuer", () => {
  ...
  it.effect("a row with no manual flag inherits its issuer's exclusion", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: created.id } })).excludedFromRecap,
        true,
      );
      const page = yield* client.transactions.list({
        urlParams: { limit: 50, offset: 0, direction: "desc" },
  ...
  it.effect("a transaction created under an excluded issuer needs no re-run", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* issuerExcluded(true);
      const created = yield* client.transactions.create({
  ...
  it.effect("excluding an issuer holds its whole history out at once", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        (yield* client.transactions.getById({ path: { id: a.id } })).excludedFromRecap,
        undefined,
      );
      ...
      for (const id of [a.id, b.id]) {
        assert.strictEqual(
          (yield* client.transactions.getById({ path: { id } })).excludedFromRecap,
          true,
        );
      }
    }).pipe(Effect.provide(HttpLive)),
  ...
  it.effect("a manually included row keeps counting under an excluded issuer", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* issuerExcluded(true);
      const created = yield* client.transactions.create({
  ...
  it.effect("the uncurated filter drops excluded rows, by either route", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        (yield* client.transactions.count({ urlParams: { uncurated: true } })).count,
        1,
      );
    }).pipe(Effect.provide(HttpLive)),
  );
...
describe("category leaf-assignable invariant at the transactions door", () => {
  ...
  it.effect("accepts a depth-3 leaf, rejects its mid-tier parent", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const category = (slug: string, parentId: typeof CategoryId.Type | null) =>
...
describe("category filter matches the derived category (ADR 0002)", () => {
  ...
  it.effect("the category filter accepts a set — a folder's leaves in one query", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
  ...
  it.effect("the count total is signed and net over the whole filtered set", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* client.issuers.create({
        payload: {
  ...
  it.effect("the total follows the account filter and covers the whole set, not the page", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const issuer = yield* client.issuers.create({
        payload: {
...
describe("createBundle (issue #68)", () => {
  ...
  it.effect("curates a parent through the generic update endpoint", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(curated.date, new Date("2026-02-14T00:00:00.000Z"));
...
describe("recap aggregation (issue #71)", () => {
  ...
  it.effect("returns spend by issuer and by category for a period", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(recap.byIssuer, [{ id: issuer.id, spent: 42.5, count: 2 }]);
      // Categorised through the issuer's default, not by a stored column.
      assert.deepStrictEqual(recap.byCategory, [{ id: asCategory(7), spent: 42.5, count: 2 }]);
  ...
  it.effect("narrows to a selection of accounts in one request", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(recap.byIssuer, [{ id: null, spent: 50, count: 2 }]);
  ...
  it.effect("honours every exclusion reason and summarises transfers", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(recap.byIssuer, [{ id: null, spent: 20, count: 2 }]);
...
describe("a bundle that is not a cost (issue #76)", () => {
  ...
  it.effect("flags a non-negative bundle, and clears it once corrected", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual((yield* client.transactions.recap({ urlParams: {} })).byIssuer, [
...
describe("bundle / transfer exclusivity (issue #75)", () => {
  ...
  it.effect("never suggests a bundled row, and offers none to one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(yield* client.transactions.transferCandidates(), []);
    }).pipe(Effect.provide(HttpLive)),
  );
});
