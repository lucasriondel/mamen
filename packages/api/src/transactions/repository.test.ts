...
const RepoAndSqlTest = TransactionRepo.Default.pipe(Layer.provideMerge(DatabaseTest));
...
/** A day in March 2026, the month `DATE` sits in — the transfer-window fixtures. */
const day = (n: number) => new Date(`2026-03-${String(n).padStart(2, "0")}T00:00:00.000Z`);
...
describe("TransactionFromRow storage codec", () => {
  ...
  it("round-trips a bare transaction (optional fields → null columns)", () => {
    ...
    assert.deepStrictEqual(decode(row), new Transaction({ ...bare, kind: "bank" }));
  });
});
...
describe("TransactionRepo", () => {
  ...
  it.effect("excludedFromRecap + manualExcluded round-trip through create", () =>
    Effect.gen(function* () {
      const repo = yield* TransactionRepo;
      const created = yield* repo.create(make({ excludedFromRecap: true, manualExcluded: true }));
  ...
  it.effect("a row defaults to not-excluded, and update flips it both ways", () =>
    Effect.gen(function* () {
      const repo = yield* TransactionRepo;
      const created = yield* repo.create(make());
  ...
  it.effect("remove deletes the row", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(error, new NotFound({ resource: "transaction", id: created.id }));
    }).pipe(Effect.provide(RepoTest)),
  );
  ...
  it.effect("getById / update / remove fail NotFound on a missing id", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(yield* repo.getById(missing).pipe(Effect.flip), expected);
      assert.deepStrictEqual(
        yield* repo.update(missing, { amount: 1 }).pipe(Effect.flip),
        expected,
      );
      assert.deepStrictEqual(yield* repo.remove(missing).pipe(Effect.flip), expected);
    }).pipe(Effect.provide(RepoTest)),
  );
  ...
  describe("composable list/count filters", () => {
    ...
    it.effect("accountId accepts a set (the recap's selection, issue #86)", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        yield* seed(repo);
      ...
      it.effect(
        ...
        () =>
          Effect.gen(function* () {
            ...
            yield* repo.create(make({ categoryId: asCategory(4), manualCategory: true }));
    ...
    describe("free-text search (issue #40)", () => {
      ...
      it.effect("matches the raw issuer string, case-insensitively", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(page.total, 1);
          assert.strictEqual(page.items[0]?.rawIssuerString, "CARREFOUR MARKET");
        }).pipe(Effect.provide(RepoAndSqlTest)),
      );
      ...
      it.effect("count honors the search filter", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          yield* seedSearch(repo);
          assert.strictEqual((yield* repo.count({ search: "spotify" })).count, 1);
        }).pipe(Effect.provide(RepoAndSqlTest)),
      );
    });
    ...
    describe("uncurated filter", () => {
      ...
      const seedCuration = (repo: TransactionRepo) =>
        Effect.gen(function* () {
          ...
          yield* repo.create(make({ rawIssuerString: "ISSUER ONLY", issuerId: asIssuer(9) }));
          yield* repo.create(make({ rawIssuerString: "NOTE ONLY", notes: "check this" }));
          yield* repo.create(make({ rawIssuerString: "DERIVED CAT", issuerId: asIssuer(8) }));
        });
      ...
      it.effect("treats a whitespace-only note as no note", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          yield* repo.create(make({ rawIssuerString: "BLANK NOTE", notes: "  " }));
          const page = yield* repo.list({ ...listAll, uncurated: true });
          assert.strictEqual(page.total, 1);
        }).pipe(Effect.provide(RepoAndSqlTest)),
      );
      ...
      it.effect("false returns the complement, absent returns both", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          yield* seedCuration(repo);
          assert.strictEqual((yield* repo.list({ ...listAll, uncurated: false })).total, 3);
      ...
      describe("excluded rows are exempt", () => {
        ...
        it.effect("…and it is not in the complement either", () =>
          Effect.gen(function* () {
            ...
            assert.strictEqual((yield* repo.list({ ...listAll, uncurated: false })).total, 0);
        ...
        it.effect("a row excluded through its issuer is exempt too", () =>
          Effect.gen(function* () {
            ...
            yield* repo.create(make({ rawIssuerString: "INHERITED", issuerId: asIssuer(10) }));

            assert.strictEqual((yield* repo.list({ ...listAll, uncurated: true })).total, 0);
            assert.strictEqual((yield* repo.list({ ...listAll, uncurated: false })).total, 0);
          }).pipe(Effect.provide(RepoAndSqlTest)),
        );
        ...
        it.effect("a row forced back into the recap is curated again", () =>
          Effect.gen(function* () {
            ...
            assert.strictEqual((yield* repo.list({ ...listAll, uncurated: false })).total, 1);
          }).pipe(Effect.provide(RepoAndSqlTest)),
        );
    ...
    describe("excludedFromRecap filter", () => {
      const seedExclusion = (repo: TransactionRepo) =>
        Effect.all([
          ...
          repo.create(make({ rawIssuerString: "OTHER ACCOUNT", accountId: asAccount(2) })),
        ]);
      ...
      it.effect("false returns the complement, absent returns both", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          yield* seedExclusion(repo);
          assert.strictEqual((yield* repo.list({ ...listAll, excludedFromRecap: false })).total, 2);
      ...
      it.effect("count honors the excludedFromRecap filter", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          yield* seedExclusion(repo);
          assert.strictEqual((yield* repo.count({ excludedFromRecap: true })).count, 1);
        }).pipe(Effect.provide(RepoTest)),
      );
      ...
      it.effect("true also returns duplicate-excluded rows", () =>
        Effect.gen(function* () {
          ...
          assert.deepStrictEqual(page.items.map((t) => t.rawIssuerString).sort(), [
            "DUPLICATE ROW",
            "EXCLUDED ROW",
          ]);
        }).pipe(Effect.provide(RepoTest)),
    ...
    describe("isTransferLeg filter", () => {
      const seedLegs = (repo: TransactionRepo) =>
        Effect.gen(function* () {
          const debit = yield* repo.create(make({ rawIssuerString: "LEG OUT", amount: -30 }));
      ...
      it.effect("count honors the isTransferLeg filter", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          yield* seedLegs(repo);
          assert.strictEqual((yield* repo.count({ isTransferLeg: true })).count, 2);
        }).pipe(Effect.provide(RepoTest)),
      );
    });
    ...
    it.effect("count honors every filter list does", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual((yield* repo.count({ accountId: asAccount(1) })).count, 2);
        assert.strictEqual((yield* repo.count({ importMonth: "2026-03" })).count, 1);
        assert.strictEqual(
          (yield* repo.count({
            accountId: asAccount(1),
            categoryId: asCategory(7),
  ...
  describe("the month filter buckets by the row's own date (issue #87)", () => {
    it.effect("returns the rows dated in the month, not those stamped with it", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        ...
        assert.strictEqual((yield* repo.count({ importMonth: "2026-04" })).count, 1);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("bounds the month at its first and last instant", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const first = yield* repo.create(make({ date: new Date("2026-03-01T00:00:00.000Z") }));
        const last = yield* repo.create(make({ date: new Date("2026-03-31T23:59:59.999Z") }));
        // The instants either side, which must fall out.
        yield* repo.create(make({ date: new Date("2026-02-28T23:59:59.999Z") }));
        yield* repo.create(make({ date: new Date("2026-04-01T00:00:00.000Z") }));
    ...
    it.effect("follows a bundle parent's overridden date into another month", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        ...
        assert.strictEqual((yield* repo.getById(parent.id)).importMonth, "2026-03");
        ...
        assert.strictEqual((yield* repo.list({ ...listAll, importMonth: "2026-03" })).total, 0);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("a key that is not a YYYY-MM month matches nothing", () =>
      Effect.gen(function* () {
        ...
        for (const key of ["2026", "2026-13", "2026-00", "not-a-month"]) {
          assert.strictEqual((yield* repo.list({ ...listAll, importMonth: key })).total, 0, key);
        }
      }).pipe(Effect.provide(RepoTest)),
    );
  });
  ...
  describe("recap aggregation", () => {
    ...
    it.effect("sums spend by issuer and by category, as positive magnitudes", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        yield* repo.create(
          spent({
    ...
    it.effect("nets transfer legs out of the breakdowns and summarises them", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(spent({ amount: -30, issuerId: asIssuer(1) }));
        const credit = yield* repo.create(
          spent({
            amount: 30,
            accountId: asAccount(2),
    ...
    it.effect("leaves a bundle that sums non-negative out of the spend", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(spent({ amount: -200, issuerId: asIssuer(1) }));
        const b = yield* repo.create(spent({ amount: 230, issuerId: asIssuer(2) }));
    ...
    it.effect("answers the same summary for a fixture of every partition", () =>
      Effect.gen(function* () {
        ...
        yield* repo.create(spent({ amount: -20, ...shop, manualCategory: true }));
        yield* repo.create(spent({ amount: -5, ...shop, manualCategory: true }));
        ...
        const a = yield* repo.create(spent({ amount: -200, ...shop, manualCategory: true }));
        const b = yield* repo.create(spent({ amount: 150 }));
        yield* repo.createBundle([a.id, b.id], "Weekend away");
        // A transfer pair — out of the breakdowns, into the transfer line.
        const debit = yield* repo.create(spent({ amount: -30, issuerId: asIssuer(2) }));
        const credit = yield* repo.create(
    ...
    describe("excluded summary (issue #87)", () => {
      ...
      it.effect("claims an excluded transfer leg, which no other line reports", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          const debit = yield* repo.create(spent({ amount: -30, issuerId: asIssuer(2) }));
          const credit = yield* repo.create(
      ...
      it.effect("follows the period and account filters like every other line", () =>
        Effect.gen(function* () {
          const repo = yield* TransactionRepo;
          const excludedIn = {
            excludedFromRecap: true,
    ...
    it.effect("bounds the period on the transaction date, inclusively", () =>
      Effect.gen(function* () {
        ...
        yield* repo.create(spent({ amount: -1, date: new Date("2026-07-01T00:00:00.000Z") }));
        yield* repo.create(spent({ amount: -2, date: new Date("2026-07-31T23:59:59.999Z") }));
        ...
        assert.strictEqual(byId((yield* repo.recap({})).byIssuer).none?.count, 4);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("narrows to one account, or to a selection of them", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          byId((yield* repo.recap({ accountId: asAccount(2) })).byIssuer).none?.spent,
          20,
        );
        // The picker is multi-select, and the whole selection is ONE query.
        assert.strictEqual(
          byId((yield* repo.recap({ accountId: [asAccount(1), asAccount(3)] })).byIssuer).none
            ?.spent,
          50,
        );
      }).pipe(Effect.provide(RepoTest)),
    ...
    it.effect("sums in integer cents, so a period of small amounts is exact", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
    ...
    it.effect("aggregates the whole set, past any page size", () =>
      Effect.gen(function* () {
        ...
        yield* repo.bulkCreate(
          Array.from({ length: 1200 }, () => spent({ amount: -1, issuerId: asIssuer(1) })),
        );
    ...
    it.effect("returns empty breakdowns and no transfers for an empty set", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const recap = yield* repo.recap({});
    ...
    it.effect("lists the distinct months of the transaction dates, newest first", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        yield* repo.create(
          spent({
  ...
  describe("link/unlink transfer", () => {
    it.effect("links a valid pair and stamps min(id) on every leg as the group id", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -30 }));
        const b = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
        ...
        assert.strictEqual((yield* repo.getById(a.id)).transferGroupId, groupId);
        assert.strictEqual((yield* repo.getById(b.id)).transferGroupId, groupId);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("links an N-leg group (-100 repaid by +60 and +40)", () =>
      Effect.gen(function* () {
        ...
        const c1 = yield* repo.create(make({ amount: 60, accountId: asAccount(2) }));
        const c2 = yield* repo.create(make({ amount: 40, accountId: asAccount(3) }));
        ...
        for (const id of [debit.id, c1.id, c2.id]) {
          assert.strictEqual((yield* repo.getById(id)).transferGroupId, groupId);
        }
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("balances in integer cents, not as floats (-10.10 + 10.10)", () =>
      Effect.gen(function* () {
        ...
        const b = yield* repo.create(make({ amount: 10.1, accountId: asAccount(2) }));
    ...
    it.effect("does not enforce ≥2 distinct accounts", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -30, accountId: asAccount(1) }));
        const b = yield* repo.create(make({ amount: 30, accountId: asAccount(1) }));
        const result = yield* repo.linkTransfer([a.id, b.id]);
        assert.strictEqual(result.count, 2);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("rejects fewer than 2 legs (a set, so dupes collapse)", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "too-few-legs" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("rejects a non-zero sum", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -30 }));
        const b = yield* repo.create(make({ amount: 25, accountId: asAccount(2) }));
        const error = yield* repo.linkTransfer([a.id, b.id]).pipe(Effect.flip);
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "unbalanced" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("rejects an unknown (non-existent) id", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -30 }));
        const error = yield* repo.linkTransfer([a.id, asTx(9999)]).pipe(Effect.flip);
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "unknown-id" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("rejects a leg already carrying a transferGroupId", () =>
      Effect.gen(function* () {
        ...
        const c = yield* repo.create(make({ amount: -30, accountId: asAccount(3) }));
        const error = yield* repo.linkTransfer([a.id, c.id]).pipe(Effect.flip);
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "already-grouped" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("rejects a leg that is a refund (isRefund set)", () =>
      Effect.gen(function* () {
        ...
        const b = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
        const error = yield* repo.linkTransfer([a.id, b.id]).pipe(Effect.flip);
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "is-refund" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("a rejected link writes nothing (no partial stamping)", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual((yield* repo.getById(a.id)).transferGroupId, undefined);
        assert.strictEqual((yield* repo.getById(b.id)).transferGroupId, undefined);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("unlink clears transferGroupId on every leg of the group", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(result.count, 3);
        for (const id of [debit.id, c1.id, c2.id]) {
          assert.strictEqual((yield* repo.getById(id)).transferGroupId, undefined);
        }
      }).pipe(Effect.provide(RepoTest)),
    );
  ...
  describe("createBundle (issue #68)", () => {
    it.effect("sums its members' amounts and takes the earliest date", () =>
      Effect.gen(function* () {
        ...
        const parent = yield* repo.createBundle([payback.id, spend.id], "Weekend away");
    ...
    it.effect("stores the label in rawIssuerString, leaving issuer and category unset", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -20, issuerId: asIssuer(3) }));
        const b = yield* repo.create(
          make({
            amount: -5,
            categoryId: asCategory(7),
    ...
    it.effect("refuses fewer than two members (a set, so dupes collapse)", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -20 }));
        const error = yield* repo.createBundle([a.id, a.id], "Lonely").pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "too-few-members" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("refuses an unknown (non-existent) id", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -20 }));
        const error = yield* repo.createBundle([a.id, asTx(9999)], "Ghost").pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "unknown-id" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("refuses a member that already belongs to another bundle", () =>
      Effect.gen(function* () {
        ...
        const error = yield* repo.createBundle([a.id, c.id], "Second").pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "already-bundled" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("refuses a member that is itself a bundle parent", () =>
      Effect.gen(function* () {
        ...
        const error = yield* repo.createBundle([inner.id, c.id], "Outer").pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "nested-bundle" }));
    ...
    it.effect("hides members from the list and its signed total", () =>
      Effect.gen(function* () {
        ...
        const unrelated = yield* repo.create(make({ amount: -10, rawIssuerString: "COFFEE" }));
        const parent = yield* repo.createBundle([a.id, b.id], "Weekend away");
  ...
  describe("editing a bundle parent (issue #72)", () => {
    ...
    it.effect("stores a date override without touching the derived amount", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(
        ...
        assert.deepStrictEqual(dated.date, new Date("2026-02-14T00:00:00.000Z"));
  ...
  describe("bundle members ride with the list (issue #73)", () => {
    ...
    it.effect("projects a member's derived fields like any other row", () =>
      Effect.gen(function* () {
        ...
        const a = yield* repo.create(make({ amount: -20, issuerId: asIssuer(7) }));
        const b = yield* repo.create(make({ amount: -5 }));
        yield* repo.createBundle([a.id, b.id], "Weekend away");
  ...
  describe("changing a bundle's membership (issue #74)", () => {
    it.effect("adding a member recomputes the parent's amount and date", () =>
      Effect.gen(function* () {
        ...
        const parent = yield* repo.createBundle([spend.id, payback.id], "Weekend away");
        assert.strictEqual(parent.amount, -50);
        ...
        assert.deepStrictEqual(updated.date, new Date("2026-03-07T00:00:00.000Z"));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("a member joining earlier pulls the parent's date back", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(updated.date, new Date("2026-03-01T00:00:00.000Z"));
    ...
    it.effect("a recompute keeps the parent's overridden date", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(updated.date, new Date("2026-02-14T00:00:00.000Z"));
    ...
    it.effect("refuses to add a row that already belongs to a bundle", () =>
      Effect.gen(function* () {
        ...
        const error = yield* repo.addBundleMember(second.id, a.id).pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "already-bundled" }));
    ...
    it.effect("refuses to add to a row that is not a bundle parent", () =>
      Effect.gen(function* () {
        ...
        const error = yield* repo.addBundleMember(bank.id, other.id).pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "not-a-bundle" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("removing a member returns it to the list and recomputes the parent", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(
          make({ amount: -200, date: new Date("2026-03-07T00:00:00.000Z") }),
        ...
        const parent = yield* repo.createBundle([a.id, b.id, c.id], "Weekend away");
  ...
  describe("bundles survive every delete path (issue #74)", () => {
    ...
    it.effect("deleting the parent releases its members, never deletes them", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -200 }));
        const b = yield* repo.create(make({ amount: 150 }));
    ...
    it.effect("a bulk delete of a parent AND a member leaves nothing dangling", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -200 }));
        const b = yield* repo.create(make({ amount: 150 }));
        ...
        assert.strictEqual((yield* repo.bulkDelete([parent.id, b.id])).count, 2);
        assert.strictEqual((yield* repo.getById(a.id)).bundleId, undefined);
        assert.strictEqual((yield* repo.list(listAll)).total, 1);
      }).pipe(Effect.provide(RepoTest)),
    );

    it.effect("deleteByImportBatch releases the members of a deleted parent", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -200, importBatchId: "batch-1" }));
        const b = yield* repo.create(make({ amount: 150, importBatchId: "batch-1" }));
        ...
        // behind either way.
        assert.strictEqual((yield* repo.deleteByImportBatch("batch-1")).count, 2);
  ...
  describe("a bundle that is not a cost is flagged (issue #76)", () => {
    it.effect("flags a fresh bundle whose members sum to a credit", () =>
      Effect.gen(function* () {
        ...
        const parent = yield* repo.createBundle([spend.id, payback.id], "Overcollected");
        ...
        assert.deepStrictEqual(flagsOf(yield* repo.getById(parent.id)), ["non-negative-bundle"]);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("flags a bundle whose members cancel out exactly", () =>
      Effect.gen(function* () {
        ...
        const parent = yield* repo.createBundle([spend.id, payback.id], "Paid back in full");
    ...
    it.effect("flags a parent left non-negative by a deleted member", () =>
      Effect.gen(function* () {
        ...
        const parent = yield* repo.createBundle([spend.id, other.id, payback.id], "Weekend away");
  ...
  describe("a scoped delete dissolves the bundles it touches (issue #77)", () => {
    it.effect("counts a bundle wholly inside the target account+month", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(yield* repo.bundleImpact(asAccount(1), "2026-03"), { count: 1 });
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("counts a bundle only partly inside the target month", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(yield* repo.bundleImpact(asAccount(1), "2026-04"), { count: 1 });
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("counts a bundle only partly inside the target account", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -200 }));
        const b = yield* repo.create(make({ amount: 150, accountId: asAccount(2) }));
        const parent = yield* repo.createBundle([a.id, b.id], "Weekend");
        assert.strictEqual(parent.accountId, asAccount(1));

        assert.deepStrictEqual(yield* repo.bundleImpact(asAccount(2), "2026-03"), { count: 1 });
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("counts each touched bundle once", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(yield* repo.bundleImpact(asAccount(1), "2026-03"), { count: 2 });
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("counts nothing when the month holds no bundled row", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(yield* repo.bundleImpact(asAccount(1), "2026-04"), { count: 0 });
        assert.deepStrictEqual(yield* repo.bundleImpact(asAccount(2), "2026-03"), { count: 0 });
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("deleteByImportBatch dissolves the bundles the batch touches", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -200, importBatchId: "batch-1" }));
        const b = yield* repo.create(make({ amount: -30, importBatchId: "batch-1" }));
        const c = yield* repo.create(make({ amount: 150, importBatchId: "batch-2" }));
        const parent = yield* repo.createBundle([a.id, b.id, c.id], "Weekend");

        assert.strictEqual((yield* repo.deleteByImportBatch("batch-2")).count, 1);
        assert.deepStrictEqual(
          yield* repo.getById(parent.id).pipe(Effect.flip),
          new NotFound({ resource: "transaction", id: parent.id }),
        );
  ...
  describe("auto-dissolve transfer groups on delete", () => {
    it.effect("single delete of one leg of a two-leg group clears the survivor", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const a = yield* repo.create(make({ amount: -30 }));
        const b = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
    ...
    it.effect("single delete of one leg of an N-leg group (N > 2) keeps the rest grouped", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(make({ amount: -100 }));
        const c1 = yield* repo.create(make({ amount: 60, accountId: asAccount(2) }));
        ...
        assert.strictEqual((yield* repo.getById(debit.id)).transferGroupId, groupId);
        assert.strictEqual((yield* repo.getById(c1.id)).transferGroupId, groupId);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("deleteByImportBatch auto-dissolves a group whose survivor is elsewhere", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        // Legs in different accounts/batches so dropping one batch removes
        // exactly one leg, leaving the cross-account survivor.
        const a = yield* repo.create(make({ amount: -30, importBatchId: "batch-1" }));
        const b = yield* repo.create(
          make({
            amount: 30,
            accountId: asAccount(2),
    ...
    it.effect("a bulk delete of an entire N-leg group leaves nothing dangling", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(make({ amount: -100 }));
        const c1 = yield* repo.create(make({ amount: 60, accountId: asAccount(2) }));
        ...
        assert.strictEqual(result.count, 2);
        assert.strictEqual((yield* repo.getById(c2.id)).transferGroupId, undefined);
      }).pipe(Effect.provide(RepoTest)),
    );
  });
  ...
  describe("suggestTransfers (internal-transfer counterparts)", () => {
    ...
    it.effect("never suggests a dismissed counterpart, from either side", () =>
      Effect.gen(function* () {
        ...
        const kept = yield* repo.create(make({ amount: 30, accountId: asAccount(3) }));
    ...
    it.effect("suggests an opposite-sign, equal-magnitude, cross-account leg", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30 }));
        const match = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
    ...
    it.effect("excludes a different magnitude (cent-exact match required)", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30 }));
    ...
    it.effect("matches magnitude in integer cents (-10.10 ⇄ +10.10)", () =>
      Effect.gen(function* () {
        ...
        const match = yield* repo.create(make({ amount: 10.1, accountId: asAccount(2) }));
    ...
    it.effect("excludes a same-account leg (a transfer moves between accounts)", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30 }));
    ...
    it.effect("includes a leg exactly TRANSFER_DATE_WINDOW_DAYS away, excludes one past it", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        ...
        yield* repo.create(make({ amount: 30, accountId: asAccount(3), date: day(12) }));
    ...
    it.effect("excludes a candidate already in a transfer group", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30 }));
        const grouped = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
        const other = yield* repo.create(make({ amount: -30, accountId: asAccount(3) }));
    ...
    it.effect("excludes a refund candidate", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30 }));
        yield* repo.create(make({ amount: 30, accountId: asAccount(2), isRefund: true }));
    ...
    it.effect("returns [] for a refund target (ineligible)", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30, isRefund: true }));
        yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
    ...
    it.effect("404s an unknown id", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const error = yield* repo.suggestTransfers(asTx(9999)).pipe(Effect.flip);
    ...
    it.effect("orders candidates nearest-date first", () =>
      Effect.gen(function* () {
        ...
        const far = yield* repo.create(make({ amount: 30, accountId: asAccount(2), date: day(9) }));
  ...
  describe("transferCandidates (detected transfers, grouped by debit leg)", () => {
    it.effect("groups each pair under its debit leg, oriented by sign", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(make({ amount: -30, accountId: asAccount(1) }));
        const credit = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
    ...
    it.effect("a debit matching several credits is one entry with several counterparts", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(
    ...
    it.effect("reports the whole-day gap between the legs", () =>
      Effect.gen(function* () {
        ...
        yield* repo.create(make({ amount: 30, accountId: asAccount(2), date: day(4) }));
    ...
    it.effect("excludes same-account, wrong-magnitude, and out-of-window rows", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(
        ...
        yield* repo.create(make({ amount: 30, accountId: asAccount(4), date: day(12) }));
    ...
    it.effect("each leg derives exclusion through its own issuer", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(out[0]?.counterparts[0].transaction.excludedFromRecap, undefined);
      }).pipe(Effect.provide(RepoAndSqlTest)),
    );
    ...
    it.effect("excludes already-grouped and refund legs", () =>
      Effect.gen(function* () {
        ...
        const g2 = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
  ...
  describe("dismissTransferPairs", () => {
    ...
    it.effect("dismissing one pair leaves the leg's other pairs present", () =>
      Effect.gen(function* () {
        ...
        yield* repo.dismissTransferPairs([{ debitId: debit.id, creditId: dismissed.id }]);
    ...
    it.effect("dismissing one credit's pairs leaves another credit's alone", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        ...
        const out = yield* repo.transferCandidates();
        assert.deepStrictEqual(
          out.map((c) => [c.leg.id, c.counterparts.map((x) => x.transaction.id)]),
          [
    ...
    it.effect("is idempotent — re-dismissing a stored pair writes nothing", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const debit = yield* repo.create(make({ amount: -30 }));
        const credit = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
    ...
    it.effect("deleting a leg deletes the dismissals naming it", () =>
      Effect.gen(function* () {
        ...
        const debit = yield* repo.create(make({ amount: -30 }));
        const credit = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
        const other = yield* repo.create(make({ amount: -50, accountId: asAccount(1) }));
        const otherCredit = yield* repo.create(make({ amount: 50, accountId: asAccount(2) }));
        yield* repo.dismissTransferPairs([
          { debitId: debit.id, creditId: credit.id },
  ...
  describe("bundle / transfer exclusivity (issue #75)", () => {
    it.effect("refuses to bundle a transfer leg", () =>
      Effect.gen(function* () {
        ...
        const error = yield* repo.createBundle([a.id, c.id], "Weekend").pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "is-transfer-leg" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("refuses to add a transfer leg to an existing bundle", () =>
      Effect.gen(function* () {
        ...
        const other = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
        yield* repo.linkTransfer([leg.id, other.id]);

        const error = yield* repo.addBundleMember(parent.id, leg.id).pipe(Effect.flip);
        assert.deepStrictEqual(error, new BundleInvalid({ reason: "is-transfer-leg" }));
    ...
    it.effect("refuses to transfer-link a bundle member", () =>
      Effect.gen(function* () {
        ...
        const b = yield* repo.create(make({ amount: -5 }));
        yield* repo.createBundle([a.id, b.id], "Weekend");
        const counterpart = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));

        const error = yield* repo.linkTransfer([a.id, counterpart.id]).pipe(Effect.flip);
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "is-bundled" }));
        // Nothing was stamped: the refusal is atomic like every other one.
        assert.strictEqual((yield* repo.getById(counterpart.id)).transferGroupId, undefined);
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("refuses to transfer-link a bundle parent", () =>
      Effect.gen(function* () {
        ...
        const error = yield* repo.linkTransfer([parent.id, counterpart.id]).pipe(Effect.flip);
        assert.deepStrictEqual(error, new TransferInvalid({ reason: "is-bundled" }));
      }).pipe(Effect.provide(RepoTest)),
    );
    ...
    it.effect("never suggests a bundled counterpart", () =>
      Effect.gen(function* () {
        const repo = yield* TransactionRepo;
        const target = yield* repo.create(make({ amount: -30 }));
        const bundled = yield* repo.create(make({ amount: 30, accountId: asAccount(2) }));
        const sibling = yield* repo.create(make({ amount: -5, accountId: asAccount(2) }));
