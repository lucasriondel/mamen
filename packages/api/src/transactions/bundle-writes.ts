...
export const bundleMemberRefusal = (
  ...
  if (candidates.some((c) => c.bundleId !== undefined)) return "already-bundled";
  if (candidates.some((c) => c.kind === "bundle")) return "nested-bundle";
  if (candidates.some((c) => c.transferGroupId !== undefined)) return "is-transfer-leg";
  return undefined;
};
...
export type BundleWriteDeps = {
  ...
  readonly storedById: (id: typeof TransactionId.Type) => Effect.Effect<Option.Option<Transaction>>;
  ...
  readonly readById: (id: typeof TransactionId.Type) => Effect.Effect<Transaction>;
...
export const bundleWrites = ({
   ...
   * `IN ()`).
   */
  const releaseBundleMembers = (parentIds: ReadonlyArray<number>): Effect.Effect<number> =>
    parentIds.length === 0
      ? Effect.succeed(0)
      : sql<{
          id: number;
   ...
   */
  const dissolveBundle = (parentId: typeof TransactionId.Type): Effect.Effect<{ count: number }> =>
    Effect.gen(function* () {
      const found = yield* storedById(parentId);
      if (Option.isNone(found) || found.value.kind !== "bundle") return { count: 0 };

      const count = yield* releaseBundleMembers([parentId]);
      yield* sql`DELETE FROM transactions WHERE id = ${parentId}`.pipe(orDieSql);
      return { count };
    });
   ...
   */
  const bundlesTouching = (scope: Fragment): Effect.Effect<ReadonlyArray<number>> =>
    sql<{
      parentId: number;
  ...
  const dissolveBundlesTouching = (scope: Fragment): Effect.Effect<void> =>
    bundlesTouching(scope).pipe(
      Effect.flatMap((parentIds) =>
        Effect.forEach(parentIds, (id) => dissolveBundle(TransactionId.make(id)), {
          discard: true,
        }),
      ),
    );
   ...
   */
  const recomputeBundleParent = (parentId: typeof TransactionId.Type): Effect.Effect<boolean> =>
    Effect.gen(function* () {
      const found = yield* storedById(parentId);
  ...
  const requireEligible = (
    ...
    return reason === undefined ? Effect.void : Effect.fail(new BundleInvalid({ reason }));
  };
  ...
  const createBundle = (
    ...
    Effect.gen(function* () {
      ...
      if (ids.length < 2)
        return yield* Effect.fail(new BundleInvalid({ reason: "too-few-members" }));
  ...
  const addBundleMember = (
    ...
    Effect.gen(function* () {
      ...
      if (parent.value.kind !== "bundle")
        return yield* Effect.fail(new BundleInvalid({ reason: "not-a-bundle" }));
      ...
      if (!survived) return yield* Effect.fail(new BundleInvalid({ reason: "too-few-members" }));
  ...
  const removeBundleMember = (
    ...
    Effect.gen(function* () {
      ...
      if (parentId === undefined)
        return yield* Effect.fail(new BundleInvalid({ reason: "not-a-member" }));
