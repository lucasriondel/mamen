 ...
 */
export const TransactionsLive = HttpApiBuilder.group(Api, "transactions", (handlers) =>
  Effect.gen(function* () {
    ...
    return handlers
      ...
      .handle("removeManualIssuer", (_) => matcher.removeManualIssuer(_.path.id))
      .handle("bulkDelete", (_) => repo.bulkDelete(_.payload.ids))
      .handle("bulkGet", (_) => repo.bulkGet(_.payload.ids))
      .handle("deleteByImportBatch", (_) => repo.deleteByImportBatch(_.path.batchId))
      ...
      .handle("unlinkTransfer", (_) => repo.unlinkTransfer(_.payload.transferGroupId))
      .handle("createBundle", (_) => repo.createBundle(_.payload.ids, _.payload.label))
      .handle("addBundleMember", (_) =>
        repo.addBundleMember(_.payload.bundleId, _.payload.transactionId),
      )
      .handle("removeBundleMember", (_) => repo.removeBundleMember(_.payload.transactionId))
      .handle("dissolveBundle", (_) => repo.dissolveBundle(_.payload.bundleId));
  }),
