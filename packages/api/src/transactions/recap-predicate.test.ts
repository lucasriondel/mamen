...
import { AccountId, IssuerId, type TransactionCreate } from "@mamen/shared/contract";
...
describe("countsTowardRecap, run alone (issue #80)", () => {
  ...
  it.effect("excludes transfer legs, excluded and duplicate-excluded rows", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      const repo = yield* TransactionRepo;
      ...
      yield* repo.create(spent({ amount: -50, excludedFromRecap: true, manualExcluded: true }));
