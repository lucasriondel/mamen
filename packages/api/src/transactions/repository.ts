...
export const TransactionFromRow = Schema.transform(TransactionRow, Transaction, {
  ...
  decode: (row) => ({
    ...
    ...(row.linkedRefundId !== null ? { linkedRefundId: row.linkedRefundId } : {}),
    ...(row.transferGroupId !== null ? { transferGroupId: row.transferGroupId } : {}),
    ...
    ...(row.duplicateNote !== null ? { duplicateNote: row.duplicateNote } : {}),
  ...
  encode: (t) => ({
    ...
    anomalyFlags:
      t.anomalyFlags !== undefined ? Schema.encodeSync(AnomalyFlagsJson)(t.anomalyFlags) : null,
 ...
 */
const legFromRow = (row: typeof TransferCandidateRow.Type, prefix: "f" | "t"): Transaction => {
  const pick = (col: string) => row[`${prefix}_${col}` as keyof typeof row] as never;
...
const groupCandidates = (
  ...
  const byLeg = new Map<number, { leg: Transaction; counterparts: TransferCounterpart[] }>();
 ...
 */
export class TransactionRepo extends Effect.Service<TransactionRepo>()("api/TransactionRepo", {
  effect: Effect.gen(function* () {
    ...
    const buildConditions = (f: Filters): Array<Fragment> => {
      ...
      if (f.accountId !== undefined) {
        const ids = Array.isArray(f.accountId) ? f.accountId : [f.accountId as number];
        conditions.push(ids.length === 0 ? sql`1 = 0` : sql`t.accountId IN ${sql.in(ids)}`);
      }
      ...
      if (f.categoryId !== undefined) {
        if (f.categoryId === UNASSIGNED_FILTER) {
          ...
          conditions.push(
            ids.length === 0 ? sql`1 = 0` : sql`${derivedCategory} IN ${sql.in(ids)}`,
          );
        }
      }
      ...
      if (f.isTransferLeg !== undefined)
        conditions.push(f.isTransferLeg ? isTransferLeg : sql`NOT ${isTransferLeg}`);
      ...
      // either.
      conditions.push(
        f.bundleId !== undefined ? sql`t.bundleId = ${f.bundleId}` : isNotBundleMember,
      );
      ...
      if (f.kind !== undefined) conditions.push(sql`COALESCE(t.kind, 'bank') = ${f.kind}`);
      ...
      if (f.importBatchId !== undefined) conditions.push(sql`t.importBatchId = ${f.importBatchId}`);
      if (f.startDate !== undefined) conditions.push(sql`t.date >= ${f.startDate.toISOString()}`);
      if (f.endDate !== undefined) conditions.push(sql`t.date <= ${f.endDate.toISOString()}`);
      if (f.isRefund !== undefined) conditions.push(sql`t.isRefund = ${f.isRefund ? 1 : 0}`);
      if (f.isDuplicateExcluded !== undefined)
        conditions.push(sql`t.isDuplicateExcluded = ${f.isDuplicateExcluded ? 1 : 0}`);
      ...
      if (f.excludedFromRecap !== undefined)
        conditions.push(f.excludedFromRecap ? isRecapExcluded : sql`NOT ${isRecapExcluded}`);
      ...
      if (search) {
        ...
        const contains = (col: Fragment) => sql`${col} LIKE ${like} ESCAPE '\\'`;
        ...
        const amountLike = `%${search.replace(",", ".").replace(/[\\%_]/g, "\\$&")}%`;
        conditions.push(
    ...
    const orderClause = (orderBy: "date" | undefined, direction: "asc" | "desc") =>
      orderBy === "date"
        ? direction === "asc"
    ...
    const byIdQuery = SqlSchema.findOne({
      ...
      Result: TransactionFromRow,
      execute: (id) => sql`SELECT ${readColumns} ${readFrom} WHERE t.id = ${id}`,
    });
    ...
    const insertQuery = SqlSchema.single({
      ...
      Result: TransactionFromRow,
      execute: (row) => sql`INSERT INTO transactions ${sql.insert(row)} RETURNING *`,
    });

    const updateQuery = SqlSchema.single({
      Request: Schema.Any as Schema.Schema<WriteRow & { id: typeof TransactionId.Type }>,
      Result: TransactionFromRow,
      execute: (row) =>
    ...
    const upsertQuery = SqlSchema.findAll({
      ...
      execute: (row) => sql`INSERT OR REPLACE INTO transactions ${sql.insert(row)} RETURNING id`,
    });
    ...
    const bundleMembersOfQuery = SqlSchema.findAll({
      Request: TransactionId,
      Result: TransactionFromRow,
      execute: (parentId) => sql`SELECT * FROM transactions WHERE bundleId = ${parentId}`,
    });
    ...
    const bulkGetQuery = SqlSchema.findAll({
      Request: Schema.Any as Schema.Schema<ReadonlyArray<number>>,
      Result: TransactionFromRow,
      execute: (ids) => sql`SELECT ${readColumns} ${readFrom} WHERE ${sql.in("t.id", ids)}`,
    });
    ...
    const hasChildrenQuery = SqlSchema.findOne({
      ...
      execute: (id) => sql`SELECT 1 AS one FROM categories WHERE parentId = ${id} LIMIT 1`,
    });
      ...
      categoryId == null
        ...
        : hasChildrenQuery(categoryId).pipe(
            ...
            Effect.flatMap((found) =>
              Option.isSome(found) ? Effect.fail(new CategoryNotLeaf({ categoryId })) : Effect.void,
            ),
          );
    ...
    const assertAllLeaves = (
      ...
      const ids = [
        ...new Set(records.flatMap((r) => (r.categoryId != null ? [r.categoryId] : []))),
      ];
      return ids.length === 0
        ? Effect.void
    ...
    const {
      ...
      storedMembersOf: (parentId) => bundleMembersOfQuery(parentId).pipe(orDieSql),
    ...
    const accountMonthScope = (accountId: typeof AccountId.Type, importMonth: string): Fragment =>
     ...
     */
    const dissolveUndersizedGroups = (groupIds: ReadonlyArray<number>): Effect.Effect<void> => {
     ...
     * `IN ()`).
     */
    const forgetDismissals = (ids: ReadonlyArray<number>): Effect.Effect<void> => {
     ...
     */
    const cleanupAfterDelete = (rows: ReadonlyArray<DeletedRow>): Effect.Effect<void> =>
      Effect.gen(function* () {
        const deletedParents = rows.flatMap((r) => (r.kind === "bundle" ? [r.id] : []));
        yield* releaseBundleMembers(deletedParents);

        const gone = new Set(deletedParents);
        const orphanedParents = [
          ...new Set(rows.flatMap((r) => (r.bundleId !== null ? [r.bundleId] : []))),
        ...
        yield* dissolveUndersizedGroups(
          rows.flatMap((r) => (r.transferGroupId !== null ? [r.transferGroupId] : [])),
        );
    ...
    const deleteAndDissolve = <E, R>(statement: Effect.Effect<ReadonlyArray<DeletedRow>, E, R>) =>
      statement.pipe(
        orDieSql,
        Effect.flatMap((rows) => cleanupAfterDelete(rows).pipe(Effect.as({ count: rows.length }))),
      );
    ...
    const list = (filter: ListFilter) =>
      Effect.all({
        ...
        Effect.bind("bundleMembers", ({ items }) => {
          const parentIds = items.flatMap((t) => (t.kind === "bundle" ? [t.id] : []));
     ...
     */
    const transferCandidates = (): Effect.Effect<ReadonlyArray<TransferCandidate>> =>
    ...
    const dismissTransferPairs = (
      ...
      pairs.length === 0
        ...
        : Effect.gen(function* () {
            ...
            const dismissedAt = new Date(yield* Clock.currentTimeMillis).toISOString();
    ...
    const update = (id: typeof TransactionId.Type, changes: TransactionUpdate) =>
    ...
    const bulkCreate = (records: ReadonlyArray<TransactionCreate>) =>
      assertAllLeaves(records).pipe(
        Effect.andThen(
          Effect.forEach(records, (payload) => insertQuery(toWriteRow(payload))).pipe(orDieSql),
        ),
      );
    ...
    const bulkPut = (records: ReadonlyArray<Transaction>) =>
      Effect.forEach(records, (record) => upsertQuery(toWriteRowWithId(record))).pipe(
    ...
    const deleteByImportBatch = (batchId: string) => deleteScope(sql`t.importBatchId = ${batchId}`);
    ...
    const linkTransfer = (
      ...
      Effect.gen(function* () {
        ...
        if (ids.length < 2)
          return yield* Effect.fail(new TransferInvalid({ reason: "too-few-legs" }));

        const legs = yield* bulkGetQuery(ids).pipe(orDieSql);
        if (legs.length !== ids.length)
          return yield* Effect.fail(new TransferInvalid({ reason: "unknown-id" }));
        if (legs.some((l) => l.transferGroupId !== undefined))
          return yield* Effect.fail(new TransferInvalid({ reason: "already-grouped" }));
        if (legs.some((l) => l.isRefund || l.linkedRefundId !== undefined))
          return yield* Effect.fail(new TransferInvalid({ reason: "is-refund" }));
        ...
        if (legs.some((l) => l.bundleId !== undefined || l.kind === "bundle"))
          return yield* Effect.fail(new TransferInvalid({ reason: "is-bundled" }));
        ...
        const cents = legs.reduce((sum, l) => sum + Math.round(l.amount * 100), 0);
        if (cents !== 0) return yield* Effect.fail(new TransferInvalid({ reason: "unbalanced" }));
