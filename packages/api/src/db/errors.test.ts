...
const sqlError = (message: string) =>
	new SqlError({ cause: new Error(message), message });

describe("conflictOrDie", () => {
	it.effect("maps a UNIQUE violation to a typed Conflict", () =>
		Effect.gen(function* () {
			const error = yield* sqlError(
				"UNIQUE constraint failed: settings.key",
			).pipe(Effect.fail, conflictOrDie("setting"), Effect.flip);
...
describe("orDieSql", () => {
	it.effect("turns any SqlError into a defect", () =>
		Effect.gen(function* () {
			const exit = yield* sqlError("boom").pipe(
				Effect.fail,
				orDieSql,
				Effect.exit,
			);
