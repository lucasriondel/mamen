...
const allCategories = Effect.gen(function* () {
  ...
  const rows = yield* sql`SELECT id, name, color, icon, parentId FROM categories`;
 ...
 */
const plant = (row: { name: string; color: string | null; parentId: number | null }) =>
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
...
describe("0015 category colour inheritance + Lucide icon names", () => {
  ...
  it.effect("rewrites every seeded icon to a kebab-case Lucide id", () =>
    Effect.gen(function* () {
      ...
      for (const row of rows) {
        assert.match(row.icon, LUCIDE_ID, `${row.name} icon "${row.icon}" is not a Lucide id`);
      }
  ...
  it.effect("leaves the tree's shape and lookup indexes intact", () =>
    Effect.gen(function* () {
      ...
      const names = (indexes as unknown as Array<{ name: string }>).map((r) => r.name);
      for (const index of [
  ...
  it.effect("nulls the old hardcoded grey, even on a folder with children", () =>
    Effect.gen(function* () {
