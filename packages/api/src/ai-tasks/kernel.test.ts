...
describe("resolving a half-edit", () => {
	it("keeps the stored model when only the model is named", () => {
		...
		assert.deepStrictEqual(
			applyChange({ task: "extract-pdf", model: "gpt-5-mini" }, current),
	...
	it("changes nothing when neither half is named", () => {
		...
		assert.deepStrictEqual(
			applyChange({ task: "extract-pdf" }, current),
			current,
		);
	});
});
...
describe("the patch door, over every provider", () => {
	it("accepts a provider's default model when a credential is stored", () => {
		for (const provider of AI_PROVIDERS) {
			assert.isNull(
				rejectPatch([{ task: "extract-pdf", provider }], fresh, all),
			);
		}
	});
	...
	it("checks a model-only edit against the provider in the stored row", () => {
		...
		assert.isNull(
			rejectPatch(
				[{ task: "extract-pdf", model: "gemini-2.5-pro" }],
				stored,
				all,
			),
		);
		...
		assert.strictEqual(
			rejectPatch([{ task: "extract-pdf", model: "gpt-5" }], stored, all)
				?.reason,
			"model-not-served",
		);
	});
	...
	it("refuses a model-only edit on a stored provider whose key is gone", () => {
		...
		assert.strictEqual(
			rejectPatch([{ task: "extract-pdf", model: "gpt-5" }], stored, none)
				?.reason,
			"no-credential",
		);
	});
...
describe("resolving one task", () => {
	...
	it("gives the same reason a save of the same choice would have", () => {
		...
		assert.strictEqual(
			rejectTask("extract-pdf", stored, none)?.reason,
			"no-credential",
		);
		assert.strictEqual(
			rejectPatch(
