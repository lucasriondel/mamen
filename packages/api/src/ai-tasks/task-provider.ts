import type { AiProvider, AiTask, AiTaskSetting, SecretStatus } from "@mamen/shared/contract";
...
const refuse = (rejection: AiTaskRejection) => Effect.fail(new TaskProviderRejected(rejection));

export class TaskProvider extends Effect.Service<TaskProvider>()("api/TaskProvider", {
  effect: Effect.gen(function* () {
     ...
     */
    const readConfigured: Effect.Effect<Set<AiProvider>> = secrets
      .statusAll()
    ...
    const patch = (
      ...
      Effect.gen(function* () {
        ...
        yield* tasks.writeAll(
          changes.map(
            (change) => [change.task, applyChange(change, current[change.task])] as const,
          ),
        );
     ...
     */
    const resolve = (task: AiTask): Effect.Effect<ResolvedAiTask, TaskProviderRejected> =>
      Effect.gen(function* () {
        const { current, configured } = yield* state;
