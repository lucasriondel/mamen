...
const secretFor = (provider: string) =>
	`sk-${provider}-api03-Kj28fnQ2xLmPqR7v-3f9`;
...
describe("saving a task's provider and model", () => {
	...
	it.effect(
		"lands on the new provider's default when no model is named",
		() => {
			useKey(KEY);
			return Effect.gen(function* () {
				const client = yield* HttpApiClient.make(Api);
...
describe("resolving what a task runs on", () => {
	...
	it.effect(
		"refuses an unknown task id rather than inventing an answer",
		() => {
			useKey(KEY);
			return Effect.gen(function* () {
				const http = yield* HttpClient.HttpClient;
