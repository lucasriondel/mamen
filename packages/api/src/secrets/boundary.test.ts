...
describe("exactly one module decrypts", () => {
  it("no other module imports the decrypt function", () => {
    ...
    const importsDecrypt = /import\s*\{[^}]*\bdecrypt\b[^}]*\}\s*from\s*"[^"]*crypto\/aes-gcm"/;
...
describe("the plaintext reader's in-process callers are named", () => {
  ...
  it("is imported by the two AI-runner transports and nothing else", () => {
    ...
    assert.deepStrictEqual(importers, ["src/ai-runner/claude.ts", "src/ai-runner/service.ts"]);
  });
});

describe("the plaintext reader is not on the barrel", () => {
  it("exports the outward surface and nothing else", () => {
    assert.deepStrictEqual(Object.keys(barrel).sort(), ["SecretsLive", "SecretsRepo"]);
  });
  ...
  it.effect("gives the outward repository no method that returns a secret", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(Object.keys(repo).sort(), ["clear", "put", "status", "statusAll"]);
    }).pipe(Effect.provide(barrel.SecretsRepo.Default), Effect.provide(DatabaseTest)),
  );
});
