...
const withKey = (key: string) =>
	Effect.withConfigProvider(
		ConfigProvider.fromMap(new Map([["TOKEN_ENCRYPTION_KEY", key]])),
	);
...
describe("SecretsRepo storage", () => {
	it.effect("stores the credential as ciphertext, never as itself", () =>
		Effect.gen(function* () {
			...
			assert.notInclude(
				Buffer.from(blobs[0], "base64").toString("latin1"),
				SECRET,
			);
	...
	it.effect("trims the paste before storing it", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(
				read.pipe(Option.map(Redacted.value)),
				Option.some(SECRET),
			);
		}).pipe(Effect.provide(TestLive)),
	);
		...
		Effect.gen(function* () {
			...
			const capture = Logger.replace(
				...
				Logger.make(({ message, annotations, cause }) =>
					lines.push(
						`${JSON.stringify(message)} ${JSON.stringify(annotations)} ${cause}`,
					),
				),
			);
