 ...
 */
export class AppSettingsRepo extends Effect.Service<AppSettingsRepo>()("api/AppSettingsRepo", {
  effect: Effect.gen(function* () {
    ...
    const putQuery = SqlSchema.single({
      ...
      Result: AppSettings,
      execute: (row) => sql`INSERT OR REPLACE INTO appSettings ${sql.insert(row)} RETURNING *`,
    });
    ...
    const get = () =>
      getQuery(void 0).pipe(
        ...
        Effect.flatMap((found) =>
          Option.match(found, {
            onNone: () => Effect.fail(new NotFound({ resource: "appSettings", id: "app" })),
    ...
    const put = (settings: AppSettings) => putQuery({ id: settings.id }).pipe(orDieSql);
