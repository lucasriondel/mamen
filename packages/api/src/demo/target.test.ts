...
const resolve = (argv: ReadonlyArray<string>, env: Record<string, string> = {}) =>
...
describe("resolveSeedTarget", () => {
  ...
  it("refuses the API's own database file, wherever it sits", () => {
    ...
    for (const path of ["mamen.db", "packages/api/mamen.db", "/elsewhere/mamen.db"]) {
      const target = resolve([path]);
      assert.strictEqual(target.ok === false && target.reason, "app-database", path);
    }
  });
