...
const stamp = (month: string, day: number) =>
	`${String(day).padStart(2, "0")}/${month.slice(5)}`;
...
const batchOf = (accountId: number, month: string) =>
	`demo-${month}-account-${accountId}`;
...
const RECURRING: ReadonlyArray<Recurring> = [
	{
		...
		day: 27,
		label: (month) =>
			`VIR SEPA GROUPE VERLAINE SALAIRE ${month.slice(5)}/${month.slice(0, 4)}`,
		cents: 245_000,
		sign: "credit",
	},
	{
...
const recurringRows = (): {
	...
	for (const spec of RECURRING) {
		...
		for (const [index, month] of DEMO_MONTHS.entries()) {
			...
			const cents =
				typeof spec.cents === "number" ? spec.cents : spec.cents[index];
...
const bundleParent = (
	...
	if (derived === undefined)
		throw new Error(`demo dataset: bundle "${tag}" has no members`);
...
const BUNDLES = [
	{ tag: "weekend", label: "Week-end à Sète", categorySlug: "travel" },
] as const;
...
export const buildDemoDataset = (): DemoDataset => {
	...
	for (const bundle of BUNDLES) {
		...
		planned.push(
			bundleParent(members, bundle.label, bundle.categorySlug, bundle.tag),
		);
	}
	...
	const transactions: ReadonlyArray<DemoTransaction> = ordered.map(
		(row, index) => {
			...
			return {
				...
				transferGroupId:
					row.transferTag === undefined
						? null
						: (transferGroups.get(row.transferTag) ?? null),
				kind: isParent ? "bundle" : "bank",
				bundleId:
					row.bundleTag === undefined
						? null
						: refId(`bundle-parent-${row.bundleTag}`),
				manualDate: 0,
				anomalyFlags: null,
