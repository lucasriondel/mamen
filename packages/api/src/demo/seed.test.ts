 ...
 */
const SeedTest = Layer.mergeAll(TransactionRepo.Default, DatabaseRepo.Default).pipe(
...
const bucketsById = <B extends { id: number | null }>(buckets: ReadonlyArray<B>) =>
  new Map(buckets.map((b) => [b.id, b]));

/** What a recap's issuer breakdown adds up to. */
const spent = (recap: { byIssuer: ReadonlyArray<{ spent: number }> }) =>
...
describe("seedDemo", () => {
  it.effect("fills an empty database from its migrations alone", () =>
    Effect.gen(function* () {
      ...
      const dump = yield* DatabaseRepo.pipe(Effect.flatMap((repo) => repo.exportAll()));
  ...
  it.effect("gives every issuer a childless leaf as its default category", () =>
    Effect.gen(function* () {
      ...
      for (const issuer of issuers) {
        ...
        assert.isFalse(parents.has(issuer.defaultCategoryId ?? -1), issuer.name);
      }
    }).pipe(Effect.provide(SeedTest)),
  );
  ...
  it.effect("refuses, writing nothing, when a category it needs is gone", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual([...failure.missingCategories].sort(), ["cafes", "rent"]);
  ...
  it.effect("breaks spend down by issuer and by category, month after month", () =>
    Effect.gen(function* () {
  ...
  it.effect("fills the Accounts month grid, every account and every month", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      yield* seedDemo;
  ...
  it.effect("holds every rule to the issuer its rows already carry", () =>
    Effect.gen(function* () {
      ...
      const { outcomes, skippedRuleIds } = derive(dump.transactions, dump.rules);
