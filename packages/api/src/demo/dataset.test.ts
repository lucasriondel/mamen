...
describe("the demo dataset", () => {
	...
	it("gives every transfer group the smallest leg's id, on both legs", () => {
		...
		for (const t of data.transactions) {
			...
			groups.set(t.transferGroupId, [
				...(groups.get(t.transferGroupId) ?? []),
				t,
			]);
		}
		...
		for (const [groupId, legs] of groups) {
			...
			assert.strictEqual(
				new Set(legs.map((l) => l.accountId)).size,
				legs.length,
			);
	...
	it("shows every curation state the app can paint", () => {
		const has = (
			predicate: (t: (typeof data.transactions)[number]) => boolean,
	...
	it("gives every rule an issuer that exists, and every issuer a leaf slug", () => {
		...
		for (const rule of data.rules) {
			if (rule.matchAccountId !== null)
				assert.isTrue(accountIds.has(rule.matchAccountId));
		}

		// The same pattern twice is fine only when the two can never compete for a
		// row — which is what the account matcher is for.
		const plain = data.rules.filter(
			(r) => r.matchAccountId === null && r.matchValue === null,
		);
	...
	it("describes each subscription with the charges it stands for", () => {
		assert.isAtLeast(
			data.subscriptions.filter((s) => s.status === "possibly-cancelled")
				.length,
			1,
		);
		...
		for (const sub of data.subscriptions) {
			...
			assert.strictEqual(
				sub.issuerName,
				data.issuers.find((i) => i.id === sub.issuerId)?.name,
			);
		}
	});
