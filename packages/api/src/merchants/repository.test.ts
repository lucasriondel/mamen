...
// The repository over a fresh `:memory:` DB. `MerchantRepo.Default` needs a
// SqlClient, provided by `DatabaseTest`; built per test for isolation.
const RepoTest = MerchantRepo.Default.pipe(Layer.provide(DatabaseTest));
...
describe("MerchantRepo", () => {
	...
	it.effect("create keeps optional imageUrl + defaultCategoryId", () =>
		Effect.gen(function* () {
			...
			const created = yield* repo.create(
				make({
					imageUrl: "/uploads/merchants/x.png",
					defaultCategoryId: Schema.decodeSync(CategoryId)(7),
				}),
			);
			assert.strictEqual(created.imageUrl, "/uploads/merchants/x.png");
	...
	it.effect("getByName matches exactly (case-sensitive)", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(
				error,
				new NotFound({ resource: "merchant", id: "netflix" }),
			);
		}).pipe(Effect.provide(RepoTest)),
	);
	...
	it.effect("setImage sets imageUrl, then getById reflects it", () =>
		Effect.gen(function* () {
			...
			const updated = yield* repo.setImage(
				created.id,
				"/uploads/merchants/logo.png",
			);
			assert.strictEqual(updated.imageUrl, "/uploads/merchants/logo.png");

			const fetched = yield* repo.getById(created.id);
			assert.strictEqual(fetched.imageUrl, "/uploads/merchants/logo.png");
		}).pipe(Effect.provide(RepoTest)),
	);
	...
	it.effect("clearImage removes imageUrl (back to absent)", () =>
		Effect.gen(function* () {
			const repo = yield* MerchantRepo;
			const created = yield* repo.create(make());
			yield* repo.setImage(created.id, "/uploads/merchants/logo.png");
	...
	it.effect("getById fails NotFound on a missing id", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(
				error,
				new NotFound({ resource: "merchant", id: asId(404) }),
			);
		}).pipe(Effect.provide(RepoTest)),
	);
