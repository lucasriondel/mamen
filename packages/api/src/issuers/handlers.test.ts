...
const issuerFiles = (prefix: string) =>
  readdirSync(join(uploadsDir, "issuers")).filter((name) => name.startsWith(prefix));

/** A single-colour PNG of the given size — the upload fixture. */
...
describe("issuers endpoints", () => {
  ...
  it.effect("remove deletes the issuer (then getById 404s)", () =>
    Effect.gen(function* () {
      ...
      const error = yield* client.issuers.getById({ path: { id: created.id } }).pipe(Effect.flip);
  ...
  it.effect("remove cascades the image-file cleanup", () =>
    Effect.gen(function* () {
      ...
      const gone = yield* client.issuers.getById({ path: { id: created.id } }).pipe(Effect.flip);
      assert.deepStrictEqual(gone, new NotFound({ resource: "issuer", id: created.id }));
      const after = yield* http.get(url);
      assert.strictEqual(after.status, 404);
    }).pipe(Effect.provide(HttpLive)),
  );

  it.effect("uploadImage normalises a wide PNG to a 128x128 WebP and stores only that", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const created = yield* client.issuers.create({ payload: make() });
  ...
  it.effect("uploadImage refuses an allow-listed MIME whose body isn't an image (415)", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const created = yield* client.issuers.create({ payload: make() });
      ...
      const error = yield* client.issuers
        .uploadImage({
          path: { id: created.id },
          payload: imageFormData("image/png", "not-really.png", new Uint8Array([1, 2, 3, 4])),
        })
        .pipe(Effect.flip);
  ...
  it.effect("uploadImage 404s on a missing issuer", () =>
    Effect.gen(function* () {
      ...
      const error = yield* client.issuers
        ...
        .pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "issuer", id: asId(999) }));
    }).pipe(Effect.provide(HttpLive)),
  );
  ...
  it.effect("getById 404s on a missing id", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const error = yield* client.issuers.getById({ path: { id: asId(999) } }).pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "issuer", id: asId(999) }));
    }).pipe(Effect.provide(HttpLive)),
  );
  ...
  it.effect("getByName 404s on a missing name", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const error = yield* client.issuers.getByName({ path: { name: "nope" } }).pipe(Effect.flip);
  ...
  it.effect("remove 404s on a missing id", () =>
    Effect.gen(function* () {
      const client = yield* HttpApiClient.make(Api);
      const error = yield* client.issuers.remove({ path: { id: asId(999) } }).pipe(Effect.flip);
      assert.deepStrictEqual(error, new NotFound({ resource: "issuer", id: asId(999) }));
    }).pipe(Effect.provide(HttpLive)),
  );
});
