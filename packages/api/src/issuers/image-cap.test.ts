...
vi.mock(
	new URL("image-normalise.ts", import.meta.url).pathname,
...
const withServer = <A>(f: (base: string, uploadsDir: string) => Promise<A>) => {
	...
	return Effect.gen(function* () {
		...
		return yield* Effect.promise(() =>
			f(`http://127.0.0.1:${port}`, uploadsDir),
		);
...
const getIssuer = (base: string, id: number) =>
	fetch(`${base}/api/issuers/${id}`).then(
		(res) => res.json() as Promise<{ imageUrl?: string }>,
	);
...
describe("the image cap is a pre-decode guard", () => {
	...
	it("refuses an oversized upload before sharp is asked to decode it", async () => {
		...
		const { stored, uploadsDir } = await withServer(
			async (base, uploadsDir) => {
	...
	it("accepts an upload just under the cap, so the refusal is about size", async () => {
		...
		const { imageUrl, uploadsDir } = await withServer(
			async (base, uploadsDir) => {
				const issuer = await createIssuer(base);
		...
		assert.deepStrictEqual(readdirSync(join(uploadsDir, "issuers")), [
			filename,
		]);
