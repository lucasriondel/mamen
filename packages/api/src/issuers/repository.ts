...
export const IssuerFromRow = Schema.transform(IssuerRow, Issuer, {
  ...
  decode: (row) => ({
    ...
    ...(row.defaultCategoryId !== null ? { defaultCategoryId: row.defaultCategoryId } : {}),
...
const requireOne = (
  ...
  Option.match(found, {
    onNone: () => Effect.fail(new NotFound({ resource: "issuer", id: key })),
...
export class IssuerRepo extends Effect.Service<IssuerRepo>()("api/IssuerRepo", {
  effect: Effect.gen(function* () {
    ...
    const orderClause = (orderBy: "name" | undefined) =>
      orderBy === "name" ? sql`ORDER BY name COLLATE NOCASE ASC` : sql`ORDER BY id`;
    ...
    const countQuery = SqlSchema.single({
      ...
      Result: CountResult,
      execute: (f) => sql`SELECT COUNT(*) AS count FROM issuers ${whereClause(f)}`,
    });
    ...
    const byNameCiQuery = SqlSchema.findOne({
      Request: Schema.String,
      Result: IssuerFromRow,
      execute: (name) => sql`SELECT * FROM issuers WHERE name COLLATE NOCASE = ${name}`,
    });
    ...
    const updateQuery = SqlSchema.single({
      Request: Schema.Any as Schema.Schema<WriteRow & { id: typeof IssuerId.Type }>,
      Result: IssuerFromRow,
      execute: (row) =>
    ...
    const update = (id: typeof IssuerId.Type, changes: IssuerUpdate) =>
      assertLeaf(changes.defaultCategoryId).pipe(
        ...
        Effect.flatMap((current) => {
          ...
          const notes = changes.notes !== undefined ? changes.notes : (current.notes ?? null);
    ...
    const remove = (id: typeof IssuerId.Type) =>
      getById(id).pipe(
        Effect.flatMap(() => orDieSql(sql`DELETE FROM issuers WHERE id = ${id}`)),
        Effect.asVoid,
      );
