...
const requireOne = (
  ...
  Option.match(found, {
    onNone: () => Effect.fail(new NotFound({ resource: "category", id: key })),
 ...
 * `Conflict`.
 */
export class CategoryRepo extends Effect.Service<CategoryRepo>()("api/CategoryRepo", {
  effect: Effect.gen(function* () {
    ...
    const childCountQuery = SqlSchema.single({
      ...
      Result: CountResult,
      execute: (id) => sql`SELECT COUNT(*) AS count FROM categories WHERE parentId = ${id}`,
    });
    ...
    const issuerRefCountQuery = SqlSchema.single({
      Request: Schema.Number,
      Result: CountResult,
      execute: (id) => sql`SELECT COUNT(*) AS count FROM issuers WHERE defaultCategoryId = ${id}`,
    });
    ...
    const insertQuery = SqlSchema.single({
      ...
      Result: Category,
      execute: (row) => sql`INSERT INTO categories ${sql.insert(row)} RETURNING *`,
    });
      ...
      parentId == null
        ...
        : Effect.iterate(parentId as number | null, {
            ...
            Effect.flatMap((reached) =>
              reached === id
                ? Effect.fail(new CategoryWouldCycle({ categoryId: id, parentId }))
                : Effect.void,
            ),
          );
    ...
    const assertParentTakesNoMoney = (
      ...
      parentId == null
        ? Effect.void
        : Effect.all({
            transactions: txRefCountQuery(parentId).pipe(Effect.map((r) => r.count)),
            issuers: issuerRefCountQuery(parentId).pipe(Effect.map((r) => r.count)),
          }).pipe(
            orDieSql,
     ...
     */
    const assertNoDependents = (id: number): Effect.Effect<void, CategoryInUse> =>
      Effect.all({
    ...
    const list = (filter: ListFilter) =>
      Effect.all({
        ...
        total: countQuery({ parentId: filter.parentId }).pipe(Effect.map((r) => r.count)),
      }).pipe(
    ...
    const bulkCreate = (records: ReadonlyArray<CategoryCreate>) =>
      Effect.forEach(records, (payload) => assertParentTakesNoMoney(payload.parentId)).pipe(
        Effect.andThen(
          nowIso.pipe(
            Effect.flatMap((now) =>
              Effect.forEach(records, (payload) => insertQuery(toInsertRow(payload, now))),
            ),
            orDieSql,
          ),
        ),
      ...
      getById(id).pipe(
        ...
        Effect.flatMap((current) =>
          ...
          assertNoCycle(id, changes.parentId).pipe(
            ...
            Effect.andThen(updateQuery(new Category({ ...current, ...changes })).pipe(orDieSql)),
          ),
        ),
      );
    ...
    const spill = (
      ...
      getById(id).pipe(
        ...
        Effect.flatMap((now) =>
          sql
            .withTransaction(
              insertQuery(toInsertRow({ ...payload, parentId: id }, now)).pipe(
                Effect.tap((leaf) =>
                  Effect.all(
                    [
    ...
    const remove = (id: typeof CategoryId.Type) =>
      getById(id).pipe(
        ...
        Effect.andThen(orDieSql(sql`DELETE FROM categories WHERE id = ${id}`)),
        Effect.asVoid,
      );
