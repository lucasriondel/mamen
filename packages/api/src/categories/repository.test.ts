...
import { type CategoryCreate, CategoryId, NotFound } from "@mamen/shared/contract";
...
describe("CategoryRepo", () => {
  ...
  it.effect("create keeps a non-null parentId", () =>
    Effect.gen(function* () {
      ...
      const child = yield* repo.create(make({ slug: "child", parentId: parent.id }));
  ...
  it.effect("list orders by sortOrder when asked", () =>
    Effect.gen(function* () {
      ...
      yield* repo.create(make({ slug: "third", parentId: parent.id, sortOrder: 30 }));
      yield* repo.create(make({ slug: "first", parentId: parent.id, sortOrder: 10 }));
      yield* repo.create(make({ slug: "second", parentId: parent.id, sortOrder: 20 }));
  ...
  it.effect("list composes parentId + sortOrder together", () =>
    Effect.gen(function* () {
      const repo = yield* CategoryRepo;
      const parent = yield* repo.create(make({ slug: "parent" }));
      yield* repo.create(make({ slug: "b", parentId: parent.id, sortOrder: 20 }));
      yield* repo.create(make({ slug: "a", parentId: parent.id, sortOrder: 10 }));
  ...
  it.effect("getById fails NotFound on a missing id", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(error, new NotFound({ resource: "category", id: asId(404) }));
    }).pipe(Effect.provide(RepoTest)),
  );
