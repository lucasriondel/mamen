...
const source = readFileSync(fileURLToPath(new URL("./ports.ts", import.meta.url)), "utf8");
...
describe("the configs that bind them", () => {
  ...
  it.each(Object.entries(callers))("%s reads its port from this module", (_name, path) => {
  ...
  it.each(Object.entries(callers))("%s states no port as a literal", (_name, path) => {
    const config = readFileSync(path, "utf8");
...
describe("the docs that quote them", () => {
  ...
  it("records every row, the reserved ones included", () => {
    ...
    for (const row of PORTS) {
      ...
      const lines = README.split("\n").filter((line) => line.startsWith(`| ${row.port} |`));
