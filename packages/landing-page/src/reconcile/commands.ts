...
export function readManifests(): Map<string, ReadonlySet<string>> {
  ...
  for (const entry of readdirSync(`${REPO_ROOT}packages`, { withFileTypes: true })) {
    ...
    // A directory under `packages/` is only a workspace if it carries a
    // manifest. A retired package leaves its build output behind — `dist/`,
    // `node_modules/`, a tsbuildinfo — and that leftover is not a workspace
    // whose scripts a reader could run.
    const manifest = `${REPO_ROOT}packages/${entry.name}/package.json`;
    if (existsSync(manifest)) add(manifest);
  }
