...
- The reintroduction path is **not a developer choosing Radix**. `shadcn` is
  still a devDependency (it installs the gousse items), and a snippet pasted from
  the shadcn docs brings the import with it. None of that is a decision anyone
  makes out loud, which is why the failure has to be a red test rather than a
  review comment.
- **`components.json` declares the `base-nova` style** (issue #103), which is the
  one field a stock `shadcn add` resolves against: the `@shadcn` registry's URL
  template is `…/r/styles/{style}/{name}.json`. The style picks the variant and
  the variant picks the primitive system — under the `new-york` it replaces,
  `shadcn add dialog` emitted `@radix-ui/react-dialog`; under `base-nova` it
  emits Base UI. This is configuration for the *next* pull and nothing else: no
  vendored file is regenerated by it, and the `@gousse` registry does not read
  the field at all (its URL carries no `{style}`). A scratch pull confirmed both
  halves — the gousse items import no primitive library whatsoever, and
  `tokens.css` and `theme.css` come out byte-identical to what is in the tree.
- **What the stock registry emits would still not compile here.** `base-nova`
  items import `@base-ui/react` — Base UI's 1.0 package name — and mamen is
  pinned to `@base-ui-components/react@1.0.0-rc.0`, the last release published
  under the old one. The items declare no npm dependency either, so such an
  import resolves to nothing installed. Renaming the dependency is
  [issue #104](https://github.com/lucasriondel/mamen/issues/104), deliberately
  not part of #103: `1.0.0-rc.0` to `1.7.0` crosses the 1.0 release, so it is an
  upgrade with an API to re-derive — the popover's four restated positioning
  defaults above all — rather than a find-and-replace.
- `src/test/components-json.test.ts` holds the style, the `@gousse` namespace and
  the aliases a re-install writes through.
