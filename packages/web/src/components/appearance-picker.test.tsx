...
const spectrum = () => screen.getByRole("slider", { name: /saturation and brightness/i });

async function open(label = "Food") {
  const user = userEvent.setup();
  await user.click(screen.getByRole("button", { name: `Change ${label} appearance` }));
...
describe("AppearancePicker", () => {
  ...
  it("renders a single trigger showing the icon in its resolved colour", async () => {
    ...
    expect(screen.getByRole("button", { name: "Change Groceries appearance" })).toBeInTheDocument();
  ...
  it("steps into the grid on Enter in the search field", async () => {
    ...
    expect(onSubmit).not.toHaveBeenCalled();
    await waitFor(() => expect(screen.getByRole("button", { name: "shopping-bag" })).toHaveFocus());
  });
  ...
  it("moves the spectrum when a palette colour is picked", async () => {
    ...
    expect(spectrum()).toHaveAttribute("aria-valuetext", "Saturation 94%, brightness 91%");
  ...
  it("moves the spectrum when a hex is typed", async () => {
    ...
    expect(spectrum()).toHaveAttribute("aria-valuetext", "Saturation 100%, brightness 100%");
  });
  ...
  it("shapes the trigger as a pill", () => {
    ...
    expect(screen.getByRole("button", { name: "Change Food appearance" }).className).toContain(
      "rounded-full",
    );
  });
});
