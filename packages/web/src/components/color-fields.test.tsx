...
describe("PaletteGrid", () => {
  ...
  it("offers only canonically spelled palette colours", () => {
    ...
    expect(new Set(PALETTE.map((entry) => entry.hex)).size).toBe(PALETTE.length);
  });
...
describe("SpectrumArea", () => {
  ...
  it("walks brightness with the arrow keys", async () => {
    ...
    expect(spectrum()).toHaveAttribute("aria-valuetext", "Saturation 100%, brightness 50%");

    spectrum().focus();
    await user.keyboard("{ArrowDown}");

    expect(spectrum()).toHaveAttribute("aria-valuetext", "Saturation 100%, brightness 49%");
  ...
  it("walks saturation with the arrow keys", async () => {
    ...
    expect(spectrum()).toHaveAttribute("aria-valuetext", "Saturation 99%, brightness 50%");
