...
describe("TransactionRow", () => {
	...
	it("applies selected styling when isSelected is true", () => {
		...
		expect((row as HTMLElement).style.backgroundColor).toBe(
			"hsl(var(--ring) / 0.08)",
		);
	});
	...
	it("calls onClick when clicked", async () => {
		...
		render(
			<TransactionRow transaction={makeTransaction()} onClick={handleClick} />,
		);
	...
	it("shows badge but no link icon for orphan refunds", () => {
		render(
			<TransactionRow transaction={makeTransaction({ isRefund: true })} />,
		);
		expect(screen.getByTestId("refund-badge")).toBeInTheDocument();
	...
	it("link icon has correct aria-label for refund", () => {
		...
		expect(
			screen.getByLabelText("Navigate to linked purchase"),
		).toBeInTheDocument();
	});
	...
	it("link icon has correct aria-label for purchase side", () => {
		...
		expect(
			screen.getByLabelText("Navigate to linked refund"),
		).toBeInTheDocument();
	});
});
