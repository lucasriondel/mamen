...
function renderLayout(ui: React.ReactElement, value: SidebarCollapsedContextValue = shell()) {
  return render(<SidebarCollapsedProvider value={value}>{ui}</SidebarCollapsedProvider>);
}
...
describe("PageLayout", () => {
  ...
  it("re-opens the panel through the shell's own toggle", async () => {
    ...
    const toggle = vi.fn();
    renderLayout(<PageLayout title="Accounts" />, shell({ collapsed: true, toggle }));
  ...
  it("hands the shell its trigger, so focus can follow the control", () => {
    ...
    renderLayout(<PageLayout title="Accounts" />, shell({ collapsed: true, triggerRef }));
  ...
  it("renders a page's own actions in the topbar, beside the title", () => {
    ...
    expect(within(topbar("Issuers")).getByRole("button", { name: "New" })).toBeInTheDocument();
  });
