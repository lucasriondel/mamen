...
async function renderBreadcrumb(segments: BreadcrumbSegment[], className?: string): Promise<ReturnType<typeof render>> {
  ...
  const routeTree = rootRoute.addChildren([indexRoute, transactionsRoute, merchantsRoute])
...
describe('Breadcrumb', () => {
  ...
  it('renders earlier segments as links', async () => {
    ...
    const nav = screen.getByRole('navigation', { name: 'Breadcrumb', hidden: true })
    const link = within(nav).getByRole('link', { name: 'Transactions', hidden: true })
