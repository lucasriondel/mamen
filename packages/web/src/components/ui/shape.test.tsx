...
describe("the shape contract", () => {
  ...
  it("gives the dialog panel the box corner and its close button a pill", () => {
    ...
    expect(screen.getByRole("button", { name: "Close" }).className).toContain("rounded-full");
  });
  ...
  it("gives the popover surface the box corner", () => {
    ...
    expect(screen.getByText("Anchored").parentElement?.className).toContain("rounded-2xl");
  });
