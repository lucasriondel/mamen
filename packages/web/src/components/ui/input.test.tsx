...
describe("Input", () => {
  ...
  it("defaults to a text input but lets the type be overridden", () => {
    ...
    expect(screen.getByRole("textbox", { name: "Field" })).toHaveAttribute("type", "text");

    rerender(<Input aria-label="Field" type="number" />);
    expect(screen.getByRole("spinbutton", { name: "Field" })).toHaveAttribute("type", "number");
  });
  ...
  it("lets a call-site class win over the default it conflicts with", () => {
    render(<Input aria-label="Issuer name" className="w-full bg-gousse-panel" />);
