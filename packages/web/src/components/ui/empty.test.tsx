...
describe("Empty", () => {
  it("renders the title, the description and the action", () => {
    ...
    expect(screen.getByRole("link", { name: "Import a statement" })).toBeInTheDocument();
  });
  ...
  it("renders an optional icon above the title", () => {
    render(
      <Empty
        // oxlint-disable-next-line jsx-a11y/prefer-tag-over-role -- a named `<svg>` is what the callers pass; an `<img>` would test a different thing
  ...
  it("takes gousse's dashed border by default and a solid one on request", () => {
    ...
    expect(container.querySelector("div")?.className).toContain("border-dashed");

    rerender(<Empty title="All caught up" variant="solid" />);
    expect(container.querySelector("div")?.className).not.toContain("border-dashed");
  });

  it("merges a call-site class onto the panel", () => {
    const { container } = render(<Empty title="No categories yet" className="mt-8" />);
