...
describe("Checkbox", () => {
  ...
  it("reports a change to its call-site", async () => {
    ...
    render(<Checkbox aria-label="Exclude Spotify" checked={false} onChange={onChange} />);
  ...
  it("refuses a click when disabled", async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    render(<Checkbox aria-label="Exclude Spotify" checked={false} disabled onChange={onChange} />);
