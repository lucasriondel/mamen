...
describe('seedCategories', () => {
  ...
  it('assigns correct colors to categories', async () => {
    ...
    const allCategories = await db.categories.toArray()
    const shopping = allCategories.find((c) => c.name === 'Shopping' && c.parentId === null)!
    expect(shopping.color).toBe('#3B82F6')

    const dining = allCategories.find((c) => c.name === 'Dining' && c.parentId === null)!
  ...
  it('assigns correct icons to categories', async () => {
    ...
    const travel = allCategories.find((c) => c.name === 'Travel' && c.parentId === null)!
