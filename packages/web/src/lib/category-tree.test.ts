...
describe("category-tree", () => {
  ...
  describe("buildTree", () => {
    it("nests leaves under their folder in sort order", () => {
      ...
      expect(foodNode?.children.map((c) => c.name)).toEqual(["Groceries", "Restaurants"]);
    });
  ...
  describe("descendantIds", () => {
    it("expands a folder to its leaf ids (one hop)", () => {
      ...
      expect(descendantIds(categories, food.id)).toEqual([groceries.id, restaurants.id]);
    });
    ...
    it("descends the whole subtree to a leaf at any depth", () => {
      ...
      expect(descendantIds(categories, life.id)).toEqual([streaming.id, music.id, bills.id]);
      // A mid-tier folder rolls up only its own subtree.
      expect(descendantIds(categories, subs.id)).toEqual([streaming.id, music.id]);
    });
  });
  ...
  describe("subtreeIds", () => {
    ...
    it("is just the node for a childless leaf", () => {
      const { groceries, categories } = seed();
      expect(subtreeIds(categories, groceries.id)).toEqual(new Set([groceries.id]));
    });
  });
  ...
  describe("resolveCategoryColor", () => {
    ...
    it("recolouring a folder recolours every inheriting descendant", () => {
      ...
      expect(resolveCategoryColor([food, groceries, cafes], groceries)).toBe("#ef4444");
    ...
    it("falls back to the neutral constant when the walk reaches a null root", () => {
      ...
      expect(resolveCategoryColor([root, leaf], leaf)).toBe(NEUTRAL_CATEGORY_COLOR);
      expect(resolveCategoryColor([root, leaf], root)).toBe(NEUTRAL_CATEGORY_COLOR);
    });
    ...
    it("terminates on a detached node and on a cycle", () => {
      ...
      expect(resolveCategoryColor([detached], detached)).toBe(NEUTRAL_CATEGORY_COLOR);
  ...
  describe("resolveCategoryColors", () => {
    it("agrees with the single-node resolver for every shape in a list", () => {
      ...
      for (const cat of categories) {
        expect(colorById.get(cat.id)).toBe(resolveCategoryColor(categories, cat));
      }
    ...
    it("keys every category passed in, and only those", () => {
      ...
      const { categories } = seed();
      const colorById = resolveCategoryColors(categories);
      expect([...colorById.keys()].sort()).toEqual(categories.map((c) => c.id).sort());
    });
