...
export interface SidebarCollapsedContextValue {
	...
	collapsed: boolean;
	/** Flip it — what a page's {@link PageLayout} trigger calls on click. */
	toggle: () => void;
	/** The trigger button each `PageLayout` renders while collapsed, so focus can
...
export function useSidebarCollapsedContext(): SidebarCollapsedContextValue {
	...
	if (!value) {
		throw new Error("useSidebarCollapsedContext must be used within AppShell");
	}
	return value;
}
