...
import { createSubscriptionSchema, subscriptionSchema } from "@/lib/schemas";
...
describe("subscriptionSchema", () => {
	...
	it("validates a valid subscription", () => {
		expect(subscriptionSchema.parse(validSubscription)).toEqual(
			validSubscription,
		);
	});
	...
	it("validates all frequency values", () => {
		for (const frequency of ["weekly", "monthly", "yearly"] as const) {
			expect(
				subscriptionSchema.parse({ ...validSubscription, frequency }),
			).toBeDefined();
		}
	});
