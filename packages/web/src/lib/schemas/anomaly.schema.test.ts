...
describe("anomalyTypeSchema", () => {
	...
	it("validates potential-duplicate", () => {
		expect(anomalyTypeSchema.parse("potential-duplicate")).toBe(
			"potential-duplicate",
		);
	});
...
describe("anomalyFlagSchema", () => {
	...
	it("validates flag with dismissedAt", () => {
		const dismissed = {
			...validFlag,
			dismissed: true,
			dismissedAt: "2026-02-02T10:00:00.000Z",
		};
	...
	it("rejects invalid anomaly type", () => {
		expect(() =>
			anomalyFlagSchema.parse({ ...validFlag, type: "invalid" }),
		).toThrow();
	});
...
describe("anomalySettingsSchema", () => {
	...
	it("rejects multiplier below 1", () => {
		expect(() =>
			anomalySettingsSchema.parse({
				...validSettings,
				multiplierThreshold: 0.5,
			}),
		).toThrow();
	});
	...
	it("rejects minTransactions above 20", () => {
		expect(() =>
			anomalySettingsSchema.parse({
				...validSettings,
				minTransactionsForDetection: 21,
			}),
		).toThrow();
	});
	...
	it("accepts minTransactions at boundaries", () => {
		expect(
			anomalySettingsSchema.parse({
				...validSettings,
				minTransactionsForDetection: 3,
			}),
		).toBeDefined();
		expect(
			anomalySettingsSchema.parse({
				...validSettings,
				minTransactionsForDetection: 20,
			}),
		).toBeDefined();
	});
});
