...
export const rulesApi = {
	getAll: (params: { merchantId?: number } = {}) =>
		api.get<Rule[]>(
			`/rules${params.merchantId !== undefined ? `?merchantId=${params.merchantId}` : ""}`,
		),
	...
	count: (params: { merchantId?: number } = {}) =>
		api
			.get<{ count: number }>(
				`/rules/count${params.merchantId !== undefined ? `?merchantId=${params.merchantId}` : ""}`,
			)
			.then((r) => r.count),

	getByMerchantIdAndPattern: (merchantId: number, pattern: string) =>
		api.get<Rule>(
			`/rules/by-merchant-pattern/${merchantId}/${encodeURIComponent(pattern)}`,
		),
	...
	bulkAdd: async (records: Omit<Rule, "id">[]) => {
		const result = await api.post<{ ids: number[] }>("/rules/bulk-add", {
			records,
		});
		return result.ids;
	},
