...
import { ArrowLeft, CircleHelp, ExternalLink, Plus, Search, SquarePen } from "lucide-react";
 ...
 */
export function AssignmentPicker({ transactionId, rawIssuerString, date }: AssignmentPickerProps) {
  ...
  const createWithRule = () => {
    ...
    createIssuer.mutate({ name: trimmed }, { onSuccess: (issuer) => goToNewRule(issuer.id) });
  };
  ...
  const searchOnGoogle = () => {
    close();
    const url = `https://www.google.com/search?q=${encodeURIComponent(`"${rawIssuerString}"`)}`;
      ...
      <PopoverContent className="p-0">
        <Command shouldFilter={false} label="Assign an issuer">
          <CommandInput
            ...
            placeholder={
              mode === "add-rule" ? "Pick an issuer to add a rule to…" : "Search issuers…"
            }
          ...
          <CommandList>
            ...
            {issuers.length > 0 ? (
              <CommandGroup heading={mode === "add-rule" ? "Add a rule to…" : "Match an issuer"}>
                {issuers.map((issuer) => (
                  <CommandItem
                    key={issuer.id}
            ...
            {mode === "add-rule" ? (
              ...
              <>
                ...
                <CommandGroup heading="Or">
                  <CommandItem value="__add_rule__" onSelect={enterAddRule} disabled={pending}>
                    <SquarePen size={16} className="shrink-0 text-gousse-muted" aria-hidden />
                    <span className="truncate">Add a rule to an existing issuer</span>
                  </CommandItem>
                  {canCreate ? (
                    <CommandItem value="__create__" onSelect={createWithRule} disabled={pending}>
                      <Plus size={16} className="shrink-0 text-gousse-muted" aria-hidden />
                      <span className="truncate">Create issuer “{trimmed}” with a rule</span>
                    </CommandItem>
                  ) : null}
                  <CommandItem value="__google_search__" onSelect={searchOnGoogle}>
                    <Search size={16} className="shrink-0 text-gousse-muted" aria-hidden />
                    <span className="truncate">Search “{rawIssuerString}” on Google</span>
                  </CommandItem>
                  {isPaypal ? (
                    <CommandItem value="__paypal_activity__" onSelect={openPaypalActivity}>
