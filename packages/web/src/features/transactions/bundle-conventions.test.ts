...
describe("bundle UI conventions (issue #82)", () => {
  ...
  it.each(Object.entries(SOURCES))("%s declares its shapes as type aliases", (_path, source) => {
  ...
  it.each(Object.entries(SOURCES))("%s declares one component", (_path, source) => {
    const components = [...source.matchAll(COMPONENT_DECLARATION)].map((match) => match[1]);
