...
describe('replaceLinkRefund', () => {
  it('clears old purchase reference', async () => {
    const oldPurchaseId = await db.transactions.add(createTransaction({ amount: -29.99 }))
    const newPurchaseId = await db.transactions.add(createTransaction({ amount: -31.50, rawMerchantString: 'STORE' }))
    const refundId = await db.transactions.add(createTransaction({ amount: 29.99 }))
...
describe('category inheritance', () => {
  it('linkRefund inherits categoryId from purchase when refund has no category', async () => {
    const purchaseId = await db.transactions.add(createTransaction({ amount: -29.99, categoryId: 5 }))
    const refundId = await db.transactions.add(createTransaction({ amount: 29.99 }))
  ...
  it('linkRefund does NOT override refund categoryId when refund already has a category', async () => {
    const purchaseId = await db.transactions.add(createTransaction({ amount: -29.99, categoryId: 5 }))
    const refundId = await db.transactions.add(createTransaction({ amount: 29.99, categoryId: 10 }))
  ...
  it('replaceLinkRefund inherits category from new purchase when refund has no manual category', async () => {
    const oldPurchaseId = await db.transactions.add(createTransaction({ amount: -29.99 }))
    const newPurchaseId = await db.transactions.add(createTransaction({ amount: -31.50, categoryId: 7 }))
    const refundId = await db.transactions.add(createTransaction({ amount: 29.99 }))
