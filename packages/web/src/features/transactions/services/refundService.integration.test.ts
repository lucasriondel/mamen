...
const createTx = (overrides: Partial<Parameters<typeof db.transactions.add>[0]> = {}) => ({
  accountId: 1,
...
describe('Refund integration flows', () => {
  it('full flow: link refund → verify bidirectional → undo → verify restored', async () => {
    ...
    const refundId = await db.transactions.add(createTx({ amount: 29.99, rawMerchantString: 'AMAZON REFUND' }))
  ...
  it('data integrity: linked transactions reference each other bidirectionally', async () => {
    const purchaseId = await db.transactions.add(createTx({ amount: -50.00 }))
    const refundId = await db.transactions.add(createTx({ amount: 50.00 }))
    ...
    const linkedFromPurchase = await db.transactions.get(purchase!.linkedRefundId!)
  ...
  it('prevents double-linking (purchase already linked)', async () => {
    const purchaseId = await db.transactions.add(createTx({ amount: -29.99 }))
    const refund1Id = await db.transactions.add(createTx({ amount: 29.99, rawMerchantString: 'REFUND 1' }))
    const refund2Id = await db.transactions.add(createTx({ amount: 29.99, rawMerchantString: 'REFUND 2' }))

    await linkRefund(refund1Id, purchaseId)
    await expect(linkRefund(refund2Id, purchaseId)).rejects.toThrow('already has a linked refund')
  })
  ...
  it('can query linked transactions via linkedRefundId index', async () => {
    ...
    // Query by index
    const linkedToPurchase = await db.transactions.where('linkedRefundId').equals(purchaseId).toArray()
