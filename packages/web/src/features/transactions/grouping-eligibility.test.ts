...
describe("bundle / transfer-group exclusivity", () => {
	...
	it("refuses a bundle parent the transfer", () => {
		expect(
			isTransferEligible(row({ kind: "bundle" } as Partial<Transaction>)),
		).toBe(false);
	});
	...
	it("keeps refusing an already-grouped row and a refund", () => {
		expect(
			isTransferEligible(row({ transferGroupId: 7 } as Partial<Transaction>)),
		).toBe(false);
		expect(
			isTransferEligible(row({ isRefund: true } as Partial<Transaction>)),
		).toBe(false);
		expect(
			isTransferEligible(row({ linkedRefundId: 9 } as Partial<Transaction>)),
		).toBe(false);
	});
});
