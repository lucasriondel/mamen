...
vi.mock("@mamen/sdk", async (importOriginal) => {
	...
	return {
		...
		transactionMutations: {
			createBundle: (ids: unknown, label: unknown) =>
				createBundleFn(ids, label),
			addBundleMember: (bundleId: unknown, id: unknown) =>
				addBundleMemberFn(bundleId, id),
...
describe("useBundle", () => {
	...
	it("invalidates the rules cache after a bundle is created", async () => {
		...
		await waitFor(() =>
			expect(result.current.createBundle.isSuccess).toBe(true),
		);
	...
	it("invalidates the rules cache after a bundle is dissolved", async () => {
		...
		await waitFor(() =>
			expect(result.current.dissolveBundle.isSuccess).toBe(true),
		);
		expect(isStale(RULES_KEY)).toBe(true);
	});
	...
	it("invalidates the rules cache after a member leaves", async () => {
		...
		await waitFor(() =>
			expect(result.current.removeFromBundle.isSuccess).toBe(true),
		);
		expect(isStale(RULES_KEY)).toBe(true);
	});
	...
	it("invalidates the rules cache after a member is added", async () => {
		...
		await waitFor(() =>
			expect(result.current.addToBundle.isSuccess).toBe(true),
		);
		expect(isStale(RULES_KEY)).toBe(true);
	});
	...
	it("invalidates the rules cache after the parent is edited", async () => {
		...
		await waitFor(() =>
			expect(result.current.setBundleDate.isSuccess).toBe(true),
		);
		expect(isStale(RULES_KEY)).toBe(true);
	});
