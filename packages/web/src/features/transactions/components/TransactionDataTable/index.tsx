...
export function TransactionDataTable({ highlightId }: TransactionDataTableProps): React.ReactElement {
  ...
  const tableMeta: TransactionTableMeta = useMemo(
    ...
    [getMerchantCreatedAt, handleDismissAnomaly, handleDismissDuplicate, handleExcludeDuplicate, navigateToTransaction, animatingIdSet, animationPhase],
  )
  ...
  const selectionCount = Object.keys(rowSelection).filter((k) => rowSelection[k]).length

  const anyModalOpen = merchantModalOpen || categoryPickerOpen || refundLink.isOpen || deleteTransactions.isDeleteDialogOpen
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setCursorRowId((current) => (current === String(highlightId) ? null : current))
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        >
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            const isHighlighted = animatingIdSet.has(rowId) && (animationPhase === 'highlight' || animationPhase === 'settle')
      ...
      <DeleteConfirmDialog
        ...
        onOpenChange={(open) => { if (!open) deleteTransactions.cancelDelete() }}
        ...
        onCategorySelect={(categoryId, subcategoryId) => {
          if (categoryBatchIds.length > 1) {
            ...
            if (categoryBatchFirstId !== null) {
              const idx = flatRows.findIndex((t) => t.id === categoryBatchFirstId)
              if (idx !== -1) {
