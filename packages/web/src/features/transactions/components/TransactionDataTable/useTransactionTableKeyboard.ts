...
export const useTransactionTableKeyboard = ({
  ...
  const moveCursor = useCallback(
    (direction: 1 | -1, shiftKey: boolean) => {
      ...
      if (shiftKey) {
        ...
        if (Object.keys(tbl.getState().rowSelection).length === 0 && prevIndex >= 0) {
  ...
  const resolveTarget = useCallback((): ActionTarget | null => {
    ...
    if (cursorRef.current !== null) {
      const tx = rowsRef.current.find((r) => String(r.id) === cursorRef.current)
      if (tx) return { mode: 'single', transaction: tx }
    }

    if (hoveredRef.current !== null) {
      const tx = rowsRef.current.find((r) => String(r.id) === hoveredRef.current)
      if (tx) return { mode: 'single', transaction: tx }
    }
