...
beforeEach(() => {
  ...
  createCategory.mockReset().mockResolvedValue({ id: 42, name: "Coffee gear", parentId: 1 });
});
...
describe("CategoryPicker", () => {
  ...
  it("removes an override on a marked row, reverting to the issuer default", async () => {
    ...
    render(
      <CategoryPicker transaction={tx({ id: 101, manualCategory: true })} category={groceries} />,
    );
  ...
  it("hides the remove action on an inherited (non-override) row", async () => {
    ...
    render(
      <CategoryPicker transaction={tx({ id: 102, manualCategory: false })} category={groceries} />,
    );
