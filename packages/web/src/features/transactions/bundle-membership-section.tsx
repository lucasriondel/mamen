...
import { BUNDLE_REFUSED_REASON, isBundleEligible } from "./grouping-eligibility";
 ...
 * recomputed.
 */
export function BundleMembershipSection({ transaction: txn }: { transaction: Transaction }) {
  ...
  const parent = txn.bundleId != null ? bundles.find((b) => b.id === txn.bundleId) : undefined;
  ...
  const joinForm = (
    ...
    >
      ...
      <Button type="submit" size="sm" disabled={!canJoin || pending || choice === ""}>
      ...
      {txn.bundleId != null ? (
        <div className="flex flex-col gap-3">
          <p className="text-sm text-gousse-muted">
            This transaction is part of a bundle, so it is counted through the row that stands for
          ...
          <p className="text-xs text-gousse-muted">
            Removing the second-to-last member dissolves the bundle — the rows come back exactly as
            they are.
          </p>
        </div>
      ) : !canJoin ? (
        <div className="flex flex-col gap-3">
          <p className="text-sm text-gousse-muted italic">{BUNDLE_REFUSED_REASON}</p>
          {joinForm}
        </div>
        ...
        <p className="text-sm text-gousse-muted italic">
          No bundles yet. Select this row and the ones it belongs with in the transactions list to
          make one.
        </p>
      ) : (
        joinForm
