        ...
        return {
          ...
          queryFn: async () => {
            const matches = issuersList
              .filter((issuer) => issuer.name.toLowerCase().includes(term.trim().toLowerCase()))
...
function makeRouter(rawIssuerString = "ACME PAYROLL", date = new Date(2026, 0, 15)) {
...
async function open(rawIssuerString = "ACME PAYROLL", date?: Date) {
  ...
  await user.click(await screen.findByRole("button", { name: rawIssuerString }));
...
describe("AssignmentPicker", () => {
  ...
  it("creates an issuer with a rule, then navigates to its rule page with the pattern", async () => {
    ...
    await waitFor(() =>
      expect(createIssuer).toHaveBeenCalledWith(expect.objectContaining({ name: "ACME PAYROLL" })),
    );
  ...
  it("adds a rule to an existing issuer: pick, then navigate with the pattern", async () => {
    ...
    await user.click(await screen.findByText("Add a rule to an existing issuer"));
    await user.click(await screen.findByText("Spotify"));
  ...
  it("escapes regex metacharacters in the pre-filled pattern", async () => {
    ...
    const pattern = stub.textContent?.replace(/^Rule page for issuer 10 pattern=/, "");
    expect(pattern).toBe("PAYPAL \\*EBAY \\(FR\\)");
    // The escaped pattern compiles and matches the literal it came from.
    expect(new RegExp(pattern as string, "i").test("PAYPAL *EBAY (FR)")).toBe(true);
  });
  ...
  it("opens the PayPal activity feed windowed on the transaction date", async () => {
    ...
    const user = await open("PayPal Europe S.a.r.l. et Cie S.C.A", new Date(2026, 0, 15));
  ...
  it("hides 'create' when the typed name already exists", async () => {
    ...
    expect(await screen.findByRole("option", { name: "ACME PAYROLL" })).toBeInTheDocument();
    expect(screen.queryByText(/Create issuer .* with a rule/)).not.toBeInTheDocument();
  });
});
