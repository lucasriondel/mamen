...
function renderPage(id = 100, shellValue = OPEN_SHELL) {
  return render(withShell(<RouterProvider router={makeRouter(id)} />, shellValue));
}
...
describe("TransactionDetailPage", () => {
  ...
  it("keeps the amount, the counterparty and the date in one topbar", async () => {
    ...
    const topbar = (await screen.findByRole("heading", { level: 1, name: "Spotify" })).closest(
      "header",
    ) as HTMLElement;
  ...
  it("names the issuer the list read would have missed", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Spotify" })).toBeVisible();
    ...
    expect(issuerByIdsMock.mock.calls.some(([ids]) => [...ids].includes(812))).toBe(true);
  });
  ...
  it("offers the curation controls on any row, bundle parent included", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Weekend Bretagne" })).toBeVisible();
    ...
    expect(screen.getByTitle("Set a category for this transaction")).toBeVisible();
  ...
  it("offers dissolving only on the bundle parent", async () => {
    ...
    expect(await screen.findByRole("button", { name: /dissolve bundle/i })).toBeVisible();
  });
  ...
  it("warns on a bundle whose members do not sum to a cost", async () => {
    ...
    expect(screen.getByRole("button", { name: /dissolve bundle/i })).toBeVisible();
  });
  ...
  it("tells a bundle parent it can't be part of a transfer", async () => {
    ...
    expect(
      await screen.findByText(/bundled transaction can't be part of a transfer/i),
    ).toBeVisible();
    expect(screen.queryByRole("button", { name: /link as transfer/i })).toBeNull();
  });
  ...
  it("reads its counterparts from the shared candidate cache", async () => {
    candidateRows = [{ leg: TXN, counterparts: [{ transaction: COUNTERPART, daysApart: 1 }] }];
    renderPage();
  ...
  it("offers one group-level dismissal, normalised debit-first", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /not a transfer/i }));

    expect(dismissPairsMock).toHaveBeenCalledWith([{ debitId: TXN.id, creditId: COUNTERPART.id }]);
  });
});
