 ...
 */
export function TransferSection({ transaction: txn }: { transaction: Transaction }) {
  ...
  const otherLegs = ((groupQuery.data?.items ?? []) as readonly Transaction[]).filter(
    (leg) => leg.id !== txn.id,
  );
      ...
      {isGrouped ? (
        <div className="flex flex-col gap-3">
          <p className="text-sm text-gousse-muted">
            This transaction is part of an internal transfer — its legs are excluded from your recap
            spend.
          </p>
        ...
        <div className="flex flex-col gap-3">
          <p className="text-sm text-gousse-muted">
            These look like the other side of this money movement, closest date first. Link one to
              ...
              <LegRow
                ...
                action={
                  <Button
                    ...
                    onClick={() =>
                      link.mutate([txn.id, counterpart.transaction.id] as TransactionId[])
                    }
                  >
                    Link as transfer
                  </Button>
          ...
          <div className="flex flex-col gap-1">
            <Button
              ...
              onClick={() =>
                dismiss.mutate(
                  suggestions.map((counterpart) => toTransferPair(txn, counterpart.transaction)),
                )
              }
            >
