...
describe("BundleMembershipSection (issue #74)", () => {
  ...
  it("offers the bundle parents and nothing else", async () => {
    ...
    expect(listTransactions).toHaveBeenCalledWith(expect.objectContaining({ kind: "bundle" }));
    expect(await screen.findByRole("option", { name: /weekend bretagne/i })).toBeVisible();
  });
  ...
  it("offers the way out of a bundle it already belongs to", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /remove from bundle/i }));
  ...
  it("still offers the way out to a leg that is already bundled", async () => {
    renderSection(bankRow({ transferGroupId: 77, bundleId: 300 } as Partial<Transaction>));

    expect(await screen.findByRole("button", { name: /remove from bundle/i })).toBeEnabled();
  });
