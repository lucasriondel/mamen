...
describe("BundleDissolveBlock (issue #82)", () => {
  ...
  it("says dissolving returns the members rather than deleting them", async () => {
    render(<BundleDissolveBlock disabled={false} isDissolving={false} onDissolve={vi.fn()} />);

    expect(await screen.findByText(/returns its members to the list/i)).toBeVisible();
  });

  it("dissolves on demand", async () => {
    const onDissolve = vi.fn();
    render(<BundleDissolveBlock disabled={false} isDissolving={false} onDissolve={onDissolve} />);
  ...
  it("waits on any other write on the bundle", async () => {
    render(<BundleDissolveBlock disabled isDissolving={false} onDissolve={vi.fn()} />);

    expect(await screen.findByRole("button", { name: /dissolve bundle/i })).toBeDisabled();
  });
});
