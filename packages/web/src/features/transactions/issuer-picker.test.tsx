...
/** The value of the option the picker currently marks as selected. */
const selected = () =>
  screen
    .getAllByRole("option")
...
describe("IssuerPicker", () => {
  it("opens on the row actions, in order, with no search box", async () => {
    ...
    const actions = screen.getAllByRole("option").map((o) => o.getAttribute("data-value"));
  ...
  it("navigates to the issuer page via 'Go to'", async () => {
    ...
    await user.click(screen.getByRole("option", { name: /Go to MINT ENERGIE/ }));
  ...
  it("returns to the row actions via 'Back'", async () => {
    ...
    expect(await screen.findByRole("option", { name: /Go to MINT ENERGIE/ })).toBeInTheDocument();
    expect(screen.queryByLabelText("Search issuers")).not.toBeInTheDocument();
  });
  ...
  describe("issuer note tooltip", () => {
    ...
    it("reveals the issuer's note on hover", async () => {
      const user = userEvent.setup();
      render(<RouterProvider router={makeRouter(tx({ manualIssuer: false }), withNote)} />);

      await user.hover(await screen.findByRole("button", { name: /MINT ENERGIE/ }));
    ...
    it("shows no tooltip when the issuer has no note", async () => {
      const user = userEvent.setup();
      render(<RouterProvider router={makeRouter(tx({ manualIssuer: false }))} />);
      ...
      expect(trigger).toHaveAttribute("title", "Change the issuer for this transaction");
    ...
    it("still opens the picker on a row whose issuer has a note", async () => {
      ...
      const user = userEvent.setup();
      render(<RouterProvider router={makeRouter(tx({ manualIssuer: true }), withNote)} />);
