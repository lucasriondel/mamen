...
describe("TransactionsPagination", () => {
  it("shows the page number and range, and disables Previous on page 1", () => {
    render(<TransactionsPagination page={1} pageSize={50} total={120} onPageChange={vi.fn()} />);
  ...
  it("advances to the next page on Next", async () => {
    ...
    render(
      <TransactionsPagination page={1} pageSize={50} total={120} onPageChange={onPageChange} />,
    );
  ...
  it("steps back a page on Previous", async () => {
    const onPageChange = vi.fn();
    render(
      <TransactionsPagination page={3} pageSize={50} total={120} onPageChange={onPageChange} />,
    );
  ...
  it("disables Next on the last page and clamps the range to the total", () => {
    render(<TransactionsPagination page={3} pageSize={50} total={120} onPageChange={vi.fn()} />);
    expect(screen.getByLabelText("Pagination range")).toHaveTextContent(
  ...
  it("reports a single empty page rather than 'page 1 of 0'", () => {
    render(<TransactionsPagination page={1} pageSize={50} total={0} onPageChange={vi.fn()} />);
    expect(screen.getByLabelText("Pagination range")).toHaveTextContent("Page 1 of 1 · 0–0 of 0");
    expect(screen.getByRole("button", { name: /next/i })).toBeDisabled();
    expect(screen.getByRole("button", { name: /previous/i })).toBeDisabled();
  });
});
