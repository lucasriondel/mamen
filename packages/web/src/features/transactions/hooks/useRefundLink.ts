...
export const useRefundLink = (): UseRefundLinkReturn => {
  ...
  const handleUnlink = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to unlink refund'
      toast.error(message)
    } finally {
      setIsLinking(false)
    }
  ...
  const handleReplaceLink = useCallback(
    async (newPurchaseTransactionId: number, oldPurchaseTransactionId: number) => {
      if (!sourceTransaction?.id) return
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : 'Failed to replace refund link'
        toast.error(message)
      } finally {
        setIsLinking(false)
      }
