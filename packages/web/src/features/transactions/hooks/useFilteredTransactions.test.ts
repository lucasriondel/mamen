...
describe("useFilteredTransactions", () => {
	...
	it("excludes manually categorized transactions from unmatched filter", async () => {
		...
		await waitFor(() => {
			...
			expect(result.current.transactions[0].rawMerchantString).toBe(
				"UNMATCHED",
			);
		});
	});
	...
	it("combines month range and unmatched filters", async () => {
		...
		await db.transactions.bulkAdd([
			makeTransaction({
				...
				rawMerchantString: "JAN_UNMATCHED",
			}),
			makeTransaction({
				date: new Date(2026, 0, 15),
				rawMerchantString: "JAN_MATCHED",
				merchantId: 1,
			}),
			makeTransaction({
				date: new Date(2026, 1, 5),
				rawMerchantString: "FEB_UNMATCHED",
			}),
		]);
	...
	it("includes transactions on month boundaries", async () => {
		...
		await db.transactions.bulkAdd([
			makeTransaction({
				...
				rawMerchantString: "FIRST_DAY",
			}),
			makeTransaction({
				date: new Date(2026, 0, 31, 23, 59, 59, 999),
				rawMerchantString: "LAST_DAY",
			}),
		]);
