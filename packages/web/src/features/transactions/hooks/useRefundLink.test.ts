...
describe("useRefundLink", () => {
	...
	it("opens in linked view when source has linkedRefundId", () => {
		...
		act(() => {
			result.current.openRefundLink(
				makeTransaction({ id: 1, linkedRefundId: 2 }),
			);
		});
	...
	it("handleUnlink calls unlinkRefund and closes modal", async () => {
		...
		act(() => {
			result.current.openRefundLink(
				makeTransaction({ id: refundId, linkedRefundId: purchaseId }),
			);
		});
	...
	it("handleReplaceLink calls replaceLinkRefund and closes modal", async () => {
		...
		act(() => {
			result.current.openRefundLink(
				makeTransaction({ id: refundId, linkedRefundId: oldPurchaseId }),
			);
		});
