...
function TestComponent({ onResult }: { onResult: (result: ReturnType<typeof useDrillDownFilter>) => void }): React.ReactElement {
  const result = useDrillDownFilter()
  onResult(result)
  return createElement('div', { 'data-testid': 'test-component' }, 'rendered')
}

function renderWithSearch(initialSearch = '', onResult: (result: ReturnType<typeof useDrillDownFilter>) => void): ReturnType<typeof render> {
  ...
  const transactionsRoute = createRoute({
    ...
    component: () =>
      createElement(FocusModeProvider, null,
...
describe('useDrillDownFilter', () => {
  it('returns no active filter when no search params', async () => {
    ...
    renderWithSearch('', (result) => { captured = result })
  ...
  it('parses categoryId from search params', async () => {
    let captured: ReturnType<typeof useDrillDownFilter> | null = null
    renderWithSearch('?categoryId=5', (result) => { captured = result })
  ...
  it('sets fromDashboard true when from=dashboard', async () => {
    let captured: ReturnType<typeof useDrillDownFilter> | null = null
    renderWithSearch('?categoryId=1&from=dashboard', (result) => { captured = result })
  ...
  it('handles invalid date params gracefully', async () => {
    let captured: ReturnType<typeof useDrillDownFilter> | null = null
    renderWithSearch('?categoryId=1&periodStart=invalid&periodEnd=also-invalid', (result) => { captured = result })
