...
describe("SubscriptionsView Integration", () => {
	...
	it("shows empty state when no subscriptions", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("No subscriptions detected yet"),
			).toBeInTheDocument();
		});
	...
	it("expands detail on row click showing charge history", async () => {
		...
		const txId1 = (await db.transactions.add(
			makeTx({ date: "2026-01-18", amount: -15.99 }) as Transaction,
		)) as number;
		const txId2 = (await db.transactions.add(
			makeTx({ date: "2025-12-18", amount: -15.99 }) as Transaction,
		)) as number;
	...
	it("navigates to merchant detail from expanded subscription", async () => {
		...
		await db.subscriptions.add(makeSub({ merchantId: 42 }) as Subscription);
		...
		await waitFor(() => {
			expect(
				screen.getByRole("button", { name: /View Merchant/i }),
			).toBeInTheDocument();
		});
	...
	it("filter by status shows only matching", async () => {
		...
		await user.click(
			screen.getByRole("option", { name: /Possibly cancelled/i }),
		);
