import { merchantsApi, subscriptionsApi, transactionsApi } from "@/lib/api";
import type { Subscription, SubscriptionFrequency, Transaction } from "@/types";
...
type DetectedSubscription = Omit<
	Subscription,
	"id" | "detectedAt" | "updatedAt" | "status"
>;

export const detectSubscriptions = async (): Promise<
...
> => {
	...
	for (const merchant of merchants) {
		...
		const expenses = transactions.filter((tx) => !tx.isRefund && tx.amount < 0);
...
export const runDetection = async (): Promise<{
	...
	for (const sub of detected) {
		...
		try {
			existing = await subscriptionsApi.getByMerchantAndFrequency(
				sub.merchantId,
				sub.frequency,
			);
		} catch {
