 ...
 */
export function useRecapSpend(period: Period, accountIds: readonly number[]): RecapSpendResult {
  ...
  const spend = useMemo(
    () =>
      summary === undefined ? NO_SPEND : toSpendRows(summary, { issuersById, categoriesById }),
  ...
  const isPending =
    accountsQuery.isPending || issuersPending || categoriesQuery.isPending || recapQuery.isPending;
  const isError =
    accountsQuery.isError || issuersError || categoriesQuery.isError || recapQuery.isError;
