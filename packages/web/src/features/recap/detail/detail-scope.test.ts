...
describe("toDetailScope", () => {
  it("scopes an issuer bucket over the period's inclusive date bounds", () => {
    ...
    expect((scope.startDate as Date).toISOString()).toBe("2026-07-01T00:00:00.000Z");
    expect((scope.endDate as Date).toISOString()).toBe("2026-07-31T23:59:59.999Z");
  });
  ...
  it("asks for the unassigned rows rather than dropping the filter", () => {
    expect(
      toDetailScope({ kind: "bucket", axis: "issuer", bucket: null }, { kind: "all" }, []).issuerId,
    ).toBe("none");
    expect(
      toDetailScope({ kind: "bucket", axis: "category", bucket: null }, { kind: "all" }, [])
        .categoryId,
    ).toBe("none");
  });
  ...
  it("carries a multi-account selection across as a set", () => {
    expect(
      toDetailScope({ kind: "bucket", axis: "issuer", bucket: 10 }, { kind: "all" }, [1, 2])
        .accountId,
    ).toEqual([1, 2]);
  });
  ...
  it("omits the account filter for an empty selection", () => {
    expect(
      toDetailScope({ kind: "bucket", axis: "issuer", bucket: 10 }, { kind: "all" }, []).accountId,
    ).toBeUndefined();
  });
  ...
  it("leaves the recap-exclusion filter to the URL, not the scope", () => {
    expect(
      toDetailScope({ kind: "bucket", axis: "issuer", bucket: 10 }, { kind: "all" }, [])
        .excludedFromRecap,
    ).toBeUndefined();
  });
  ...
  describe("the excluded target (issue #87)", () => {
    it("contributes no issuer or category filter", () => {
      const scope = toDetailScope({ kind: "excluded" }, { kind: "month", month: "2026-07" }, []);
      expect(scope.issuerId).toBeUndefined();
      expect(scope.categoryId).toBeUndefined();
