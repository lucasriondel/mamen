...
describe("validateRecapDetailSearch", () => {
  ...
  it("keeps the Unassigned sentinel as its own value, distinct from absent", () => {
    expect(validateRecapDetailSearch({ by: "category", bucket: "none" }).bucket).toBe("none");
    expect(validateRecapDetailSearch({ by: "category" }).bucket).toBeUndefined();
  });
  ...
  it("carries the account selection as an id list, single value or repeated", () => {
    expect(validateRecapDetailSearch({ accountIds: "2" }).accountIds).toEqual([2]);
    expect(validateRecapDetailSearch({ accountIds: ["1", "2"] }).accountIds).toEqual([1, 2]);
