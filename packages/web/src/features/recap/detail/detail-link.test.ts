...
describe("toDetailSearch", () => {
  it("carries the axis, the bucket, the period and the accounts", () => {
    const search = toDetailSearch("issuer", 10, { kind: "month", month: "2026-07" }, [1, 2]);
  ...
  it("sends the Unassigned bucket as the sentinel a URL can carry", () => {
    expect(toDetailSearch("category", null, { kind: "all" }, []).bucket).toBe("none");
  });

  it("omits the month/year the period does not use, and an empty selection", () => {
    const search = toDetailSearch("issuer", 10, { kind: "year", year: "2026" }, []);
  ...
  it("seeds the counted-only recap filter", () => {
    expect(toDetailSearch("issuer", 10, { kind: "all" }, []).excludedFromRecap).toBe(false);
  });
});
