import type { Issuer, Rule, RulePreviewInput, RuleView } from "@mamen/shared/contract";
...
export function RuleMovePanel({ rule, onDone, onCancel }: RuleMovePanelProps) {
  ...
  const previewInput: RulePreviewInput = {
    ...
    ...(rule.matchAccountId != null ? { matchAccountId: rule.matchAccountId } : {}),
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {target !== null ? (
        <>
          <p className="text-sm text-gousse-ink">
            Moving to <strong className="font-medium">{target.name}</strong>. Because category is
            derived through the issuer, these transactions may change category too.
          </p>
          ...
          {duplicate ? (
            ...
            >
              {target.name} already has a rule on <code className="font-mono">{rule.pattern}</code>.
              Moving this one leaves two rules on the same pattern — one of them will win every row
              and the other will read “0 transactions”. If {target.name} already handles this
              pattern, deleting this rule is likelier what you mean.
            </p>
          ) : null}
          ...
          <div className="max-h-72 overflow-y-auto rounded-2xl border border-gousse-line p-3">
            ...
            {previewQuery.isPending || issuersPending ? (
              ...
              <p className="text-sm text-gousse-high">Couldn’t load the move preview.</p>
            ) : preview ? (
              <div className="flex flex-col gap-4">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button size="sm" onClick={move} disabled={target === null || update.isPending}>
          Move rule
        </Button>
      </div>
    </div>
