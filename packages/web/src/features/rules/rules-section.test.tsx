...
describe("RulesSection — inline delete confirm", () => {
	...
	it("closes an open move panel when the delete confirm opens", async () => {
		...
		await user.click(
			screen.getByRole("button", { name: /Delete rule amazon/ }),
		);
...
describe("RulesSection — inline move panel", () => {
	...
	it("expands the panel in place — no modal, no route change", async () => {
		...
		expect(
			screen.getByRole("link", { name: /Edit rule amazon/ }),
		).toBeInTheDocument();
	});
	...
	it("previews the rule against the picked issuer, carrying its matchers", async () => {
		...
		expect(screen.getByText("AMZN MKTP")).toBeInTheDocument();
		expect(
			screen.getByRole("heading", { name: /Manual collisions \(1\)/ }),
		).toBeInTheDocument();
		...
		expect(
			screen.queryByRole("button", { name: /Remove manual issuer/ }),
		).not.toBeInTheDocument();
	});
	...
	it("shows the rule preview skeleton while the dry-run is in flight", async () => {
		...
		expect(
			await screen.findByText("Previewing this move…"),
		).toBeInTheDocument();
	});
	...
	it("moves with a patch carrying only the new issuer, then closes and toasts", async () => {
		...
		await waitFor(() =>
			expect(updateRule).toHaveBeenCalledWith(10, { issuerId: 2 }),
		);
		await waitFor(() =>
			expect(toastSuccess).toHaveBeenCalledWith(expect.stringContaining("AWS")),
		);
		...
		await waitFor(() =>
			expect(
				screen.queryByRole("link", { name: /Edit rule amazon/ }),
			).not.toBeInTheDocument(),
		);
	});
	...
	it("keeps the panel open and stays silent on success when the move fails", async () => {
		...
		expect(
			screen.getByRole("button", { name: "Move rule" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("link", { name: /Edit rule amazon/ }),
		).toBeInTheDocument();
