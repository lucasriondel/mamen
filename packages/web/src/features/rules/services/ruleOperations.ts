...
import { applyMatchResults, applyRulesToTransactions } from "./rulesEngine";
...
export const updateRuleWithReeval = async (
	...
	const oldMatches = allTransactions
		.filter(
			(tx) =>
				oldRegex.test(tx.rawMerchantString) &&
				tx.merchantId === rule.merchantId,
		)
		.map((tx) => tx.id!);
...
export const deleteRuleWithCleanup = async (
	...
	const matchedTxIds = allTransactions
		.filter(
			(tx) =>
				regex.test(tx.rawMerchantString) && tx.merchantId === rule.merchantId,
		)
		.map((tx) => tx.id!);
...
export const restoreDeletedRule = async (deletedRule: Rule): Promise<void> => {
	...
	const finalMatches = finalTransactions.filter(
		(tx) =>
			regex.test(tx.rawMerchantString) &&
			tx.merchantId === deletedRule.merchantId,
	).length;
