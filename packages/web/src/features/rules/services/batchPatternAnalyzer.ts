...
export const analyzeBatchPatterns = async (
	...
	const prefixes = rawStrings
		.map((s) => extractPrefix(s))
		.filter(Boolean) as string[];

	// 2. Find longest common prefix among extracted prefixes
	const commonPrefix =
		prefixes.length > 0 ? findLongestCommonPrefix(prefixes) : "";
...
export const getMatchingTransactionsOutsideSelection = async (
	...
	{ rawMerchantString: string; date: Date; amount: number; id?: number }[]
> => {
	try {
		...
		const matches = allTransactions
			.filter(
				(tx) =>
					...
					!selectedSet.has(tx.rawMerchantString),
			)
			.slice(0, 10);
