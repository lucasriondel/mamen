...
type RuleEditModalProps = {
  ...
  onSave: (ruleId: number, updates: { pattern: string; categoryOverride?: number }) => Promise<void>
}
...
export function RuleEditModal({
  ...
  const [categoryOverride, setCategoryOverride] = useState<number | undefined>(undefined)
  ...
  const selectedCategory = categoryOverride !== undefined ? getCategoryById(categoryOverride) : undefined
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          {merchant && (
            <p className="text-sm text-muted-foreground">Merchant: {merchant.name}</p>
          )}
        </DialogHeader>
