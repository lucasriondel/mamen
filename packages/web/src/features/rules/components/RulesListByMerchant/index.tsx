    ...
    return rulesWithMerchants
      .map((group) => {
        ...
        const matchingRules = group.rules.filter((rule) => {
          ...
          const category = rule.categoryOverride !== undefined
            ? getCategoryById(rule.categoryOverride)
            : getCategoryById(group.merchant.defaultCategoryId ?? 0)
          const categoryMatch = category?.name.toLowerCase().includes(query) ?? false
  ...
  if (rulesWithMerchants.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p className="text-sm mt-1">Create rules by pressing R on transactions</p>
      </div>
    )
  }
      ...
      {filteredGroups.map((group) => {
        ...
        return (
          <div key={group.merchant.id} data-testid="merchant-group">
            ...
            >
              ...
              {merchantCategory && (
                <Badge
                  ...
                  style={{ backgroundColor: `${merchantCategory.color}20`, color: merchantCategory.color }}
                >
            ...
            {!isCollapsed && (
              <div>
                {group.rules.map((rule, ruleIndex) => {
                  ...
                  const ruleCategory = rule.categoryOverride !== undefined
                    ? getCategoryById(rule.categoryOverride)
                    : undefined
            ...
            {(() => { globalIndex = startIndex + group.rules.length; return null })()}
          </div>
        )
      })}
    </div>
