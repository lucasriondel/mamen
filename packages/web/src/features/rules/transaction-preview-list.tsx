...
function TransactionRow({
  ...
  const currentIssuer =
    transaction.issuerId != null ? issuersById.get(transaction.issuerId) : undefined;
  ...
  return (
    <li className="flex items-center gap-2 py-1.5 text-sm">
      ...
      <span className="min-w-0 flex-1 truncate text-gousse-ink">{transaction.rawIssuerString}</span>
      {currentIssuer ? (
  ...
  return (
    <section className="flex flex-col gap-1">
      <h4 className="text-balance text-sm font-medium text-gousse-ink">
        {title} <span className="font-normal text-gousse-muted">({transactions.length})</span>
      </h4>
