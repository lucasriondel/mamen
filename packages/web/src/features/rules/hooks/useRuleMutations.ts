...
export const useRuleMutations = (): UseRuleMutationsReturn => {
	...
	const updateRule = useCallback(
		async (ruleId: number, updates: RuleUpdates): Promise<void> => {
			...
			try {
				...
				toast(
					`Rule updated. ${result.updatedTransactionCount} transactions matched`,
					{
						action: {
							label: "Undo",
							onClick: () => {
								undoRuleUpdate(ruleId, result.previousRule)
									.then(() => {
										invalidateEntity("rules", "transactions", "merchants");
									})
									.catch(() => {
				...
				const e =
					err instanceof Error ? err : new Error("Failed to update rule");
				setError(e);
	...
	const deleteRule = useCallback(async (ruleId: number): Promise<void> => {
		...
		queryClient
			.getQueriesData<Rule[]>({ queryKey: queryKeys.rules.all })
			.forEach(([key, data]) => {
		...
		try {
			...
			toast(
				`Rule deleted. ${result.affectedTransactionCount} transactions unmatched`,
				{
					action: {
						label: "Undo",
						onClick: () => {
							restoreDeletedRule(result.deletedRule)
								.then(() => {
									invalidateEntity("rules", "transactions", "merchants");
								})
								.catch(() => {
