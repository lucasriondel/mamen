...
vi.mock("../../services/anomalyDetector", async (importOriginal) => {
	...
	return {
		...original,
		detectHighAmountAnomalies: vi
			.fn()
			.mockResolvedValue({ flagged: 0, skippedCategories: 0 }),
	};
});
...
describe("AnomalySettingsForm", () => {
	it("loads and displays default values when no settings saved", async () => {
		...
		expect(screen.getByLabelText("Absolute threshold (optional)")).toHaveValue(
			null,
		);
		expect(
			screen.getByLabelText("Min. transactions for detection"),
		).toHaveValue(5);
	});
