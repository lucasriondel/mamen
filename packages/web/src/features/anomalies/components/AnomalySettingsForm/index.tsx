...
export function AnomalySettingsForm(): React.ReactElement {
	...
	const { data: settingsResult, isLoading } = useQuery({
		...
		queryFn: () =>
			settingsApi
				.getByKey("anomaly_settings" as any)
				.then((s) => [s])
				.catch(() => []),
	});
	...
	useEffect(() => {
		if (!isLoading && !initialized) {
			...
			setAbsolute(
				currentSettings.absoluteThreshold != null
					? String(currentSettings.absoluteThreshold)
					: "",
			);
	...
	const handleSave = useCallback(async () => {
		...
		const existing = await settingsApi
			.getByKey("anomaly_settings" as any)
			.catch(() => undefined);
		if (existing) {
		...
		detectHighAmountAnomalies().then((result) => {
			...
			if (result.flagged > 0) {
				toast.info(
					`${result.flagged} transaction(s) flagged with new thresholds`,
				);
			}
		});
	...
	return (
		<Card>
			...
			<CardContent className="space-y-4">
				...
				<div className="space-y-2">
					<Label htmlFor="absolute-threshold">
				...
				<div className="space-y-2">
					<Label htmlFor="min-transactions">
						Min. transactions for detection
					</Label>
					<Input
						id="min-transactions"
						type="number"
