...
describe("TransfersView", () => {
	...
	it("confirms the chosen pair and nothing else", async () => {
		...
		await waitFor(() =>
			expect(linkTransferMock).toHaveBeenCalledWith([DEBIT.id, CREDIT_FAR.id]),
		);
	});
	...
	it("drops a leg once it is dismissed", async () => {
		...
		await waitFor(() =>
			expect(screen.queryByText("VIR SEPA VERS LIVRET A")).toBeNull(),
		);
	...
	it("says so when nothing is outstanding", async () => {
		...
		expect(
			await screen.findByText("No transfers detected"),
		).toBeInTheDocument();
	});
});
