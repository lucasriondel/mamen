...
describe("AccountsView", () => {
	it("gives each account a card carrying its own month strip", async () => {
		...
		expect(
			screen.getByRole("group", { name: "Rainy day — 2026" }),
		).toBeInTheDocument();
	});
	...
	it("pages the year over every strip at once", async () => {
		...
		expect(
			await screen.findByRole("group", { name: "Everyday — 2024" }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("group", { name: "Rainy day — 2024" }),
		).toBeInTheDocument();
		// 2024 is fully elapsed, so March is live on both strips — and marked only
		// on the account that has it.
		expect(
			screen.getByRole("button", { name: /Mar 2024 — already imported/ }),
		).toBeInTheDocument();
	});
	...
	it("keeps listing accounts when the import history fails to load", async () => {
		...
		).toHaveTextContent(/Couldn't load your import history/);
