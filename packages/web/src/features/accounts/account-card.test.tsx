...
function renderCard({
	...
	const cells = monthCells(year, CURRENT_MONTH, (month) =>
		imported.includes(month),
	);
...
describe("AccountCard", () => {
	it("names the account, its type and its own month strip", async () => {
		...
		expect(
			screen.getByRole("group", { name: "Everyday — 2026" }),
		).toBeInTheDocument();
	});
	...
	it("keeps rename and delete behind the ··· menu", async () => {
		...
		expect(
			screen.queryByRole("menuitem", { name: /Rename Everyday/ }),
		).not.toBeInTheDocument();

		await openMenu();

		expect(
			await screen.findByRole("menuitem", { name: /Rename Everyday/ }),
		).toBeInTheDocument();
		expect(
			screen.getByRole("menuitem", { name: /Delete Everyday/ }),
		).toBeInTheDocument();
	});
	...
	it("renames through the update mutation", async () => {
		...
		await waitFor(() =>
			expect(updateAccount).toHaveBeenCalledWith(1, { name: "Holiday fund" }),
		);
	});
	...
	it("deletes an account with no transactions", async () => {
		...
		await user.click(
			await screen.findByRole("menuitem", { name: /Delete Everyday/ }),
		);
