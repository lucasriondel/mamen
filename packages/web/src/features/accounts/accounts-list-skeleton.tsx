    ...
    <SkeletonScreen label="Loading accounts…" className="flex flex-col gap-3">
      {ROW_WIDTHS.map((width, index) => (
        ...
        >
          ...
          {/* The month strip: twelve identical blocks, so the position is the
              only identity there is — the same kind of key the outer row uses,
              and harmless to repeat since the two live in different arrays. */}
