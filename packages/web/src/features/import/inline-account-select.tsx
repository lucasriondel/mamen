...
import { ACCOUNT_TYPE_OPTIONS, type AccountType } from "@/features/accounts/account-type";
  ...
  return (
    <div className="flex flex-col gap-3">
      <label className="flex flex-col gap-1 text-sm text-gousse-muted">
        ...
        <Select
          value={value ?? ""}
          onChange={(event) => onChange(Number(event.target.value) as AccountId)}
      ...
      {creating ? (
        ...
        >
          ...
          <Button variant="secondary" size="md" onClick={() => setCreating(false)}>
            Cancel
          </Button>
        </form>
      ) : (
