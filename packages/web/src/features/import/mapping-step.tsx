 ...
 * distinctions: the account has no PDF format at all, several are saved and
 * none was chosen, or the one that was chosen reported a **format verdict**
 * mismatch — the bank changed its export. Written apart from the CSV three
 * rather than folded into them, because each says something the CSV wording
 * does not: what is below is a *transcription* on all three, a PDF's several is
 * an unanswered question rather than a double match, and nothing on the CSV
 * path ever gets as far as a mismatch.
 *
 * The `reason` is never a detection verdict here — there is no header row to
 * fingerprint — it is which of the three screens the discovery run was spent
 * from, carried by `discover-start`.
 */
function reasonCopy(
