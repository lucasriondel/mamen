...
export function ImportCSVModal({
	...
	useEffect(() => {
		...
		parseCSVPreview(file)
			.then((result) => {
				...
				if (detected.descriptionColumns)
					setDescriptionColumns(detected.descriptionColumns);
				if (detected.directionColumn)
					setDirectionColumn(detected.directionColumn);
	...
	const handleImport = async (): Promise<void> => {
		...
		try {
			...
			const parsed = await parseCSVTransactions(
				file,
				preview.hasHeaders,
				mapping,
				dateFormat,
	...
	const handleSkipDuplicates = async (): Promise<void> => {
		...
		try {
			setImportPhase("applying-rules");
			const result = await importWithRules(
			...
			showImportToast(
				result,
				`, ${duplicateResult.duplicates.length} duplicates skipped`,
			);
		} catch (err) {
			setError(err instanceof Error ? err.message : "Import failed");
		} finally {
	...
	return (
		<>
						...
						<>
							...
							<div className="space-y-3">
								...
								<div className="grid grid-cols-2 gap-3">
										...
										<div className="space-y-1.5">
											{descriptionColumns.map((col, idx) => (
												...
												<div key={idx} className="flex items-center gap-1">
													...
													>
														<SelectTrigger
															id={idx === 0 ? "description-column" : undefined}
														>
															<SelectValue placeholder="Select column" />
														</SelectTrigger>
														<SelectContent>
									...
									<div className="space-y-1.5">
										<Label htmlFor="direction-column" className="text-xs">
											Direction (optional)
										</Label>
										<Select
											value={directionColumn}
											onValueChange={setDirectionColumn}
										>
							...
							<div className="flex items-center gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-md">
								...
								<div>
									...
									<p className="text-xs text-muted-foreground">
										Duplicates are identified by matching account, date, amount,
										and description
									</p>
									{duplicateResult.allDuplicates && (
														...
														<TableCell>
															{dup && (
																<Tooltip>
																	...
																	<TooltipContent>
																		<p>
																			Matches existing transaction from{" "}
			...
			<AlertDialog
				open={showImportAnywayConfirm}
				onOpenChange={setShowImportAnywayConfirm}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
