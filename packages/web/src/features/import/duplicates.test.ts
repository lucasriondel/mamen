...
import { type DuplicateCandidate, flagDuplicates, normaliseIssuerString } from "./duplicates";
...
describe("flagDuplicates", () => {
  ...
  it("matches across issuer whitespace and letter case", () => {
    expect(flagDuplicates([row({ rawIssuerString: "  shop   a " })], [row()])).toEqual([true]);
  });
  ...
  it("does not match on a different date or a different amount", () => {
    const stored = [row()];
    expect(flagDuplicates([row({ date: new Date("2026-01-16T10:00:00Z") })], stored)).toEqual([
      false,
    ]);
  ...
  it("matches an amount that only differs by float noise", () => {
    expect(flagDuplicates([row({ amount: 0.1 + 0.2 })], [row({ amount: 0.3 })])).toEqual([true]);
  });
  ...
  it("does not match a stored row on another account", () => {
    expect(flagDuplicates([row()], [row({ accountId: OTHER_ACCOUNT_ID })])).toEqual([false]);
  });
