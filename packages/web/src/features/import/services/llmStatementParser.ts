...
export const parseStatementWithLLM = async (
  ...
  const prompt = BANK_STATEMENT_PARSING_PROMPT.replace('{statement_text}', text)
  ...
  try {
    ...
    if (!jsonMatch) {
      return {
        success: false,
        error: 'Could not extract transactions from this PDF. The format may not be supported.',
        errorType: 'parse',
      }
    }
    ...
    if (!validation.success) {
      return {
        success: false,
        error: 'LLM returned unexpected data format. Try again or use CSV import.',
        errorType: 'invalid-response',
      }
    }
    ...
    if (validation.data.length === 0) {
      return {
        success: false,
        error: 'No transactions found in the PDF. The format may not be supported.',
        errorType: 'parse',
      }
    }
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      return {
        success: false,
        error: 'PDF parsing timed out. Try a shorter statement or different LLM.',
        errorType: 'timeout',
      }
    }
