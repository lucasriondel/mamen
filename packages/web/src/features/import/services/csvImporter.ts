...
export const parseCSVTransactions = async (
	...
	const descIndices = mapping.descriptionColumns.map((col) =>
		headerRow.indexOf(col),
	);
	const dirIdx = mapping.directionColumn
		? headerRow.indexOf(mapping.directionColumn)
		: -1;
...
export const importTransactions = async (
	...
	const transactions: Omit<Transaction, "id">[] = parsedTransactions.map(
		(t) => ({
			accountId,
			date: t.date,
			amount: t.amount,
