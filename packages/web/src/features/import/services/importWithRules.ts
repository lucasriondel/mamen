...
export const importWithRules = async (
	...
	Promise.all([
		...
		const totalFlagged =
			highAmountResult.flagged +
			newMerchantResult.flagged +
			duplicateResult.flagged;
		if (totalFlagged === 0) return;

		const parts: string[] = [];
		if (highAmountResult.flagged > 0)
			parts.push(`${highAmountResult.flagged} high amounts`);
		if (newMerchantResult.flagged > 0)
			parts.push(`${newMerchantResult.flagged} new merchants`);
		if (duplicateResult.pairs > 0)
			parts.push(`${duplicateResult.pairs} potential duplicate pairs`);
