 ...
 *
 * `counterpartyIban` is the fourth target the PRD names, and a real transaction
 * column since PRD #175 promoted it. It is the one target a format may decline:
 * only banks that write the other side's account number have a column to name
 * for it, so {@link ColumnMapping} lets it be `null`.
 */
export type MappedTarget = "date" | "amount" | "rawIssuerString" | "counterpartyIban";
 ...
 *
 * Written against {@link MappedTarget} so that promoting a fifth target makes
 * every format record fail to type-check until it says what fills it.
 *
 * `counterpartyIban` is `string | null` rather than optional, for that same
 * reason: a bank that writes no counterparty account number has to *say* so, or
 * "this export carries none" and "nobody got round to it" would look alike in
 * the record — and, once these are user-authored rows, in the mapping UI too.
 */
export type ColumnMapping = {
  readonly [Target in Exclude<MappedTarget, "amount" | "counterpartyIban">]: string;
} & {
