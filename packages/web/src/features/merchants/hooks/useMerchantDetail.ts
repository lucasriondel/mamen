...
import type { Merchant, Rule, Transaction } from "@/types";
...
const computeStats = (allTransactions: Transaction[]): MerchantStats => {
	...
	const distinctMonths = new Set(
		expenses.map((tx) => `${tx.date.getFullYear()}-${tx.date.getMonth()}`),
	);
	...
	return {
		...
		monthlyAverage:
			distinctMonths.size > 0 ? totalSpent / distinctMonths.size : 0,
		firstSeen: new Date(
...
const computeCategoryDistribution = (
	...
	return Array.from(catCounts.entries()).map(([catId, count]) => ({
		categoryId: catId,
		categoryLabel:
			catId != null ? (categoryMap.get(catId) ?? "Unknown") : "Uncategorized",
		count,
	}));
};
...
export const useMerchantDetail = (
	...
	const allTransactions = (data as { allTransactions: Transaction[] })
		.allTransactions;
