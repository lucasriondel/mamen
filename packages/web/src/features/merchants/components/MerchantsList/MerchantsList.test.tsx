...
describe("MerchantsList", () => {
	...
	it("sorts merchants when sort option changes", async () => {
		...
		const sortedRows = screen.getAllByRole("button", {
			name: /View .* details/,
		});
	...
	it("shows no-results state when search has no matches", async () => {
		...
		expect(
			await screen.findByText(/No merchants matching/),
		).toBeInTheDocument();
	});
	...
	it("filters to show only new merchants when toggle active", async () => {
		...
		expect(
			screen.getByText(/1 of 2 merchants \(new only\)/),
		).toBeInTheDocument();
	});
