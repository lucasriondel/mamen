...
export function TaskSettingsCard({
  ...
  const patch = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: aiTaskKeys.all }),
  });
  ...
  const canRun = (provider: AiProvider) => !isHostedProvider(provider) || configured.has(provider);
  ...
  return (
    <SettingsCard>
      {AI_TASKS.map((task) => {
        ...
        const error =
          patch.error && patch.variables?.task === task ? toErrorMessage(patch.error) : null;
        ...
        return (
          ...
          >
            {isHostedProvider(setting.provider) ? (
              <p className="mt-2 text-xs text-gousse-ink">{hostedVendorNotice(setting.provider)}</p>
            ) : null}
          </ModelRow>
        );
      })}
