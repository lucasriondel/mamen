import type { AiProvider, AiTaskSetting, SecretStatus } from "@mamen/shared/contract";
...
function status(name: AiProvider, configured: boolean, hint: string | null = null): SecretStatus {
...
const allUnset = () =>
  NOTHING_STORED.map(([name, configured, hint]) => status(name, configured, hint));
...
beforeEach(() => {
  ...
  putSecret.mockReset().mockImplementation(async (name: AiProvider) => {
    secretStatuses = secretStatuses.map((entry) =>
      entry.name === name ? status(name, true, "sk-ant-…3f9") : (entry as SecretStatus),
    );
...
describe("the credentials grid", () => {
  it("renders every provider as a tile with its label, mark and stored state", async () => {
    ...
    await waitFor(() => expect(within(tile("Anthropic")).getByText("Stored")).toBeInTheDocument());
  ...
  it("stores a pasted credential and swaps the field for its masked hint", async () => {
    ...
    await user.click(within(tile("Anthropic")).getByRole("button", { name: "Save" }));

    await waitFor(() =>
      expect(putSecret).toHaveBeenCalledWith("anthropic", "sk-ant-api03-secret-value-3f9"),
    );
    await waitFor(() =>
      expect(within(tile("Anthropic")).getByText("sk-ant-…3f9")).toBeInTheDocument(),
    );
  });
  ...
  it("stores the Claude Code token like any other credential", async () => {
    ...
    await user.click(within(tile("Claude Code")).getByRole("button", { name: "Save" }));

    await waitFor(() =>
      expect(putSecret).toHaveBeenCalledWith("claude-code", "sk-ant-oat01-3fQ2xLmPqR7v-KjnW8sd"),
    );
    await waitFor(() =>
      expect(within(tile("Claude Code")).getByText("Stored")).toBeInTheDocument(),
    );
  });
  ...
  it("returns a cleared tile to its unset state", async () => {
    ...
    await user.click(await within(tile("Anthropic")).findByRole("button", { name: "Clear" }));

    await waitFor(() => expect(clearSecret).toHaveBeenCalledWith("anthropic"));
    await waitFor(() => expect(within(tile("Anthropic")).getByText("Not set")).toBeInTheDocument());
  ...
  it("shows the deletion door's refusal on the tile it refused", async () => {
    ...
    expect(await within(tile("Anthropic")).findByRole("alert")).toHaveTextContent(
      /still using it|in use/i,
    );
...
describe("the task row", () => {
  it("shows the task's stored provider and model", async () => {
    ...
    expect(await screen.findByLabelText(`${ROW} provider`)).toHaveValue("claude-code");
    expect(screen.getByLabelText(`${ROW} model`)).toHaveValue("claude-haiku-4-5");
  });
  ...
  it("saves a provider change immediately and lands on the server's default model", async () => {
    ...
    await waitFor(() =>
      expect(patchTasks).toHaveBeenCalledWith([{ task: "extract-pdf", provider: "google" }]),
    );
  ...
  it("saves a model change immediately, without a submit button", async () => {
    ...
    await waitFor(() =>
      expect(patchTasks).toHaveBeenCalledWith([{ task: "extract-pdf", model: "claude-opus-5" }]),
    );
    expect(await screen.findByText("Saved")).toBeInTheDocument();
  });
  ...
  it("surfaces a refusal from the save-time doors and keeps the stored choice", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText(`${ROW} provider`)).toHaveValue("claude-code"),
    );
  });
  ...
  it("says the statement will be sent to a hosted vendor, and only then", async () => {
    ...
    expect(await screen.findByText(/will be sent to Anthropic/i)).toBeInTheDocument();
  });
});
