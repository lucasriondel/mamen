...
describe("IssuersView", () => {
  ...
  it("shows the issuer's default category in its own column", async () => {
    ...
    const row = await screen.findByRole("row", { name: /Spotify/ });
    await waitFor(() => expect(within(row).getByText("Music")).toBeInTheDocument());
  });
...
describe("IssuersView filtering", () => {
  ...
  it("narrows the table to name matches from the URL", async () => {
    ...
    await waitFor(async () => expect(await rowOrder(NAMES)).toEqual(["Netflix"]));
  });
  ...
  it("filters as the user types", async () => {
    ...
    await user.type(screen.getByRole("searchbox", { name: /Filter issuers/i }), "spot");
    await waitFor(async () => expect(await rowOrder(NAMES)).toEqual(["Spotify"]));
  });
