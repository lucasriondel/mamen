import type { CategoryId, IssuerCreate, IssuerId, IssuerUpdate } from "@mamen/shared/contract";
...
export function useIssuerMutations() {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: issuerKeys.all });
  ...
  const setDefaultCategory = useMutation({
    mutationFn: ({ id, categoryId }: { id: IssuerId; categoryId: CategoryId | null }) =>
