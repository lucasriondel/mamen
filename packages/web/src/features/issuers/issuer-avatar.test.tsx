...
describe("IssuerAvatar", () => {
  it("paints the issuer's image when it has one", () => {
    ...
    expect(document.querySelector("img")).toHaveAttribute("src", "/uploads/issuers/7.webp");
  ...
  it("does not read the category tree when an image already wins the chain", async () => {
    render(<IssuerAvatar imageUrl="/uploads/issuers/7.webp" defaultCategoryId={2} />);
    await waitFor(() => expect(avatar()).toHaveAttribute("data-avatar", "image"));
  ...
  it("draws nothing while the category read is in flight, rather than flashing the grey ?", async () => {
    ...
    await waitFor(() => expect(avatar()).toHaveAttribute("data-avatar", "category"));
