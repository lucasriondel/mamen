...
export function CategoryLeafPicker({
  ...
  const hasExactLeaf = categories.some(
    (c) => isLeaf(categories, c) && c.name.trim().toLowerCase() === trimmedQuery.toLowerCase(),
  );
              ...
              {canCreate ? (
                <CommandGroup>
                  <CommandItem value="__create__" onSelect={startCreate}>
                    <Plus size={16} className="shrink-0 text-gousse-accent" aria-hidden />
                    <span className="truncate">Create category “{trimmedQuery}”</span>
                  </CommandItem>
                </CommandGroup>
              ) : null}
