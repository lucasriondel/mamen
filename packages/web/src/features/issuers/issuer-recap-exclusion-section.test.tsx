...
const { IssuerRecapExclusionSection } = await import("./issuer-recap-exclusion-section");
...
describe("IssuerRecapExclusionSection", () => {
  it("excludes every transaction of a counted issuer in one write", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /exclude from recap/i }));
  ...
  it("puts an excluded issuer's transactions back into the recap", async () => {
    render(<IssuerRecapExclusionSection issuer={issuer({ excludedFromRecap: true })} />);
    const user = userEvent.setup();
  ...
  it("says which state the issuer is in, and that per-row decisions survive", () => {
    ...
    expect(screen.getByText(/don't count toward your spend totals/i)).toBeVisible();
