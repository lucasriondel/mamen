...
describe("useIssuerSearch", () => {
  it("returns the issuers whose name matches the query", async () => {
    const { result } = renderHook(() => useIssuerSearch({ query: "mint", enabled: true }));

    await waitFor(() => expect(result.current).toHaveLength(2));
    expect(result.current.map((i) => i.name)).toEqual(["MINT ENERGIE", "Mintaka"]);
  });

  it("asks for nothing while the surface is closed", async () => {
    const { result } = renderHook(() => useIssuerSearch({ query: "mint", enabled: false }));
  ...
  it("sends one request per settled term, not one per keystroke", async () => {
    const { rerender } = renderHook(({ query }) => useIssuerSearch({ query, enabled: true }), {
      initialProps: { query: "" },
    });
