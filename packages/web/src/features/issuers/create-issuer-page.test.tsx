...
beforeEach(() => {
  createIssuer.mockReset().mockResolvedValue(issuer({ id: 42 as Issuer["id"] }));
  issuersList = [issuer()];
});
...
describe("CreateIssuerPage", () => {
  ...
  it("disables Save when the name duplicates an existing issuer (case-insensitive)", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "Create issuer" })).toBeDisabled(),
    );
  });
  ...
  it("marks the name field invalid while it duplicates, and clears it once it doesn't", async () => {
    ...
    expect(screen.getByText("An issuer with this name already exists.")).toBeInTheDocument();
    ...
    expect(screen.queryByText("An issuer with this name already exists.")).not.toBeInTheDocument();
  });
  ...
  it("describes why the name field is invalid, not just that it is", async () => {
    ...
    await waitFor(() =>
      expect(input).toHaveAccessibleDescription("An issuer with this name already exists."),
    );
  ...
  it("creates an issuer with name only, then lands on its detail page", async () => {
    ...
    expect(createIssuer.mock.calls[0][0]).not.toHaveProperty("defaultCategoryId");
