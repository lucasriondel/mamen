...
describe("LogoSearchPopover", () => {
  ...
  it("searches only on explicit submit — never on a keystroke", async () => {
    ...
    await waitFor(() => expect(searchLogos).toHaveBeenCalledWith("monoprix logo"));
  ...
  it("stores a picked result and closes the popover", async () => {
    ...
    await waitFor(() =>
      expect(setImageFromUrl).toHaveBeenCalledWith(1, "https://cdn.example.com/spotify.png"),
    );
    await waitFor(() =>
      expect(screen.queryByLabelText("Logo search query")).not.toBeInTheDocument(),
    );
  });

  it("explains the unconfigured state and names what to set", async () => {
    searchLogos.mockRejectedValue(tagged("LogoSearchUnconfigured", { missing: ["LOGODEV_TOKEN"] }));
    const user = userEvent.setup();
    render(<LogoSearchPopover issuer={issuer()} />);
    await user.click(await open(user));
  ...
  it("offers a retry for a transport failure — the one state where it can help", async () => {
    searchLogos.mockRejectedValue(tagged("LogoSearchFailed", { message: "upstream 502" }));
    const user = userEvent.setup();
    render(<LogoSearchPopover issuer={issuer()} />);
    await user.click(await open(user));

    expect(await screen.findByRole("alert")).toHaveTextContent(/couldn't reach logo search/i);
  ...
  it("leaves the results up when the download is refused, so another can be picked", async () => {
    setImageFromUrl.mockRejectedValue(tagged("ImageFetchRefused", { reason: "private-address" }));
    const user = userEvent.setup();
    render(<LogoSearchPopover issuer={issuer()} />);
    await user.click(await open(user));
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(/couldn't be fetched/i);
