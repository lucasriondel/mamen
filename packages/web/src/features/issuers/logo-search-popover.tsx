...
function SearchBody({
  ...
  if (results.length === 0) {
    return <p className="text-xs text-gousse-muted">No logos found. Try a different query.</p>;
  }
...
function SearchFailure({ error, onRetry }: { error: unknown; onRetry: () => void }) {
  ...
  if (failure.kind === "unconfigured") {
    ...
    return (
      <div role="alert" className="flex flex-col gap-1 text-xs">
        <p className="font-medium text-gousse-ink">Logo search isn't set up yet.</p>
        <p className="text-gousse-muted">
          The API is missing {missing.join(" and ")}. Set them and restart it — see{" "}
          <code>{SETUP_GUIDE}</code>. Uploading a file still works.
        </p>
      </div>
    );
  }
  ...
  if (failure.kind === "quota") {
    ...
    return (
      <div role="alert" className="flex flex-col gap-1 text-xs">
        ...
        <p className="text-gousse-muted">
          Logo search's provider is refusing further lookups for now. Wait a while and search again
          — or upload a file in the meantime.
        </p>
      </div>
    );
  }
