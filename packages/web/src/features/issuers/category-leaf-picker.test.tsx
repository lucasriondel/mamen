...
describe("CategoryLeafPicker — inline create", () => {
  it("offers a Create row when the search matches no leaf", async () => {
    ...
    expect(await screen.findByText(/Create category “Coffee gear”/)).toBeInTheDocument();
  });
  ...
  it("still offers a Create row when only a folder carries the name", async () => {
    ...
    expect(await screen.findByText(/Create category “Food”/)).toBeInTheDocument();
  });
  ...
  it("creates the category with the typed name, chosen parent and icon, then selects it", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Create category", hidden: true }));
  ...
  it("offers folders as parents but no nested create row", async () => {
    ...
    await user.type(screen.getByLabelText("Search parent categories"), "Nonexistent");
    await waitFor(() => expect(parentList).toHaveTextContent("No categories found."));
