...
export function CategoriesView() {
  const categoriesQuery = useQuery(categoryQueries.list({ limit: 200, orderBy: "sortOrder" }));
 ...
 */
function shareOfParent(node: CategoryTreeNode, depth: number, actions: NodeActions): number | null {
  ...
  return (
    <Dialog open={editor !== null} onOpenChange={(open) => !open && onClose()}>
      ...
      <DialogContent ref={setDialogNode}>
        {editor?.kind === "create" && (
          <NameForm
            title={editor.parent ? `New category in ${editor.parent.name}` : "New category"}
        ...
        {editor?.kind === "spill" && (
          <SpillForm
            ...
            onSubmit={(name) =>
              mutations.spill.mutate({ id: editor.node.id, name }, { onSuccess: onClose })
            }
          />
        )}
        {editor?.kind === "rename" && (
          <NameForm
            ...
            onSubmit={(name) =>
              mutations.rename.mutate({ id: editor.node.id, name }, { onSuccess: onClose })
            }
          />
        )}
        {editor?.kind === "move" && (
          <MoveForm
            ...
            onSubmit={(parentId) =>
              mutations.move.mutate({ id: editor.node.id, parentId }, { onSuccess: onClose })
            }
          />
        )}
      </DialogContent>
...
function NameForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <DialogFooter>
        <Button variant="primary" type="submit" disabled={pending || trimmed.length === 0}>
          {submitLabel}
        </Button>
      </DialogFooter>
    </form>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <p className="text-gousse-muted text-sm">
        {node.name} holds {describeDependents(transactions, issuers)}, so it can't take a child yet.
        Name a new category to move it into — {node.name} becomes a folder and nothing is stranded.
      </p>
      <label className="flex flex-col gap-1 text-gousse-muted text-sm">
        New category name
        <input
