...
import { NEUTRAL_CATEGORY_COLOR, resolveCategoryColors, searchAllNodes } from "@/lib/category-tree";
...
export function CategoryParentPicker({
  ...
  const current = value != null ? categories.find((c) => c.id === value) : undefined;
  ...
  const currentColor = current
    ? (resolveCategoryColors(categories).get(current.id) ?? NEUTRAL_CATEGORY_COLOR)
    : undefined;
  ...
  return (
    <div className="flex flex-col gap-1" ref={setAnchor}>
      ...
      <Popover open={open} onOpenChange={handleOpenChange}>
        <PopoverTrigger
          render={
            ...
            >
              {current ? (
                <>
                  <CategoryIcon name={current.icon} color={currentColor} size={14} />
                  <span style={{ color: currentColor }}>{current.name}</span>
                </>
              ) : (
                <>
                  <FolderTree size={14} className="shrink-0 text-gousse-muted" aria-hidden />
        ...
        />
        <PopoverContent className="p-0" portalContainer={portalContainer ?? anchor}>
          ...
          <Command shouldFilter={false} label="Choose a parent category">
            ...
            />
            <CommandList>
              {nodes.length === 0 ? <CommandEmpty>No categories found.</CommandEmpty> : null}
                ...
                <>
                  ...
                  <CommandGroup>
                    ...
                    >
                      ...
                      <span className="truncate">Move to top level (no parent)</span>
                    </CommandItem>
                  </CommandGroup>
                </>
              ) : null}
