...
function renderDialog() {
  render(<CategoryCreateDialog open onOpenChange={vi.fn()} initialName="Coffee gear" />);
...
describe("CategoryCreateDialog", () => {
  it("offers one appearance editor, not an icon picker and no colour", async () => {
    ...
    expect(dialog.queryByRole("button", { name: /change .* icon$/i })).not.toBeInTheDocument();
  });
  ...
  it("opens both halves inside the modal, so the grid is reachable", async () => {
    ...
    expect(dialog.getByRole("group", { name: /colour palette/i })).toBeInTheDocument();
  ...
  it("creates the category with the icon and colour chosen up front", async () => {
    ...
    await user.click(await dialog.findByRole("button", { name: "shopping-bag" }));
  ...
  it("saves the appearance without submitting the dialog", async () => {
    ...
    await waitFor(() => expect(dialog.queryByLabelText(/hex colour/i)).not.toBeInTheDocument());
