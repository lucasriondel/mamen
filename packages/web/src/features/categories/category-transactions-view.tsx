...
import { descendantIds, isFolder, resolveCategoryColor } from "@/lib/category-tree";
import { formatCurrency } from "@/lib/format";
import { categoryQueries, type TransactionCountParams, transactionQueries } from "@/lib/sdk";
import { cn, NO_ITEMS } from "@/lib/utils";
...
export function CategoryTransactionsView() {
  ...
  const { category, ids } = useMemo(() => resolveCategoryIds(categories, id), [categories, id]);
  const hasSet = ids.length > 0;

  const scope = useMemo<TransactionCountParams>(() => ({ categoryId: ids }), [ids]);
  ...
  const countFilters = useMemo(() => composeTransactionFilters(scope, search), [scope, search]);
...
function CategoriesBackLink() {
  return (
    <Link to="/categories" className="self-start text-gousse-muted text-sm hover:text-gousse-ink">
      ← Categories
    </Link>
  );
}
