...
describe('useNetSpending', () => {
  it('returns full refund pair as EUR0 net for category', async () => {
    const catId = await db.categories.add(makeCategory() as Category) as number
    ...
    const refundId = await db.transactions.add(
      makeTransaction({ amount: 100, categoryId: catId, isRefund: true, linkedRefundId: purchaseId }) as Transaction,
    ) as number
    ...
    await waitFor(() => {
      ...
      const cat = result.current!.categories.find(c => c.categoryId === catId)
  ...
  it('returns partial refund as EUR70 net', async () => {
    ...
    const refundId = await db.transactions.add(
      makeTransaction({ amount: 30, categoryId: catId, isRefund: true, linkedRefundId: purchaseId }) as Transaction,
    ) as number
  ...
  it('calculates multiple categories with refunds independently', async () => {
    const catA = await db.categories.add(makeCategory({ name: 'Shopping', slug: 'shopping' }) as Category) as number
    const catB = await db.categories.add(makeCategory({ name: 'Dining', slug: 'dining', color: '#F97316', sortOrder: 1 }) as Category) as number
    ...
    const refundA = await db.transactions.add(
      makeTransaction({ amount: 50, categoryId: catA, isRefund: true, linkedRefundId: purchaseA }) as Transaction,
    ) as number
    ...
    await waitFor(() => {
      expect(result.current).not.toBeNull()
      const shopping = result.current!.categories.find(c => c.categoryId === catA)
      const dining = result.current!.categories.find(c => c.categoryId === catB)
  ...
  it('returns all zeros for empty time period', async () => {
    const catId = await db.categories.add(makeCategory() as Category) as number
    await db.transactions.add(
      makeTransaction({ amount: -100, categoryId: catId, date: new Date(2026, 0, 15) }) as Transaction,
    )
  ...
  it('excludes transactions outside time period', async () => {
    ...
    await db.transactions.bulkAdd([
      makeTransaction({ amount: -100, categoryId: catId, date: new Date(2026, 0, 10) }) as Transaction,
      makeTransaction({ amount: -200, categoryId: catId, date: new Date(2026, 1, 5) }) as Transaction,
    ])
  ...
  it('counts linked refund with purchase outside time period in refund period', async () => {
    ...
    const purchaseId = await db.transactions.add(
      makeTransaction({ amount: -100, categoryId: catId, date: new Date(2025, 11, 15) }) as Transaction,
    ) as number

    // Refund in January (inside range)
    const refundId = await db.transactions.add(
      makeTransaction({ amount: 100, categoryId: catId, isRefund: true, linkedRefundId: purchaseId, date: new Date(2026, 0, 10) }) as Transaction,
    ) as number
