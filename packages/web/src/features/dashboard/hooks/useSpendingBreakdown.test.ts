...
describe("useSpendingBreakdown", () => {
	...
	it("correctly groups expenses by category", async () => {
		...
		const catId2 = await db.categories.add(
			makeCategory({
				name: "Dining",
				slug: "dining",
				color: "#F97316",
	...
	it("separates income from expenses", async () => {
		...
		const incomeCatId = await db.categories.add(
			makeCategory({
				name: "Income",
				slug: "income",
				color: "#10B981",
		...
		await waitFor(() => {
			...
			expect(result.current.items.every((item) => item.totalAmount < 0)).toBe(
				true,
			);
		});
	});
	...
	it("includes Uncategorized for transactions without category", async () => {
		...
		await waitFor(() => {
			expect(result.current.items).toHaveLength(2);
			const uncategorized = result.current.items.find(
	...
	it("sorts categories by amount descending (highest absolute first)", async () => {
		const catId1 = await db.categories.add(
			makeCategory({ name: "Shopping", slug: "shopping" }) as Category,
		);
		const catId2 = await db.categories.add(
			makeCategory({
		...
		const catId3 = await db.categories.add(
			makeCategory({
				name: "Transport",
				slug: "transport",
				color: "#06B6D4",
	...
	it("uses muted color for uncategorized items", async () => {
		await db.transactions.bulkAdd([makeTransaction({ amount: -50 })]);
	...
	it("rolls up subcategories into parent categories", async () => {
		...
		const subId1 = await db.categories.add(
			makeCategory({
				name: "Online",
		...
		const subId2 = await db.categories.add(
			makeCategory({
				name: "Groceries",
		...
		await db.transactions.bulkAdd([
			makeTransaction({
				amount: -100,
				categoryId: parentId,
				subcategoryId: subId1 as number,
			}),
			makeTransaction({
				amount: -50,
				categoryId: parentId,
				subcategoryId: subId2 as number,
			}),
		]);
