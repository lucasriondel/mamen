	...
	return useMemo(() => {
		...
		for (const tx of prevTransactions) {
			if (tx.isRefund && tx.linkedRefundId) {
				...
				prevRefundsByCategory.set(
					catId,
					(prevRefundsByCategory.get(catId) ?? 0) + tx.amount,
				);
				...
				const catId = tx.categoryId ?? null;
				prevGrossByCategory.set(
					catId,
					(prevGrossByCategory.get(catId) ?? 0) + Math.abs(tx.amount),
				);
			}
		...
		const allPrevCatIds = new Set([
			...prevGrossByCategory.keys(),
			...prevRefundsByCategory.keys(),
		]);
