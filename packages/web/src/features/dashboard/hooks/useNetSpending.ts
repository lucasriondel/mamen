...
export const useNetSpending = (
	...
	for (const tx of transactions) {
		...
		if (tx.isRefund && tx.linkedRefundId) {
			...
			linkedRefundsByCategory.set(
				catId,
				(linkedRefundsByCategory.get(catId) ?? 0) + refundAmount,
			);
	...
	const allCategoryIds = new Set([
		...grossByCategory.keys(),
		...linkedRefundsByCategory.keys(),
	]);
