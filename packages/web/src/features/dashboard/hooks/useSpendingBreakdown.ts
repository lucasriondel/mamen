...
import { categoriesApi, queryKeys, transactionsApi } from "@/lib/api";
...
export const useSpendingBreakdown = (
	...
	const categoryMap = new Map<
		number,
		{ name: string; color: string; parentId: number | null }
	>();
	...
	const expensesByCategory = new Map<
		number | null,
		{ amount: number; subcategories: Map<string, number>; count: number }
	>();
	let totalExpenses = 0;
	let totalIncome = 0;
	...
	for (const tx of transactions) {
		...
		if (existing) {
			...
			expensesByCategory.set(parentId, {
				amount: tx.amount,
				subcategories: new Map(),
				count: 1,
			});
	...
	for (const [catId, data] of expensesByCategory) {
		...
		items.push({
			...
			percentage:
				totalAbsExpenses > 0
					? Math.round((Math.abs(data.amount) / totalAbsExpenses) * 100)
					: 0,
