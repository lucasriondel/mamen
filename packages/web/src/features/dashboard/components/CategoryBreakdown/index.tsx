		...
		<TooltipProvider>
			<div className="space-y-3">
				...
				<div className="space-y-2">
					{items.map((item) => (
						<CategoryRow
							...
							item={item}
							isClickable={
								item.categoryId !== null && onCategoryClick !== undefined
							}
				...
				{orphanRefunds !== undefined && orphanRefunds > 0 && (
					<div
						className="text-muted-foreground border-t pt-2 mt-2 flex items-center justify-between px-3"
						data-testid="orphan-refunds-row"
					>
						<span className="text-sm">Refunds (unlinked)</span>
						<span className="text-green-500 font-mono text-sm tabular-nums">
							+{formatCurrency(orphanRefunds)}
						</span>
					</div>
				)}
			</div>
