...
import { getTimePeriodLabel, resolveTimePeriod } from "./resolveTimePeriod";
...
describe("getTimePeriodLabel", () => {
	it("generates correct labels for all presets", () => {
		expect(getTimePeriodLabel({ type: "this-month" }, fixedNow)).toBe(
			"February 2026",
		);
		expect(getTimePeriodLabel({ type: "last-month" }, fixedNow)).toBe(
			"January 2026",
		);
		expect(getTimePeriodLabel({ type: "last-3-months" }, fixedNow)).toBe(
			"Last 3 Months",
		);
