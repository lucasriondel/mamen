  ...
  switch (period.type) {
    ...
    case 'this-year': {
      return {
        ...
        endDate: new Date(year - 1, month, now.getDate(), now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds()),
        label: `${year - 1}`,
      }
    }
...
export const computeComparison = (current: number, previous: number): ComparisonResult => {
  if (previous === 0 && current === 0) {
    return { absoluteChange: 0, percentageChange: 0, direction: 'flat', hasPreviousData: false }
  }

  if (previous === 0) {
    return { absoluteChange: current, percentageChange: 100, direction: 'up', hasPreviousData: false }
  }
...
export const getComparisonLabel = (period: TimePeriod, now = new Date()): string => {
  switch (period.type) {
    case 'this-month':
