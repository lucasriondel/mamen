...
const form = (file: string): IssueForm =>
	parse(readFileSync(`${FORMS}/${file}`, "utf8"));

/** Every issue form, by filename — `config.yml` is settings, not a form. */
const formFiles = () =>
	readdirSync(FORMS).filter(
...
function triageLabel(): string {
	...
	expect(
		row,
		"no needs-triage row in docs/agents/triage-labels.md",
	).toBeDefined();
...
describe("the pull request template", () => {
	...
	it("asks what changed, why, and how it was verified", () => {
		...
		const asks = (word: string) =>
			headings.some((heading) => heading.includes(word));
...
describe("the issue forms", () => {
	it("cover a bug report and a feature request", () => {
		expect(formFiles().sort()).toStrictEqual([
			"bug_report.yml",
			"feature_request.yml",
		]);
	});
