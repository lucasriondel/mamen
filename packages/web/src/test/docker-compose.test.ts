...
/** The API's own listen-port default, from the one place that decides it. */
const apiPortDefault = () =>
...
const documented = (text: string) =>
	new Set(
		[...text.matchAll(/^#?\s*([A-Z_][A-Z0-9_]*)=/gm)].map(
			(match) => match[1] as string,
		),
	);
...
describe("the web service", () => {
	...
	it("leaves VITE_API_URL unset, so the browser calls /api same-origin", () => {
		...
		for (const name of Object.keys(services())) {
			...
			expect(
				Object.keys((service(name).build?.args as object) ?? {}),
				name,
			).not.toContain("VITE_API_URL");
		}
	});
});
...
describe("what a local build sends to the daemon", () => {
	...
	it("keeps the .env.example files, which are what an operator copies", () => {
		...
		expect(
			dockerignore().filter((pattern) => pattern.includes(".env.example")),
		).toStrictEqual([]);
	});
});
...
describe("the operator's .env.example", () => {
	...
	it("documents every variable the compose file reads, and none it does not", () => {
		expect([...documented(read(ENV_EXAMPLE))].sort()).toStrictEqual(
			[...interpolated()].sort(),
		);
	});
