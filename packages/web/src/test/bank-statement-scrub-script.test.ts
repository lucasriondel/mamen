...
const FIXTURE =
	"packages/web/src/features/import/__fixtures__/green-got-sample.csv";
...
const verify = (dir: string, ...args: string[]) =>
	run(dir, ["--verify", ...args]);
...
describe("the script and the working-tree guard", () => {
	it("agree on the digest of what leaked", () => {
		const inGuard = GUARD_TEXT.match(
			/const LEAKED_SHA256 =\s*"([0-9a-f]{64})"/,
		);
...
describe("rewrite mode holds the rewrite to its criteria", () => {
	...
	it("refuses to leave a branch behind on the old history", () => {
		...
		const { status, output } = run(
			dir,
			["--yes"],
			stubFilterRepo("git branch -q -D feature/kept"),
		);
