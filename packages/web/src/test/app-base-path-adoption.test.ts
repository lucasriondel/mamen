...
describe("the nginx template", () => {
  it("serves the app under the prefix, matched exactly", () => {
    ...
    const bare = nginx.match(new RegExp(`location\\s+=\\s+${APP_BASE_PATH}\\s*\\{([^}]*)\\}`))?.[1];
  ...
  it("falls back to the prefixed shell, so a deep link is not a 404", () => {
    const block = nginx.match(new RegExp(`location\\s+${APP_BASE_PATH}/\\s*\\{([^}]*)\\}`))?.[1];
    expect(block).toBeDefined();
    expect(block).toMatch(
      new RegExp(`try_files\\s+\\$uri\\s+\\$uri/\\s+${APP_BASE_PATH}/index\\.html;`),
    );
  });
  ...
  it("keeps /api and /uploads at the root", () => {
    ...
    expect(nginx).toMatch(/location\s+\/api\s*\{[\s\S]*?proxy_pass\s+http:\/\/\$\{API_UPSTREAM\}/);
