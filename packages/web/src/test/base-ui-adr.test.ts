...
describe("the record", () => {
  it("carries the repo's status header and the issue it came from", () => {
    ...
    expect(current).toContain("[ADR 0003](./0003-gousse-is-vendored-from-a-shadcn-registry.md)");
  });
...
describe("every other document", () => {
  ...
  it("leaves ADR 0003 describing the vendoring, not this decision", () => {
    ...
    expect(read(VENDORING)).toContain("[ADR 0004](./0004-base-ui-is-the-only-primitive-system.md)");
  });
});
