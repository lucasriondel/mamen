...
describe("the pre-paint script", () => {
  ...
  it("prefers a stored choice over the OS", () => {
    expect(bootstrap({ stored: "light", prefersDark: true }).scheme).toBe("light");
    expect(bootstrap({ stored: "dark", prefersDark: false }).scheme).toBe("dark");
  });
  ...
  it("treats a stored `system` as no choice at all", () => {
    ...
    expect(bootstrap({ stored: "system", prefersDark: true }).scheme).toBe("dark");
  });
  ...
  it("takes a forced scheme from the URL, for automated capture", () => {
    expect(bootstrap({ search: "?theme=light", prefersDark: true }).scheme).toBe("light");
    expect(bootstrap({ search: "?theme=dark", prefersDark: false }).scheme).toBe("dark");
  });
  ...
  it("beats a contradicting stored choice with the forced one", () => {
    expect(bootstrap({ search: "?theme=light", stored: "dark" }).scheme).toBe("light");
  });
  ...
  it("still paints when storage is unavailable", () => {
    ...
    expect(bootstrap({ storageThrows: true, prefersDark: true }).scheme).toBe("dark");
  });
});
...
describe("the token ramps", () => {
  const declared = (selector: string) =>
    new Set(
      [
        ...`${tokens}\n${indexCss}`.matchAll(new RegExp(`${selector}\\s*\\{([^}]*)\\}`, "g")),
      ].flatMap((block) => [...block[1].matchAll(/(--gousse-[\w-]+):/g)].map((m) => m[1])),
    );
