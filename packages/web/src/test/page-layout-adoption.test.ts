...
const PAGES: Record<string, string> = {
  ...
  "categories.$categoryId.tsx": "src/features/categories/category-transactions-view.tsx",
  ...
  "issuers.$issuerId.rules.$ruleId.tsx": "src/features/rules/rule-form-page.tsx",
  ...
  "transactions_.$transactionId.tsx": "src/features/transactions/transaction-detail-content.tsx",
...
describe("the shared page layout", () => {
  ...
  it("left no `PageHeader` behind", () => {
    ...
    const callers = FILES.filter(({ text }) => /from "[^"]*page-header"/.test(text)).map(
      ({ path }) => path,
    );
  ...
  it("accounts for every route — as a page, or as a reason it is not one", () => {
    const routes = readdirSync(join("src", "routes")).filter((file) => file.endsWith(".tsx"));
