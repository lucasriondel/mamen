...
export function AccountsPage(): React.ReactElement {
	...
	const [reimportState, setReimportState] = useState<ReimportState | null>(
		null,
	);
	const [importModalState, setImportModalState] =
		useState<ImportModalState | null>(null);
	const [pdfPreviewState, setPdfPreviewState] =
		useState<PDFPreviewState | null>(null);
	const [pdfParsingState, setPdfParsingState] =
		useState<PDFParsingState | null>(null);
	const [llmErrorState, setLlmErrorState] = useState<LLMErrorState | null>(
		null,
	);
	const [llmNotConfiguredState, setLlmNotConfiguredState] =
		useState<LLMNotConfiguredState | null>(null);
	...
	const handleFileDropped = (
		...
		if (transactionCount > 0) {
			setReimportState({
				...
				monthKey,
				accountId,
				existingCount: transactionCount,
			});
		} else {
			...
			setLlmErrorState({
				message:
					err instanceof Error
						? err.message
						: "Could not read PDF file. The file may be corrupted or password-protected.",
				errorType: "parse",
				file,
				monthKey,
				accountId,
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							will also remove {deleteTransactionCount}{" "}
							{deleteTransactionCount === 1 ? "transaction" : "transactions"}.
			...
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>{llmErrorState?.message}</DialogDescription>
					</DialogHeader>
					<DialogFooter className="flex-col gap-2 sm:flex-row">
						{(llmErrorState?.errorType === "network" ||
							llmErrorState?.errorType === "timeout") && (
							<Button onClick={handleRetryPDF}>Retry</Button>
						)}
			...
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Set up an LLM to parse PDF statements. You can configure a local
							LLM (Ollama) or provide your own cloud API key in Settings.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter className="flex-col gap-2 sm:flex-row">
						<Button
