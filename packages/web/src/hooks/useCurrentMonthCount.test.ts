...
describe('useCurrentMonthCount', () => {
  it('returns correct count for current month transactions', async () => {
    ...
    await db.transactions.bulkAdd([
      ...
      makeTransaction({ date: new Date(now.getFullYear(), now.getMonth(), 15) }),
      makeTransaction({ date: new Date(now.getFullYear(), now.getMonth(), 20) }),
    ])
  ...
  it('excludes transactions from other months', async () => {
    ...
    await db.transactions.bulkAdd([
      makeTransaction({ date: new Date(now.getFullYear(), now.getMonth(), 10) }),
