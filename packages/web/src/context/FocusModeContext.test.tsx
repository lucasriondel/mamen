	...
	return (
		<div>
			...
			<span data-testid="filters">
				{Array.from(activeFilters).sort().join(",") || "none"}
			</span>
			<span data-testid="month-start">
...
describe("FocusModeContext", () => {
	...
	it("currentMonthRange returns correct start/end dates", () => {
		...
		expect(screen.getByTestId("month-start")).toHaveTextContent(
			expectedStart.toISOString(),
		);
		expect(screen.getByTestId("month-end")).toHaveTextContent(
