...
import { detectSubscriptions, areAmountsSimilar, runDetection } from './subscriptionDetector'

const createAccount = () =>
  db.accounts.add({ name: 'Test', type: 'checking', createdAt: new Date(), updatedAt: new Date() })
...
describe('detectSubscriptions', () => {
  ...
  it('excludes refund transactions from detection', async () => {
    ...
    await addTransaction(accountId, merchantId, 25.0, '2025-11-01', { isRefund: true })
  ...
  it('uses most recent amount as typical for 2-transaction subscription', async () => {
    ...
    await addTransaction(accountId, merchantId, -10.50, '2025-01-15')

    const subs = await detectSubscriptions()
    expect(subs).toHaveLength(1)
    expect(subs[0].typicalAmount).toBe(-10.50)
  })
})
...
describe('runDetection', () => {
  ...
  it('extends transactionIds list as new charges are detected', async () => {
    ...
    const tx1 = await addTransaction(accountId, merchantId, -9.99, '2025-10-15')
    const tx2 = await addTransaction(accountId, merchantId, -9.99, '2025-11-15')
    ...
    const tx3 = await addTransaction(accountId, merchantId, -9.99, '2025-12-15')
    await runDetection()
  ...
  it('reactivates possibly-cancelled subscription when new charge detected', async () => {
    ...
    await addTransaction(accountId, merchantId, -20.0, twoMonthsAgo.toISOString().split('T')[0])
    await addTransaction(accountId, merchantId, -20.0, oneMonthAgo.toISOString().split('T')[0])
  ...
  it('keeps active subscription when recent charge exists', async () => {
    ...
    await addTransaction(accountId, merchantId, -12.99, threeMonthsAgo.toISOString().split('T')[0])
    await addTransaction(accountId, merchantId, -12.99, twoMonthsAgo.toISOString().split('T')[0])
    await addTransaction(accountId, merchantId, -12.99, oneMonthAgo.toISOString().split('T')[0])
