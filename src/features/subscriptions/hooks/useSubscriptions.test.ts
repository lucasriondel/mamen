...
describe('useSubscriptions', () => {
  ...
  it('correctly separates active from possibly-cancelled', async () => {
    await db.subscriptions.bulkAdd([
      ...
      makeSub({ merchantId: 2, merchantName: 'OldService', status: 'possibly-cancelled' }),
    ])
  ...
  it('computes monthlyTotal from active monthly subscriptions only', async () => {
    await db.subscriptions.bulkAdd([
      makeSub({ merchantId: 1, merchantName: 'Netflix', typicalAmount: -15.99, frequency: 'monthly', status: 'active' }),
      makeSub({ merchantId: 2, merchantName: 'Spotify', typicalAmount: -9.99, frequency: 'monthly', status: 'active' }),
      makeSub({ merchantId: 3, merchantName: 'Yearly', typicalAmount: -99.99, frequency: 'yearly', status: 'active' }),
      makeSub({ merchantId: 4, merchantName: 'Cancelled', typicalAmount: -5.0, frequency: 'monthly', status: 'possibly-cancelled' }),
    ])
  ...
  it('computes yearlyTotal normalizing all frequencies', async () => {
    await db.subscriptions.bulkAdd([
      makeSub({ merchantId: 1, typicalAmount: -10.0, frequency: 'monthly', status: 'active' }),
      makeSub({ merchantId: 2, typicalAmount: -100.0, frequency: 'yearly', status: 'active' }),
      makeSub({ merchantId: 3, typicalAmount: -5.0, frequency: 'weekly', status: 'active' }),
    ])
