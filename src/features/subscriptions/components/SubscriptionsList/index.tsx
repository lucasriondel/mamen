...
type SortField = 'amount-desc' | 'amount-asc' | 'name' | 'lastCharge' | 'frequency'
...
const sortSubscriptions = (subs: Subscription[], field: SortField): Subscription[] => {
  const sorted = [...subs]
  switch (field) {
    case 'amount-desc':
      return sorted.sort((a, b) => Math.abs(b.typicalAmount) - Math.abs(a.typicalAmount))
    case 'amount-asc':
      return sorted.sort((a, b) => Math.abs(a.typicalAmount) - Math.abs(b.typicalAmount))
    case 'name':
      return sorted.sort((a, b) => a.merchantName.localeCompare(b.merchantName))
    case 'lastCharge':
      return sorted.sort((a, b) => b.lastChargeDate.localeCompare(a.lastChargeDate))
    case 'frequency':
      return sorted.sort((a, b) => frequencyOrder[a.frequency] - frequencyOrder[b.frequency])
  }
}

export function SubscriptionsList({ subscriptions }: SubscriptionsListProps): React.ReactElement {
  ...
  return (
    <div ref={containerRef} tabIndex={-1} className="flex flex-col flex-1 outline-none">
      <div className="flex items-center gap-3 px-4 py-3 border-b">
        ...
        <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as StatusFilter)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="possibly-cancelled">Possibly cancelled</SelectItem>
          </SelectContent>
        </Select>
