...
const subscriptions: Subscription[] = [
  makeSub({ id: 1, merchantName: 'Netflix', typicalAmount: -15.99, frequency: 'monthly', lastChargeDate: '2026-01-18' }),
  makeSub({ id: 2, merchantName: 'Spotify', typicalAmount: -9.99, frequency: 'monthly', lastChargeDate: '2026-01-15', merchantId: 20 }),
  makeSub({ id: 3, merchantName: 'Adobe', typicalAmount: -119.88, frequency: 'yearly', lastChargeDate: '2025-11-01', merchantId: 30 }),
  makeSub({ id: 4, merchantName: 'Gym', typicalAmount: -49.99, frequency: 'monthly', status: 'possibly-cancelled', lastChargeDate: '2025-10-01', merchantId: 40 }),
  makeSub({ id: 5, merchantName: 'Newspaper', typicalAmount: -4.99, frequency: 'weekly', lastChargeDate: '2026-01-20', merchantId: 50 }),
]
...
describe('SubscriptionsList', () => {
  ...
  it('default sort is by amount descending', () => {
    const { container } = render(<SubscriptionsList subscriptions={subscriptions} />)
    ...
    expect(within(rows[2] as HTMLElement).getByText('Netflix')).toBeInTheDocument()
  })
  ...
  it('filter by frequency shows only matching', async () => {
    ...
    const frequencyTrigger = screen.getByRole('combobox', { name: /frequency/i })
  ...
  it('filter by status shows only matching', async () => {
    ...
    const cancelledOption = screen.getByRole('option', { name: /Possibly cancelled/i })
