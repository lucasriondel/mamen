...
describe('exportAllData', () => {
  ...
  it('metadata has correct record counts', async () => {
    await db.accounts.add({ name: 'A1', type: 'checking', createdAt: new Date(), updatedAt: new Date() })
    await db.accounts.add({ name: 'A2', type: 'savings', createdAt: new Date(), updatedAt: new Date() })
    await db.transactions.add({
      accountId: 1,
      date: new Date(),
      amount: 10,
  ...
  it('full integration: seeds all tables and verifies complete export', async () => {
    ...
    await db.rules.add({ merchantId, pattern: 'AMZN.*', matchCount: 5, createdAt: new Date() })
    await db.transactions.add({
      accountId,
      date: new Date(),
      amount: 50,
    ...
    await db.categories.add({ name: 'Shopping', slug: 'shopping', color: '#ff0000', icon: 'bag', parentId: null, sortOrder: 0, createdAt: new Date() })
  ...
  it('preserves merchant-rule relationships', async () => {
    ...
    await db.rules.add({ merchantId, pattern: 'NETFLIX.*', matchCount: 3, createdAt: new Date() })
    await db.rules.add({ merchantId, pattern: 'NFLX.*', matchCount: 1, createdAt: new Date() })
  ...
  it('handles unicode and special characters in merchant strings', async () => {
    ...
    expect(data.transactions[0].rawMerchantString).toBe('Café Résumé 日本語 "quotes" & <special>')
  })
  ...
  it('selective export: only transactions yields empty other arrays', async () => {
    await db.accounts.add({ name: 'A', type: 'checking', createdAt: new Date(), updatedAt: new Date() })
    await db.merchants.add({ name: 'M', createdAt: new Date(), firstSeen: new Date() })
    await db.transactions.add({
      accountId: 1,
      date: new Date(),
      amount: 5,
