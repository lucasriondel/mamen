...
import { testConnection, getProviderDefaults, MODEL_SUGGESTIONS, isLLMConfigured } from '@/lib/llm/client'
import type { LLMProvider, LLMSettings, AppSettings } from '@/types'
...
const PROVIDER_OPTIONS: LLMProvider[] = ['ollama', 'lm-studio', 'openai', 'anthropic', 'custom']
...
export function LLMConfigForm(): React.ReactElement {
  ...
  const handleProviderChange = useCallback((provider: LLMProvider): void => {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Configure your LLM provider for PDF statement parsing. Use a local model for privacy or your own cloud API key.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        <div className="space-y-2">
          ...
          <Select value={llm.provider} onValueChange={(v) => handleProviderChange(v as LLMProvider)}>
        ...
        {requiresApiKey(llm.provider) ? (
          <div className="space-y-2">
            ...
            {llm.provider !== 'custom' && (
              <p className="text-xs text-muted-foreground">
                Your API key will be sent to {llm.endpoint || 'the configured endpoint'}
              </p>
            )}
          </div>
        ) : (
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">Not required for local LLM</p>
          </div>
        )}
...
function ConnectionStatusBadge({ status }: ConnectionStatusBadgeProps): React.ReactElement {
  switch (status) {
    case 'connected':
      return (
