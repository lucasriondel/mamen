...
describe('Net Spending Integration', () => {
  ...
  it('all transactions refunded shows EUR0 for categories', async () => {
    ...
    const r1 = await db.transactions.add(
      makeTransaction({ amount: 100, categoryId: catId, isRefund: true, linkedRefundId: p1 }) as Transaction,
    ) as number
    ...
    const r2 = await db.transactions.add(
      makeTransaction({ amount: 50, categoryId: catId, isRefund: true, linkedRefundId: p2 }) as Transaction,
    ) as number
  ...
  it('time period change recalculates net spending', async () => {
    ...
    await db.transactions.add(
      makeTransaction({ amount: -200, categoryId: catId, date: lastMonth, importMonth: lastMonthImport }) as Transaction,
    )
