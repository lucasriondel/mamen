...
describe('useTimePeriod', () => {
  ...
  it('resolves date range correctly', () => {
    ...
    const now = new Date()
    expect(result.current.resolvedRange.startDate).toEqual(new Date(now.getFullYear(), now.getMonth(), 1))
  })
  ...
  it('updates resolved range when period changes', () => {
    ...
    const now = new Date()
    expect(result.current.resolvedRange.startDate).toEqual(new Date(now.getFullYear(), 0, 1))
