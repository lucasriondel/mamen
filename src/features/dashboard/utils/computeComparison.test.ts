...
import { getPreviousPeriodRange, computeComparison, getComparisonLabel } from './computeComparison'
...
describe('getPreviousPeriodRange', () => {
  ...
  it('shifts custom range back by its duration', () => {
    ...
    const prevDurationMs = result.endDate.getTime() - result.startDate.getTime()
...
describe('getComparisonLabel', () => {
  ...
  it('returns "vs previous 3 months" for last-3-months', () => {
    expect(getComparisonLabel({ type: 'last-3-months' })).toBe('vs previous 3 months')
  })
