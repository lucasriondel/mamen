...
import { assignManualCategory, undoManualCategoryAssignment } from './assignManualCategory'
...
describe('assignManualCategory', () => {
  ...
  it('returns previous state for undo', async () => {
    const txId = (await db.transactions.add(
      makeTransaction({ merchantId: 5, categoryId: 3, subcategoryId: 7, manualCategory: false }),
    )) as number
