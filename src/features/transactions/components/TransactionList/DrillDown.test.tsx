...
describe('Drill-down integration', () => {
  it('filters transactions by categoryId from search params', async () => {
    const catId = await db.categories.add({
      name: 'Shopping', slug: 'shopping', color: '#3B82F6', icon: 'ShoppingCart',
      parentId: null, sortOrder: 0, createdAt: new Date(),
    })

    await db.transactions.bulkAdd([
      makeTransaction({ rawMerchantString: 'SHOP A', categoryId: catId as number, amount: -10 }),
      makeTransaction({ rawMerchantString: 'SHOP B', categoryId: catId as number, amount: -20 }),
      makeTransaction({ rawMerchantString: 'DINING C', categoryId: 999, amount: -30 }),
  ...
  it('shows filter indicator bar when drill-down is active', async () => {
    ...
    expect(screen.getByTestId('category-filter-chip')).toHaveTextContent('Shopping')
  ...
  it('does not show filter indicator when no drill-down', async () => {
    ...
    expect(screen.queryByTestId('drill-down-filter-bar')).not.toBeInTheDocument()
  })
  ...
  it('shows empty state with category name when drill-down has no matching transactions', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No transactions in Shopping/)).toBeInTheDocument()
    })
  ...
  it('filters by date range from search params', async () => {
    ...
    await db.transactions.bulkAdd([
      makeTransaction({
        rawMerchantString: 'FEB SHOP', categoryId: catId as number, amount: -10,
        date: new Date(2026, 1, 15),
      }),
      makeTransaction({
        rawMerchantString: 'JAN SHOP', categoryId: catId as number, amount: -20,
