...
export function AmountFilter({ filterValues, onAmountRange, onAmountPrecise, onClear }: AmountFilterProps) {
  const [open, setOpen] = useState(false)
  const [mode, setMode] = useState<'range' | 'precise'>(filterValues.amountMode ?? 'range')
  const [min, setMin] = useState(filterValues.amountMin?.toString() ?? '')
  const [max, setMax] = useState(filterValues.amountMax?.toString() ?? '')
  const [precise, setPrecise] = useState(filterValues.amountPrecise?.toString() ?? '')
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="w-64" align="start">
        <div className="flex flex-col gap-3">
          <RadioGroup value={mode} onValueChange={(v) => setMode(v as 'range' | 'precise')}>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="range" id="amount-range" />
              <Label htmlFor="amount-range" className="text-xs font-normal">Range</Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="precise" id="amount-precise" />
              <Label htmlFor="amount-precise" className="text-xs font-normal">Exact amount</Label>
            </div>
          </RadioGroup>
