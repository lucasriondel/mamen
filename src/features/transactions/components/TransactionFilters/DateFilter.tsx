...
export function DateFilter({ filterValues, onDateExact, onDateRange, onMonthYear, onClear }: DateFilterProps) {
  const [open, setOpen] = useState(false)
  const [mode, setMode] = useState<'exact' | 'range' | 'month'>(filterValues.dateMode ?? 'exact')
  const [exactDate, setExactDate] = useState<Date | undefined>(filterValues.dateExact)
  ...
  const handleApply = () => {
    if (mode === 'exact' && exactDate) {
      ...
      onMonthYear({ month: parseInt(selectedMonth), year: parseInt(selectedYear) })
    }
    setOpen(false)
  }
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="w-auto p-0" align="start">
        <div className="flex flex-col gap-3 p-3">
          <RadioGroup value={mode} onValueChange={(v) => setMode(v as 'exact' | 'range' | 'month')}>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="exact" id="date-exact" />
              <Label htmlFor="date-exact" className="text-xs font-normal">Exact date</Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="range" id="date-range" />
              <Label htmlFor="date-range" className="text-xs font-normal">Date range</Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="month" id="date-month" />
              <Label htmlFor="date-month" className="text-xs font-normal">Month + Year</Label>
            </div>
          </RadioGroup>
        </div>
