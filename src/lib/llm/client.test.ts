...
import { isLLMConfigured, getLLMError, testConnection, getProviderDefaults } from './client'
...
describe('isLLMConfigured', () => {
  ...
  it('returns true for lm-studio without API key', () => {
    expect(isLLMConfigured({ ...ollamaSettings, provider: 'lm-studio' })).toBe(true)
  })

  it('returns false for openai without API key', () => {
    expect(isLLMConfigured({ ...openaiSettings, apiKey: undefined })).toBe(false)
  })
...
describe('getLLMError', () => {
  it('returns error when settings is undefined', () => {
    expect(getLLMError(undefined)).toBe('LLM not configured. Set up in Settings first.')
  })

  it('returns error when endpoint is empty', () => {
    expect(getLLMError({ ...ollamaSettings, endpoint: '' })).toBe('LLM endpoint not configured.')
  })

  it('returns error when modelName is empty', () => {
    expect(getLLMError({ ...ollamaSettings, modelName: '' })).toBe('LLM model not selected.')
  })

  it('returns error for cloud provider without API key', () => {
    expect(getLLMError({ ...openaiSettings, apiKey: undefined })).toBe('API key required for cloud providers.')
  })
