...
export const MODEL_SUGGESTIONS: Record<string, string[]> = {
  ollama: ['llama3.2', 'llama3.1', 'mistral', 'mistral-nemo', 'qwen2.5', 'qwen2.5-coder', 'deepseek-r1', 'phi-4'],
  'lm-studio': [],
...
export const isLLMConfigured = (settings?: LLMSettings): boolean => {
  ...
  const needsKey = settings.provider !== 'ollama' && settings.provider !== 'lm-studio'
...
export const getLLMError = (settings?: LLMSettings): string | null => {
  ...
  const needsKey = settings.provider !== 'ollama' && settings.provider !== 'lm-studio'
  if (needsKey && !settings.apiKey) return 'API key required for cloud providers.'
  return null
}
...
export const testConnection = async (settings: LLMSettings): Promise<TestConnectionResult> => {
  ...
  try {
    if (settings.provider === 'ollama') {
      ...
      if (!response.ok) return { success: false, message: 'Cannot connect to Ollama. Check if it\'s running.' }
    ...
    if (!response.ok) {
      if (response.status === 401) {
        return { success: false, message: 'Invalid API key. Check your key in settings.' }
      }
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      return { success: false, message: 'Connection timed out. LLM may be overloaded.' }
    }
    return {
      success: false,
